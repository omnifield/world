...
#
# ПОДСКАЗКА — ЭТО ШАПКА, И ГРАНИЦА У НЕЁ ОДНА: первая строка, которая не комментарий.
# Номера строк здесь не стоит нарочно. Зашитый диапазон (`2,28p`) — вторая правда о том,
# где кончается шапка: правится он руками и отдельно от неё, и этой же сдачей уже
# разъезжался (`21` → `28`). Дописал строку в шапку — подсказка молча теряла хвост.
# Разбор: `1d` снимает строку `#!`, `/^#/!q` останавливает на первой строке кода.
case "${1:-}" in
