...
/**
 * Конфиг объявляет зоны, которых в этом репозитории нет НИ ОДНОЙ → он не отсюда.
...
/**
 * Адрес чекпойнта роли. Правило рамки без адреса не исполняется: агент о разделе просто не
 * узнает, а «в конце сессии» вспоминать будет уже некому. Слот `checkpoints` не объявлен →
 * блок ПУСТОЙ: молчим, адрес не выдумываем (как с `grabli`).
...
export function checkpointNotice(config, roleLabel) {
  ...
  if (!target) return [];
  const ws = target.workspace ? ` (раздел \`${target.workspace}\`)` : "";
  return [
    ``,
    `## Чекпойнт роли — состояние прогона, корень \`${target.root}\`${ws}`,
    `- Узел на РОЛЬ (**${roleLabel}**), не на сессию: свой ищи рядом с корнем, нового не заводи.`,
    `  Витрина одна — **Windshift, через MCP-инструменты**: \`get_item\`, \`get_item_children\`, \`list_items\`.`,
    ...
    `- Чекпойнт ≠ знания: здесь состояние прогона, а решения и контракты — страницами того же раздела.`,
    `- Подхватываешь чекпойнт — СВЕРЬ его с фактом (ветка, статусы узлов) ДО первого действия: он протухает молча.`,
  ];
}
...
function onboardingBanner(config) {
  return [
    ...
    `   Доступ — **MCP-инструментами Windshift** (\`list_workspaces\`, \`search_items\`, \`list_pages\`);`,
    `   адрес — слот \`services\` в \`harness.yaml\`. Инструментов не видно — СКАЖИ USER И ОСТАНОВИСЬ.`,
...
function architectBanner(config) {
  return [
    ...
    `- Триаж запросов user; арх-решения и контракты пишешь **страницами канона**; координируешь овнеров **узлами-задачами**.`,
    `- **НЕ пиши код зон сам** — ставишь задачу овнеру узлом → owner-сессию запускает user.`,
    `- Локальных \`briefs/\`-файлов НЕ заводим: истина снаружи репозитория, на витрине.`,
...
function ownerBanner(config, { scope, paths, name }) {
  ...
  return [
    ...
    `- Перед первым Edit прочитай канон продукта на витрине (Windshift, MCP) + \`${firstPath}/README.md\` (если есть).`,
    ...
    ``,
    `## Скоуп задачи`,
    `Работаешь по **узлу-задаче на витрине** (Windshift, через MCP: \`get_item\` по ключу) или прямому ТЗ от user —`,
    `НЕ по локальным файлам. **Витрина одна, других сервисов знания и задач у продукта нет** — искать негде и незачем.`,
    `Отчёт и вопросы по задаче — комментом в её узел (\`add_comment\`), не в чат. Непонятен scope — STOP, спроси. Не угадывай.`,
  ].join("\n");
}
 ...
 * Говорим сразу и с готовой командой запуска.
 */
