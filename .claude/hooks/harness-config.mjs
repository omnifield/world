...
export const DEFAULT_CONFIG = {
  ...
  zones: {},
  git: { ...GIT_INVARIANT },
  // Слот grabli: куда агенты пишут затыки/грабли. Дефолта нет — если продукт
  // не задал, запись грабли не адресована (правило рамки остаётся, но канал не сконфигурен).
  grabli: null,
  // Слот services: адрес витрины продукта — единственного источника истины (канон, роадмап,
  // работа). Агент ходит туда MCP-инструментами; REST остаётся людям. Адрес зависит от
  // окружения и может смениться вместе с туннелем. null → не сконфигурен.
  services: null,
  // Слот pilots: раздел пилотов — общая площадка продуктов, где меха обкатывается ДО каталога
  // канонов. null → продукт раздела не знает, правило молчит. Читатели слота сняты 2026-08-22
  // вместе с адресами сторонних сервисов знания и задач; поле оставлено, чтобы чужой конфиг
  // с этой секцией не падал при чтении.
  pilots: null,
  // Слот checkpoints: корень чекпойнтов ролей — состояние прогона, чтобы
 ...
 * Нужно для `zones.<z>.paths: [packages/a, packages/b]`. Block-list (`- x`)
...
/** Целевой ws для записи граблей/затыков, либо null если слот не сконфигурен. */
export function grabliTarget(config) {
  const ws = config?.grabli?.workspace;
  return typeof ws === "string" && ws.trim() ? ws.trim() : null;
}

/** База сервиса витрины (`windshift_mcp`/`windshift`) из слота services; null если не задан. */
...
/**
 * Корень чекпойнтов ролей: `{ workspace, root }`, либо null — слот не объявлен.
...
/**
 * Пути зоны как МАССИВ. Толерантный
...
/**
 * Детерминированный валидатор роль-модели:
 ...
 * Механику не трогаем — перестаём врать: это раскладка, а не ошибка.
 */
...
// состояние разное, причём зелёный был у того, которым установку ПРОВЕРЯЮТ.
 ...
 * отметки в вывод, а баннер существование папок не смотрел вовсе.
 */
 ...
 * Пересечение = машинной границы между этими зонами нет; задумано так — законно.
 */
