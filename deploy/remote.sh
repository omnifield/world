...
cmd_add() {
    ...
    if [ -n "$have_remote" ] && [ -z "$RESEND" ]; then
        ...
        docker image inspect "$image" >/dev/null 2>&1 || refuse no-image \
            ...
            "сборке нужно поле (склад пульта виден только соседям по сети $NET) — подробности в deploy/README.md"
...
$(printf '\033[1;32m✓ дверь стоит на ресурсе %s\033[0m' "$NAME")
  ...
  вход снаружи      http://$HOST:$PORT/            (если порт открыт хозяином ресурса)
