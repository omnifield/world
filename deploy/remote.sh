...
install_remote_docker() {
    ...
    # 4. САМА УСТАНОВКА. Команда печатается ЦЕЛИКОМ и до запуска — той же строкой, которая
    # уедет на машину: показанное и выполненное строятся в одном месте (`install_cmd`).
    local cmd; cmd="$(install_cmd "$fetch" "$runner")"
    say ""
