...
Проба подъёма мира: ./deploy/probe.sh [--green|--red|--both] [--build]
  ...
  --green    (умолчание) поднять и проверить: healthz · пульт · регистрация ·
             маршрут · пересоздание сохраняет поле
  ...
  --build    пересобрать образ перед подъёмом (нужно поле). Без ключа проба
             проверяет тот образ, который уже лежит, — то есть то, что поедет
