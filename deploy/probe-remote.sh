#!/usr/bin/env bash
# Проба ВЕЩИ НА ЧУЖОМ РЕСУРСЕ — что `remote.sh` делает, чего НЕ делает и как отказывает.
#
#   ./deploy/probe-remote.sh            (умолчание) зелёный прогон, затем красный
#   ./deploy/probe-remote.sh --green    только зелёный: что инструмент обязан делать
#   ./deploy/probe-remote.sh --red      только красный: нарочные поломки
#   ./deploy/probe-remote.sh --live     ЖИВОЙ прогон на настоящем втором ресурсе
#
# ┌─────────────────────────────────────────────────────────────────────────────────────┐
# │ ЧТО ЭТА ПРОБА СТЕРЕЖЁТ                                                               │
# │                                                                                      │
# │  · ВЕЩЬ ОПИСАНА РЕЦЕПТОМ, А НЕ ЗАШИТА: проба кладёт ВТОРОЙ рецепт — чужой образ,     │
# │    чужое имя контейнера, чужой порт — и поднимает его тем же путём. Потребовалась    │
# │    бы правка кода — вещь была зашита, и это дефект (`WORLD2` 3.7);                   │
# │  · ГЛАВНОЕ СВОЙСТВО: на чужой машине мир делает ТОЛЬКО ОДНО — узнаёт про докер и     │
# │    ставит его, если его нет (`WORLD2` 3.5 п. 7, канон переписан 2026-08-18). Ни      │
# │    `scp`, ни файлов, ни команды о чём-либо ещё; на машине, где докер ЕСТЬ, не        │
# │    выполняется вовсе ничего сверх проверки доступа. Проверяется по журналу вызовов,  │
# │    то есть по тому, чего не было;                                                    │
# │  · «ГОВОРИМ ДО, А НЕ ПОСЛЕ» — проверяется ПОРЯДКОМ, а не наличием фразы: слово о том,│
# │    что на машине появится докер, и сама команда обязаны стоять раньше установки;     │
# │  · ПУТЬ ДОСТАВКИ ОБРАЗА ВЫБИРАЕТ НАЛИЧИЕ, а не человек и не умолчание: есть образ у  │
# │    контроллера — копия, нет — та машина тянет сама. Прогоняются ОБА пути, а не тот,  │
# │    что удобнее подставе, и каждый обязан быть НАЗВАН до того, как займёт минуты;     │
# │  · ступени НЕ СХЛОПЫВАЮТСЯ: «ключ не принят» · «докера нет» · «докер есть, а демон   │
# │    молчит» · «нет прав ставить» · «машина не достала до сети» — разные отказы с      │
# │    разными кодами, потому что чинят их разные люди (`WORLD2` 2.3);                   │
# │  · образ не везётся второй раз, когда он там уже лежит;                              │
# │  · снятие убирает и контейнер на той стороне, и контекст на этой, а состояние        │
# │    (тома) — только когда об этом попросили явно.                                     │
# │                                                                                      │
# │ ДОКЕР И SSH ЗДЕСЬ ПОДСТАВНЫЕ — скрипты-заглушки в `PATH`, которые пишут журнал       │
# │ вызовов и отвечают по сценарию. Ни одного контейнера не запускается, ни одного       │
# │ ресурса не трогается. Это сказано прямо, а не подразумевается.                       │
# │                                                                                      │
# │ ЗЕЛЁНАЯ ЭТА ПРОБА НЕ ЗНАЧИТ, ЧТО ДВЕРЬ ВСТАЁТ НА ЧУЖОМ РЕСУРСЕ. Она стережёт         │
# │ ветвление и отказы. Живой прогон — `--live`, и он ничем не заменяется.               │
# └─────────────────────────────────────────────────────────────────────────────────────┘
#
#   код 0   полный зелёный: прогнано всё, включая живой прогон на втором ресурсе
#   код 1   краснота: инструмент повёл себя не так, как обязан
#   код 3   НЕПОЛНЫЙ ПРОГОН: часть проверок не выполнялась, и названо — какая
#
# Зелёного при неполном прогоне не бывает: «проверили, что смогли» и «проверили» — разные
# слова (`WORLD2` 4.2, пункт 5: чего не удалось измерить, не записывается вовсе).
#
# Живой прогон — на настоящем втором ресурсе, где есть докер и ssh с ключом:
#
#   WORLD_PROBE_REMOTE_ADDR=world@10.8.0.5 ./deploy/probe-remote.sh --live
#
# ОН РАЗРУШИТЕЛЕН ДЛЯ МИРА, СТОЯЩЕГО НА ТОМ РЕСУРСЕ: поднимает дверь под своим именем
# ресурса и снимает её вместе с томами и образом. Мир на том ресурсе снимается тем же
# `compose down` — если он там нужен, живой прогон гоняют на другой машине.
#
# Проверки зовутся косвенно, по имени в аргументе; линтер читает это как «функция не вызвана».
# shellcheck disable=SC2329
set -uo pipefail   # без -e: проба сама разбирает коды возврата, падать ей нельзя
export LC_ALL=C

HERE="$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")" && pwd)"
REMOTE="$HERE/remote.sh"
# ЭТО ОЖИДАНИЯ ПРОБЫ ПРО РЕЦЕПТ ДВЕРИ, А НЕ ЗНАНИЕ МЕХАНИКИ. Проба вправе знать, что лежит в
# `deploy/compose.yaml`, — она его и стережёт: имя образа и имя контейнера обязаны приезжать
# ОТТУДА. Инструменту знать их неоткуда, и отдельная проверка следит, чтобы они не вернулись
# в `remote.sh` константами.
IMAGE=ghcr.io/omnifield/world:latest
NET=omnifield-gateway
DOOR=world-door
# Откуда берётся докер, когда его на машине нет. ЭТО ОЖИДАНИЕ ПРОБЫ, а не знание механики:
# значение принадлежит `remote.sh` (`DOCKER_INSTALL_URL`), а проба стережёт, что мир ставит
# ГОТОВОЕ И ОБЩЕЕ — официальный установщик докера, — а не свой разбор дистрибутивов
# (`WORLD2` 3.5, таблица «кто изготовитель»).
INSTALL_URL=get.docker.com
# Имя ресурса для живого прогона. Своё и приметное: контекст `world-probe` не спутать с
# ресурсом человека, а `drop` в конце прогона убирает именно его.
LIVE_NAME="${WORLD_PROBE_REMOTE_NAME:-probe}"
LIVE_ADDR="${WORLD_PROBE_REMOTE_ADDR:-}"

mode=--both
case "${1:-}" in
    --green|--red|--both|--live) mode="$1" ;;
    -h|--help)
        sed -n '2,45p' "${BASH_SOURCE[0]}" | sed 's/^# \{0,1\}//'; exit 0 ;;
    "") ;;
    *)  printf 'непонятный ключ: %s (см. --help)\n' "$1" >&2; exit 2 ;;
esac

total=0; failed=0; skipped=0

part()   { printf '\n\033[1m%s\033[0m\n' "$*" >&2; }
detail() { printf '      %s\n' "$*" >&2; }
ok()     { total=$((total + 1)); printf '  \033[32m✔\033[0m %s\n' "$1" >&2; }
bad()    { total=$((total + 1)); failed=$((failed + 1))
           printf '  \033[31m✘\033[0m %s\n' "$1" >&2
           shift; local l; for l in "$@"; do detail "$l"; done; }
skip()   { skipped=$((skipped + 1)); printf '  \033[33m•\033[0m %s — НЕ ПРОГНАНО\n' "$1" >&2
           shift; local l; for l in "$@"; do detail "$l"; done; }

# ------------------------------------------------------------------ подставные инструменты
# Отдельными файлами в PATH, а не функциями: `remote.sh` — отдельный процесс, и функция до
# него не доехала бы. Файлы рождаются на прогон и умирают вместе с ним.
STUB_DIR="$(mktemp -d)"
LOG="$STUB_DIR/journal"
# ПОРЯДОК — отдельный файл, и он не то же самое, что журнал. В журнал пишутся вызовы, а сюда
# ложится ВСЁ ПОДРЯД: и то, что инструмент сказал человеку, и то, что подставные инструменты
# выполнили. Только так проверяется «говорим ДО, а не после» (`WORLD2` 3.5): слово и действие
# лежат в одном потоке, и видно, что раньше. По двум разным файлам порядок не сверить, а
# правило «ДО» — это правило про порядок, а не про наличие фразы.
ORDER="$STUB_DIR/poryadok"

cleanup() { rm -rf "$STUB_DIR"; return 0; }
trap cleanup EXIT

cat > "$STUB_DIR/docker" <<'STUB'
#!/usr/bin/env bash
# ПОДСТАВНОЙ докер: пишет журнал вызовов и отвечает по сценарию. Контейнеров не запускает,
# на чужие машины не ходит. Сценарий задаётся переменными STUB_* — их выставляет проба.
#
# РЕЦЕПТ ОН ЧИТАЕТ ПО-НАСТОЯЩЕМУ — тот файл, который ему назвали ключом `-f`. Иначе проба
# зеленела бы на подставе: подмени рецепт, а ответы те же — и «вещь берётся из рецепта»
# осталось бы словами (`WORLD2` 4.2).
printf 'docker %s\n' "$*" >> "${STUB_LOG:-/dev/null}"

RECIPE=""; prev=""; LAST=""; PROJ_FILTER=""
for a in "$@"; do
    [ "$prev" = "-f" ] && RECIPE="$a"
    case "$a" in label=com.docker.compose.project=*) PROJ_FILTER="${a##*=}" ;; esac
    prev="$a"; LAST="$a"
done

# proj — имя проекта, как его вывел бы компоуз: `name:` из рецепта, иначе имя файла.
proj() {
    local n=""
    [ -n "$RECIPE" ] && [ -f "$RECIPE" ] && n="$(sed -n 's/^name: *//p' "$RECIPE" | head -n1)"
    [ -n "$n" ] || { n="$(basename -- "${RECIPE%.*}")"; }
    printf '%s' "$n"
}
# ids — по контейнеру на каждую службу с образом. Столько же, сколько назвал рецепт.
ids() {
    local n i
    n="$(grep -c '^ *image:' "$RECIPE" 2>/dev/null || printf 0)"
    [ "$n" -gt 0 ] || return 0
    for i in $(seq 1 "$n"); do printf '%s-cid%s\n' "$(proj)" "$i"; done
}

CTX=""
if [ "${1:-}" = "--context" ]; then CTX="${2:-}"; shift 2; fi

case "$1" in
    info)     exit "${STUB_LOCAL_DAEMON:-0}" ;;
    version)
        # `version` без контекста здесь не спрашивают; с контекстом — это вопрос ЧУЖОМУ
        # демону, и он отвечает по сценарию.
        #
        # ПОСЛЕ УСТАНОВКИ ОТВЕТ МЕНЯЕТСЯ: подставной ssh, поставив докер, оставляет отметку, и
        # с этого мига чужой демон отвечает. Иначе проверка «мир подтверждает установку тем же
        # путём» зеленела бы на подставе, которая и до установки говорила «докер есть».
        [ -n "$CTX" ] || exit 0
        if [ "${STUB_REMOTE_DOCKER:-1}" = 1 ] || [ -f "${STUB_LOG}.postavlen" ]; then
            printf '27.5.1\n'; exit 0
        fi
        # ПОЧЕМУ не отвечает — двумя разными ответами: «докер стоит, а прав нет» и «докер
        # стоит, а демон молчит» чинятся по-разному, и различает их инструмент по тексту.
        case "${STUB_REMOTE_DOCKER_WHY:-down}" in
            denied) printf 'permission denied while trying to connect to the Docker daemon socket at unix:///var/run/docker.sock\n' >&2 ;;
            *)      printf 'error during connect: Cannot connect to the Docker daemon at unix:///var/run/docker.sock\n' >&2 ;;
        esac
        exit 1 ;;
    context)
        case "$2" in
            inspect)
                # `context inspect <имя>` — есть ли контекст; с `--format` — его адрес.
                [ "${STUB_CTX_EXISTS:-0}" = 1 ] || exit 1
                case "$*" in *--format*) printf '%s\n' "${STUB_CTX_ENDPOINT:-ssh://world@10.8.0.5:22}" ;; esac
                exit 0 ;;
            create|update|rm) exit "${STUB_CTX_WRITE:-0}" ;;
            ls)
                [ "${STUB_CTX_EXISTS:-0}" = 1 ] || exit 0
                printf 'world-vps\tssh://world@10.8.0.5:22\n'; exit 0 ;;
        esac
        exit 0 ;;
    compose)
        case "$*" in
            *"config --images"*)
                # Имя образа ЧИТАЕТСЯ из рецепта. Нет его там — печатать нечего, и это ровно
                # тот случай, в котором инструмент обязан отказать, а не подставить своё.
                #
                # ПОДСТАНОВКУ РАСКРЫВАЕМ, как это делает компоуз: имя в рецепте вправе
                # крутиться переменной (`${WORLD_IMAGE:-…}` — ею поднимают названный выпуск
                # вместо подвижного `latest`). Отдавай заглушка сырую строку, проба стерегла бы
                # БУКВУ файла, а не то, что инструмент получит от настоящего компоуза, — и
                # краснела бы на верной правке рецепта (`WORLD2` 4.2).
                syraya="$(sed -n 's/^ *image: *//p' "$RECIPE" 2>/dev/null | head -n1)"
                case "$syraya" in
                    '${'*':-'*'}')
                        vnutri="${syraya#'${'}"
                        peremennaya="${vnutri%%:-*}"
                        umolchanie="${vnutri#*:-}"; umolchanie="${umolchanie%\}}"
                        printf '%s\n' "${!peremennaya:-$umolchanie}" ;;
                    *)  printf '%s\n' "$syraya" ;;
                esac
                exit 0 ;;
            *config*)
                # Рецепт, который компоуз не разбирает: слово-метка в файле. Настоящий
                # компоуз спорит с отступами, подстановками и полями — заглушке хватит
                # одного признака, потому что проверяем мы ВЕТВЛЕНИЕ, а не разбор YAML.
                if grep -q 'СЛОМАНО' "$RECIPE" 2>/dev/null; then
                    printf 'validating %s: services must be a mapping\n' "$RECIPE" >&2; exit 1
                fi
                printf 'name: %s\n' "$(proj)"; exit 0 ;;
            *" ps -aq"*|*" ps -q"*) ids; exit 0 ;;
            *version*)           exit "${STUB_LOCAL_COMPOSE:-0}" ;;
            *" up "*|*" up")
                case "${STUB_UP:-ok}" in
                    ok)   printf 'Container world-door  Started\n'; exit 0 ;;
                    busy) printf 'Error response from daemon: driver failed programming external connectivity: Bind for 0.0.0.0:8080 failed: port is already allocated\n' >&2; exit 1 ;;
                    # ВТОРОЕ СЛОВО ДОКЕРА О ТОМ ЖЕ. Он говорит про занятый порт по-разному —
                    # от версии и системы, — и шаблонов в `remote.sh` поэтому три. Стеречь
                    # надо каждый: мёртвый шаблон выглядит заботой, а не срабатывает
                    # (`WORLD2-158`: один такой там и лежал, целиком внутри соседнего).
                    busy2) printf 'Error response from daemon: driver failed programming external connectivity: bind: address already in use\n' >&2; exit 1 ;;
                    *)    printf 'Error response from daemon: something else entirely\n' >&2; exit 1 ;;
                esac ;;
            *" down"*) exit "${STUB_DOWN:-0}" ;;
            *logs*)    printf '(журнал двери)\n'; exit 0 ;;
        esac
        exit 0 ;;
    image)
        case "$2" in
            inspect)
                if [ -n "$CTX" ]; then exit $(( ${STUB_REMOTE_IMAGE:-0} == 1 ? 0 : 1 ))
                else exit $(( ${STUB_LOCAL_IMAGE:-1} == 1 ? 0 : 1 )); fi ;;
            rm) exit 0 ;;
        esac
        exit 0 ;;
    pull)
        # Реестр отвечает по сценарию. Отказ выглядит как отказ докера, а не как наш:
        # инструмент обязан разобрать ЧУЖОЙ ответ, а не свой собственный.
        #
        # В ПОТОК ПОРЯДКА кладём отметку: путь доставки обязан быть НАЗВАН до того, как он
        # занял минуты, и проверяется это порядком, а не наличием фразы где-нибудь в выводе.
        printf 'STUB-DOCKER: пошла тяга из реестра\n' >> "${STUB_ORDER:-/dev/null}"
        [ "${STUB_PULL:-0}" = 0 ] && exit 0
        printf 'Error response from daemon: denied\n' >&2; exit 1 ;;
    save)
        printf 'STUB-DOCKER: пошла копия образа\n' >> "${STUB_ORDER:-/dev/null}"
        printf 'подставной образ\n'; exit 0 ;;
    load) cat >/dev/null; printf 'Loaded image: ghcr.io/omnifield/world:latest\n'; exit "${STUB_LOAD:-0}" ;;
    network)
        case "$2" in
            inspect) exit $(( ${STUB_NET_EXISTS:-0} == 1 ? 0 : 1 )) ;;
            create)  exit "${STUB_NET_CREATE:-0}" ;;
            rm)      exit "${STUB_NET_RM:-0}" ;;
        esac
        exit 0 ;;
    inspect)
        # Один вызов, два вопроса: здоровье и имя контейнера. Имя выводим из проекта — то
        # есть из рецепта, а не из константы: спросили про чужую вещь — получили её имя.
        case "$*" in
            *".State.Health"*) printf '%s\n' "${STUB_HEALTH:-healthy}" ;;
            *".Name"*)         printf '/%s\n' "$LAST" ;;
        esac
        exit 0 ;;
    ps)
        case "$*" in
            # `ps -a --format '{{.Label "com.docker.compose.project"}}'` — какие ВЕЩИ вообще
            # стоят на ресурсе. Ответ идёт от демона, а не от нашего рецепта.
            *Label*) printf '%s\n' "${STUB_THINGS:-world}" ;;
            *)       printf 'Up 3 minutes (healthy)\n' ;;
        esac
        exit 0 ;;
    # ПУБЛИКАЦИЯ. Умолчание — ЗАВЕДОМО ЧУЖОЕ ЧИСЛО, и это не каприз (`WORLD2-133`).
    # Раньше подстава отвечала `8080` — тем же портом, что стоит в рецепте двери, — и
    # «прочитал факт» стало неотличимо от «подставил свою константу»: ревью-порча 16.08
    # вернула `host_port() { printf 8080; }`, и проба дала 60 из 60 зелёных.
    #
    # `48173` не встречается в дереве нигде: ни в рецептах, ни в коде, ни в других пробах.
    # Значит увидеть его в стуке можно ровно одним способом — прочитав у докера.
    port)     printf '8080/tcp -> 0.0.0.0:%s\n' "${STUB_HOST_PORT:-48173}"; exit 0 ;;
    volume)
        # Тома ищутся по метке проекта — значит и отвечать надо от имени проекта.
        if [ -n "$PROJ_FILTER" ]; then
            printf '%s-field\n%s-stand\n' "$PROJ_FILTER" "$PROJ_FILTER"
        else
            printf 'world-field\nworld-stand\n'
        fi
        exit 0 ;;
esac
exit 0
STUB

cat > "$STUB_DIR/ssh" <<'STUB'
#!/usr/bin/env bash
# ПОДСТАВНОЙ ssh: пишет журнал вызовов и отвечает по сценарию. Никуда не ходит.
#
# Он отвечает на ТРИ разных вопроса, и это не прихоть заглушки: ровно столько разговоров мир
# ведёт с чужой машиной — «есть ли докер», «чем его ставить» и сама установка. Появится
# четвёртый — он попадёт в ветку `*)`, и проба это увидит: команда, которой заглушка не
# знает, значит мир делает на машине юзера что-то сверх условия своего существования.
#
# В журнал вызов ложится ОДНОЙ строкой, даже если команда многострочная: журнал читают
# построчно, и разорванная надвое команда пряталась бы от сторожа половиной себя — а сторож
# здесь стережёт, что мир не делает на чужой машине ничего сверх докера.
{ printf 'ssh %s' "$*" | tr '\n' ' '; printf '\n'; } >> "${STUB_LOG:-/dev/null}"
[ "${STUB_SSH_OK:-1}" = 1 ] || { printf 'Permission denied (publickey).\n' >&2; exit 255; }

# Последний аргумент — то, что велено выполнить на той машине.
cmd="${!#}"
case "$cmd" in
    true) exit 0 ;;

    # Вопрос «есть ли докер вообще».
    *"WORLD-PROBE docker="*)
        # Два РАЗНЫХ несчастья, и заглушка их различает: машина оборвала связь — и машина
        # ответила, да не о том (так отвечает чужая оболочка юзера: fish, csh).
        [ "${STUB_PROBE_DIES:-0}" = 1 ] && { printf 'Connection closed by remote host\n' >&2; exit 255; }
        [ "${STUB_PROBE_ALIEN:-0}" = 1 ] && { printf 'fish: Unknown command: command\n'; exit 0; }
        printf 'WORLD-PROBE docker=%s\n' "${STUB_REMOTE_DOCKER_CLI:-no}"; exit 0 ;;

    # Вопрос «чем ставить»: права, чем скачать и достаёт ли машина до установщика.
    *"WORLD-PROBE root="*)
        # Два РАЗНЫХ несчастья, и заглушка их различает: машина оборвала связь — и машина
        # ответила, да не о том (так отвечает чужая оболочка юзера: fish, csh).
        [ "${STUB_PROBE_DIES:-0}" = 1 ] && { printf 'Connection closed by remote host\n' >&2; exit 255; }
        [ "${STUB_PROBE_ALIEN:-0}" = 1 ] && { printf 'fish: Unknown command: command\n'; exit 0; }
        printf 'WORLD-PROBE root=%s sudo=%s fetch=%s net=%s\n' \
            "${STUB_REMOTE_ROOT:-yes}" "${STUB_REMOTE_SUDO:-no}" \
            "${STUB_REMOTE_FETCH:-curl}" "${STUB_REMOTE_NET:-yes}"; exit 0 ;;

    # САМА УСТАНОВКА. Отметка в потоке порядка — по ней проба видит, что слово сказано ДО
    # действия, а не после. Удача оставляет след (`.postavlen`), и с этого мига подставной
    # докер на той стороне начинает отвечать.
    *"| sh"*|*"| sudo"*)
        printf 'STUB-SSH: пошла установка докера на чужую машину\n' >> "${STUB_ORDER:-/dev/null}"
        # ЧТО ИМЕННО УШЛО НА МАШИНУ — дословно, отдельным файлом. По нему проба сверяет
        # ушедшее с показанным человеку: сверка по подстроке ловит только вырожденный случай
        # («не напечатали вовсе»), а разойтись они могут и на правах.
        printf '%s\n' "$cmd" >> "${STUB_LOG}.ustanovka"
        case "${STUB_DOCKER_INSTALL:-ok}" in
            ok)
                : > "${STUB_LOG}.postavlen"
                printf 'подставной установщик: докер поставлен\n'; exit 0 ;;
            # Установщик отработал, а докер так и не отвечает: отметки нет НАРОЧНО.
            silent)
                printf 'подставной установщик: докер поставлен\n'; exit 0 ;;
            *)
                printf 'E: Unable to locate package docker-ce\n' >&2; exit 100 ;;
        esac ;;

    *)  printf 'STUB-SSH: НЕОПОЗНАННАЯ команда на чужой машине\n' >> "${STUB_ORDER:-/dev/null}"
        exit 0 ;;
esac
STUB

# СТУК СНАРУЖИ — подставной, и это единственный способ увидеть, КУДА стучались.
#
# `remote.sh` в конце спрашивает у докера, куда опубликована вещь, и стучится по этому
# адресу с ЭТОЙ машины. Пока `curl` был настоящим, стук уходил в никуда (адрес выдуманный),
# и порт в нём не проверял никто — а значит зашитая константа вместо прочитанного значения
# прошла бы незамеченной. Ровно это и случилось 16.08 (`WORLD2-133`).
#
# Заглушка отвечает кодом HTTP, потому что стук спрашивает ВИДИМОСТЬ порта, а не здоровье:
# годится любой ответ. Что важно — она пишет в журнал полный адрес, и по нему видно порт.
cat > "$STUB_DIR/curl" <<'STUB'
#!/usr/bin/env bash
printf 'curl %s\n' "$*" >> "${STUB_LOG:-/dev/null}"
printf '%s' "${STUB_KNOCK_CODE:-200}"
exit 0
STUB

# `wget` — ловушка того же рода: `remote.sh` берёт его, когда нет `curl`. Настоящий здесь
# звать некому, а позовут — это увидит журнал.
cat > "$STUB_DIR/wget" <<'STUB'
#!/usr/bin/env bash
printf 'wget %s\n' "$*" >> "${STUB_LOG:-/dev/null}"
printf 'HTTP/1.1 200 OK\n' >&2
exit 0
STUB

# scp и rsync — заглушки-ловушки. Настоящих здесь звать некому, и если позовут, это увидит
# журнал: класть что-либо на чужую машину зона не вправе (`TECH` 2).
for trap_tool in scp rsync; do
    cat > "$STUB_DIR/$trap_tool" <<STUB
#!/usr/bin/env bash
printf '$trap_tool %s\n' "\$*" >> "\${STUB_LOG:-/dev/null}"
exit 0
STUB
done

chmod +x "$STUB_DIR"/*
PATH="$STUB_DIR:$PATH"; export PATH

# ------------------------------------------------------------------ ВТОРОЙ РЕЦЕПТ
# Ради него вся ступень и делалась: чужая вещь, ни одним словом не совпадающая с дверью.
# Своё имя проекта, свой образ, своё имя контейнера, свой порт. Если она поднимается тем же
# путём и без единой правки кода — вещь описана рецептом, а не зашита (`WORLD2` 3.7).
#
# Файл временный: класть второй рецепт в зону значило бы завести вещь, которой никто не
# заказывал. Проверяем МЕХАНИКУ, а не пополняем каталог.
OTHER="$STUB_DIR/scales.yaml"
cat > "$OTHER" <<'RECIPE'
name: scales
services:
  scales:
    image: ghcr.io/example/scales:v1
    container_name: scales-box
    ports:
      - "9099:9099"
RECIPE

OTHER_IMAGE=ghcr.io/example/scales:v1

# Рецепт, который компоуз не разберёт, и рецепт без имени образа — две РАЗНЫЕ поломки.
BROKEN="$STUB_DIR/сломанный.yaml"
printf 'name: СЛОМАНО\nservices: [это не отображение]\n' > "$BROKEN"
NOIMAGE="$STUB_DIR/безобраза.yaml"
printf 'name: безобраза\nservices:\n  что-то:\n    ports:\n      - "1:1"\n' > "$NOIMAGE"

# ------------------------------------------------------------------ прогон инструмента
# run <сценарий…> -- <аргументы remote.sh> — прогнать `remote.sh` с чистым журналом.
# Вывод целиком кладём в OUT, журнал вызовов — в JOURNAL: проба смотрит на обе вещи, потому
# что отказ обязан быть и читаемым, и разбираемым, а ветвление видно только по журналу.
OUT=""; JOURNAL=""; RC=0
run() {
    local -a env=()
    while [ "$1" != "--" ]; do env+=("$1"); shift; done
    shift
    : > "$LOG"; rm -f "$LOG.postavlen" "$LOG.ustanovka"
    # Вывод инструмента и отметки подставных инструментов ложатся в ОДИН файл и оба ДОПИСЫВАЮТ
    # (`>>`): так порядок в файле — это настоящий порядок событий. Возьми мы подстановку
    # `$(…)`, слово и действие оказались бы в разных потоках, и «сказано ДО» проверить было бы
    # нечем — осталось бы «сказано где-то», а это другое свойство.
    : > "$ORDER"
    env STUB_LOG="$LOG" STUB_ORDER="$ORDER" "${env[@]}" "$REMOTE" "$@" >> "$ORDER" 2>&1; RC=$?
    OUT="$(cat "$ORDER")"
    JOURNAL="$(cat "$LOG")"
}

# Сценарий по умолчанию: ресурс не заведён, ssh пускает, докер на той стороне есть,
# образа там нет, локально есть, сети нет, подъём удаётся, дверь здорова.
DEFAULT=(STUB_CTX_EXISTS=0 STUB_SSH_OK=1 STUB_REMOTE_DOCKER=1
         STUB_REMOTE_IMAGE=0 STUB_LOCAL_IMAGE=1 STUB_NET_EXISTS=0
         STUB_UP=ok STUB_HEALTH=healthy)

# ТО ЖЕ САМОЕ, НО МАШИНА ЧИСТАЯ: докера на ней нет вовсе. Это не поломка и не отказ, а
# законное состояние, которое мир умеет привести в годное (`WORLD2` 3.5 п. 7): юзер привёл
# машину в мир — значит согласился на условие, которым мир на ней бывает.
CHISTAYA=(STUB_CTX_EXISTS=0 STUB_SSH_OK=1 STUB_REMOTE_DOCKER=0 STUB_REMOTE_DOCKER_CLI=no
          STUB_REMOTE_ROOT=yes STUB_REMOTE_SUDO=no STUB_REMOTE_FETCH=curl STUB_REMOTE_NET=yes
          STUB_REMOTE_IMAGE=0 STUB_LOCAL_IMAGE=1 STUB_NET_EXISTS=0
          STUB_UP=ok STUB_HEALTH=healthy)

has()    { case "$JOURNAL" in *"$1"*) return 0 ;; esac; return 1; }
in_out() { case "$OUT" in *"$1"*) return 0 ;; esac; return 1; }

# stroka <кусок> — номер первой строки вывода, где встретился кусок. Пусто — не встретился.
stroka() { printf '%s\n' "$OUT" | grep -n -F -- "$1" | head -n1 | cut -d: -f1; }

# skazano_do <имя> <слово> <действие> — СЛОВО обязано стоять в выводе РАНЬШЕ ДЕЙСТВИЯ.
# Главный сторож правила «говорим ДО, а не после» (`WORLD2` 3.5). Проверять наличие фразы
# бесполезно: «докер поставлен» после установки тоже фраза, и она тоже про докер.
skazano_do() {
    local name="$1" slovo="$2" delo="$3" a b
    a="$(stroka "$slovo")"; b="$(stroka "$delo")"
    if [ -z "$b" ]; then
        bad "$name → действия «$delo» не случилось вовсе" "$OUT"
    elif [ -z "$a" ]; then
        bad "$name → слова «$slovo» нет в выводе" \
            "юзер узнаёт об изменении своей машины из журнала после — это ровно то, чего канон не допускает"
    elif [ "$a" -lt "$b" ]; then
        ok "$name"
    else
        bad "$name → сказано ПОСЛЕ, а не до (слово в строке $a, действие в строке $b)" \
            "«говорим ДО» — правило про ПОРЯДОК: сказанное вдогонку это уже пересказ, а не предупреждение"
    fi
}

# v_stroku — многострочное в одну строку для отказа. Через sed, а не `tr`: `tr` работает
# ПОБАЙТОВО и разрезал бы многобайтный символ пополам, отдав ломаный UTF-8 — и вывод сторожа
# стал бы нечитаемым ровно тогда, когда он что-то нашёл.
v_stroku() { printf '%s' "$1" | sed ':a;N;$!ba;s/\n/ · /g'; }

# pokazano_to_zhe <имя> — ПОКАЗАННОЕ ЧЕЛОВЕКУ И УШЕДШЕЕ НА МАШИНУ — одна и та же команда,
# слово в слово. Сверяются ДВЕ ЗАПИСИ ЦЕЛИКОМ, а не наличие подстроки: по подстроке ловится
# только вырожденный случай «не напечатали вовсе», а разойтись они могут и на одном слове —
# показать команду без прав, а выполнить с `sudo`. Человек согласился бы на одно, а получил
# другое, с правами root (`WORLD2` 3.5: «что именно там запускается, юзер обязан узнать
# словами»).
pokazano_to_zhe() {
    local name="$1" pokazano ushlo
    pokazano="$(printf '%s\n' "$OUT" | awk '
        /будет выполнено ровно это:/ {beri=1; next}
        beri && /^ *$/ {exit}
        beri {sub(/^ +/, ""); sub(/ +$/, ""); print}')"
    ushlo="$(sed 's/^ *//; s/ *$//' "$LOG.ustanovka" 2>/dev/null | grep -v '^$' || true)"
    if [ -z "$ushlo" ]; then
        bad "$name → на машину ничего не уходило" "$OUT"
    elif [ -z "$pokazano" ]; then
        bad "$name → человеку не показали НИЧЕГО, а на машине выполнено:" "$ushlo" \
            "юзер соглашается на выполнение на своей машине: не увидев команду, он соглашается вслепую"
    elif [ "$pokazano" = "$ushlo" ]; then
        ok "$name"
    else
        bad "$name → показанное и ушедшее РАЗОШЛИСЬ" \
            "показано человеку: $(v_stroku "$pokazano")" \
            "ушло на машину:   $(v_stroku "$ushlo")" \
            "человек согласился на одно, а на его машине выполнилось другое — и это может быть другое ПО ПРАВАМ"
    fi
}

# na_chuzhoy_mashine_tolko_docker <имя> — КЛАСС, а не случай: по ssh мир вправе выполнять
# только то, без чего мира на машине нет вовсе, — спросить про докер и поставить его. Всё
# прочее (`apt install чего-нибудь ещё`, `git clone`, наши скрипты, `mkdir`) — это уже
# распоряжение чужой машиной, и его быть не должно ни в одном сценарии.
na_chuzhoy_mashine_tolko_docker() {
    local name="$1" calls stray
    calls="$(printf '%s\n' "$JOURNAL" | grep '^ssh ' || true)"
    stray="$(printf '%s\n' "$calls" \
             | grep -v ' true$' \
             | grep -v 'WORLD-PROBE' \
             | grep -v "$INSTALL_URL" || true)"
    if [ -n "$stray" ]; then
        bad "$name → по ssh пошло что-то сверх докера" "$stray" \
            "мир вправе выполнить на машине юзера ровно одно: узнать про докер и поставить его (WORLD2 3.5 п. 7)"
        return 1
    fi
    if printf '%s\n' "$calls" | grep -qE 'git clone|mkdir|apt-get install|yum install|dnf install|apk add|npm |pip '; then
        bad "$name → на чужой машине ставится что-то помимо докера" "$calls" \
            "список не открывается: «ну и вот это ещё нужно» — разговор про долг, а не про установку"
        return 1
    fi
    if in_out "НЕОПОЗНАННАЯ команда"; then
        bad "$name → на чужой машине выполнена команда, которой заглушка не знает" "$OUT" \
            "четвёртого разговора с чужой машиной у мира нет: есть ли докер · чем ставить · установка"
        return 1
    fi
    ok "$name"
    return 0
}

# want_refusal <код> <имя> <сценарий…> -- <аргументы> — инструмент ОБЯЗАН отказать этим кодом.
# Стережём КОД, а не формулировку: проба, привязанная к тексту, зеленеет на верной правке и
# учит не трогать буквы (`WORLD2` 4.2).
want_refusal() {
    local code="$1" name="$2"; shift 2
    run "$@"
    if in_out "REMOTE-REFUSAL: $code"; then
        ok "$name → $code"
    elif in_out "REMOTE-REFUSAL: "; then
        local got; got="$(printf '%s\n' "$OUT" | sed -n 's/^REMOTE-REFUSAL: //p' | head -n1)"
        bad "$name → ждали $code, получили $got" \
            "ступени схлопнулись: разные поломки чинят разные люди, и общий код отправляет чинить не туда"
    else
        bad "$name → отказа нет вовсе (код возврата $RC)" \
            "молчание хуже отказа: юзеру нечего чинить (WORLD2 2.3)"
    fi
}

# ------------------------------------------------------------------ зелёный прогон
green() {
    part "ЗЕЛЁНЫЙ ПРОГОН — что инструмент обязан делать (докер и ssh подставные)"

    run "${DEFAULT[@]}" -- add vps --addr world@10.8.0.5
    if [ "$RC" -eq 0 ]; then ok "подъём на чистом ресурсе прошёл целиком"
    else bad "подъём на чистом ресурсе не прошёл (код $RC)" "$OUT"; fi

    # Прежний путь не сломан: команда без ключей ставит дверь — рецепт двери остался
    # умолчанием КОМАНДЫ. Съедь это умолчание, и человек, поднимавший дверь одной строкой,
    # поднял бы неизвестно что.
    if has "compose -f $HERE/compose.yaml"; then ok "без --recipe берётся рецепт двери — прежний путь цел"
    else bad "умолчание команды съехало с рецепта двери" "$JOURNAL"; fi

    # Имя контекста ПРОИЗВОДНО от имени ресурса. Стереги мы «какое-то имя», инструмент мог
    # бы выдумывать его заново каждый раз, и `drop` перестал бы находить свой же ресурс.
    if has "context create world-vps"; then ok "контекст назван производно: vps → world-vps"
    else bad "контекста world-vps не создано" "имя контекста обязано выводиться из имени ресурса, а не выдумываться"; fi

    if has "ssh -o BatchMode=yes"; then ok "доступ проверен отдельно, ssh с BatchMode"
    else bad "доступ не проверялся ssh с BatchMode" "без BatchMode ssh спросит пароль и проверка повиснет вместо красноты"; fi

    # Порядок ступеней: доступ проверяется ДО того, как заводится контекст. Иначе на машине
    # человека остаётся контекст к ресурсу, который его не пустил.
    if [ "$(printf '%s\n' "$JOURNAL" | grep -n '^ssh ' | head -n1 | cut -d: -f1)" -lt \
         "$(printf '%s\n' "$JOURNAL" | grep -n 'context create' | head -n1 | cut -d: -f1)" ]; then
        ok "доступ проверяется ДО создания контекста"
    else
        bad "контекст заводится раньше проверки доступа" \
            "не пустивший ресурс оставил бы за собой контекст на машине человека"
    fi

    if has "save $IMAGE" && has "--context world-vps load"; then
        ok "образ едет ПОСТАВКОЙ КОПИИ: save здесь → load туда, через ssh"
    else
        bad "образ не поехал копией" \
            "он лежит у контроллера — значит путь выбран наличием и это копия (WORLD2-150 B2)"
    fi

    if has "--context world-vps network create $NET"; then ok "общая сеть создаётся НА ТОМ ресурсе"
    else bad "сеть $NET на ресурсе не создаётся" "войти в несуществующую сеть докер не даёт — первый подъём не встал бы"; fi

    if has "--context world-vps compose" && has " up -d --no-build"; then
        ok "дверь поднимается компоузом через контекст, без сборки на той стороне"
    else
        bad "подъём идёт не через контекст либо со сборкой" "сборка на чужом ресурсе потребовала бы там исходников — их туда не кладут"
    fi

    if has "--context world-vps inspect"; then ok "здоровье двери спрашивается у ЧУЖОГО демона"
    else bad "здоровье двери не проверялось" "«запущен» и «отвечает» — разные вещи"; fi

    # ────────────────────────────────────────── КУДА СТУЧАЛИСЬ (`WORLD2-133`)
    # Порт стука берётся у ДОКЕРА — из факта публикации, а не из своего значения. Стеречь это
    # можно ровно одним способом: подстава отвечает портом, которого в дереве нет ни у кого
    # (`48173`), и в стуке обязан оказаться он.
    #
    # Прежде подстава отвечала `8080`, то есть портом двери, — и порча 16.08 («`host_port`
    # возвращает зашитый 8080») прошла зелёной: 60 из 60. Проверялось «что-то стучится», а
    # утверждалось «стучится туда, куда опубликовано». Разница видна только на чужом числе.
    if has "curl "; then
        if has ":48173/"; then
            ok "стук ушёл в порт, названный ДОКЕРОМ (48173) — прочитан, а не выдуман"
        else
            bad "стук ушёл не в опубликованный порт" \
                "$(printf '%s\n' "$JOURNAL" | grep '^curl ' || true)" \
                "порт обязан читаться из ответа докера: подстава публикует 48173, и он же обязан быть в стуке" \
                "число 48173 не встречается в дереве нигде — совпасть с зашитой константой оно не может"
        fi
        if has ":8080/"; then
            bad "в стуке всплыл 8080 — порт двери из рецепта, а не из публикации" \
                "$(printf '%s\n' "$JOURNAL" | grep '^curl ' || true)" \
                "ровно так выглядела порча 16.08: значение подставлено, а не прочитано"
        else
            ok "порта двери (8080) в стуке нет — значение не подставлено своё"
        fi
    else
        bad "стука снаружи не было вовсе" \
            "после подъёма `remote.sh` обязан спросить видимость порта — иначе проверять нечего"
    fi

    # ГЛАВНАЯ проверка зоны: на ту машину не положено ничего.
    part "НА ТУ МАШИНУ НЕ ПОЛОЖЕНО НИЧЕГО (TECH 2)"
    if has "scp " || has "rsync "; then
        bad "файлы копируются на чужую машину (scp/rsync)" "на ресурсе должен появиться только контейнер и образ"
    else
        ok "ни scp, ни rsync — файлов на ту сторону не уезжает"
    fi

    # НА МАШИНЕ, ГДЕ ДОКЕР ЕСТЬ, МИР НЕ ДЕЛАЕТ РОВНО НИЧЕГО. Это и есть граница «есть докер —
    # не трогаем» в проверяемом виде: единственная законная команда — проверка доступа
    # (`true`). Появись здесь хоть вопрос «а какой у тебя докер» — мир уже полез в чужое.
    local ssh_calls stray
    ssh_calls="$(printf '%s\n' "$JOURNAL" | grep '^ssh ' || true)"
    stray="$(printf '%s\n' "$ssh_calls" | grep -v ' true$' || true)"
    if [ -n "$stray" ]; then
        bad "на машине С ДОКЕРОМ по ssh идут команды сверх проверки доступа" "$stray" \
            "чужой докер не наше дело, пока отвечает: ни спрашивать про него, ни чинить, ни обновлять мир не вправе"
    else
        ok "докер на машине есть — мир не выполнил на ней ничего, кроме проверки доступа"
    fi
    na_chuzhoy_mashine_tolko_docker "и ни apt, ни git clone, ни установки чего-либо"

    # ------------------------------------------------------------------------------------
    part "МАШИНА БЕЗ ДОКЕРА ГОТОВИТСЯ САМА (WORLD2 3.5 п. 7, канон переписан 2026-08-18)"
    # Ступень B1 целиком. Машина без докера — законное состояние, а не отказ: докер не вещь
    # В мире, а то, ЧЕМ мир на машине бывает, и без него мира на ней нет вовсе.
    run "${CHISTAYA[@]}" -- add vps --addr world@10.8.0.5
    if [ "$RC" -eq 0 ]; then ok "чистая машина доведена до стоящей вещи ОДНОЙ командой"
    else bad "на машине без докера подъём не прошёл (код $RC)" "$OUT" \
        "«поставьте докер сами и повторите» — это отказ там, где мир умеет привести машину в годное"; fi

    if has "$INSTALL_URL"; then ok "докер ставится ОФИЦИАЛЬНЫМ установщиком ($INSTALL_URL), а не нашим разбором дистрибутивов"
    else bad "докер на той машине не ставился вовсе" "$JOURNAL" \
        "сегодня это был отказ no-remote-docker; канон 2026-08-18 велит ставить"; fi

    # Метка машине стережётся ПОРЯДКОМ, как и слова человеку. Она заведена затем, чтобы
    # ждущий знал, чего ждёт, — метка после шага это отчёт, а не предупреждение, и контроллеру
    # довозить до пульта будет уже нечего.
    skazano_do "путь назван и машине ДО установки: REMOTE-PATH: docker-install" \
        "REMOTE-PATH: docker-install" "STUB-SSH: пошла установка"

    # ГОВОРИМ ДО, А НЕ ПОСЛЕ — и это про порядок, а не про наличие фразы.
    skazano_do "юзер узнаёт про докер ДО того, как докер появится" \
        "появится ДОКЕР" "STUB-SSH: пошла установка"
    skazano_do "и слышит САМУ КОМАНДУ до её запуска, а не из журнала после" \
        '"$docker_installer" | sh' "STUB-SSH: пошла установка"
    pokazano_to_zhe "показанное человеку и ушедшее на машину — одна команда, слово в слово"
    if in_out "docker_installer=\$($INSTALL_URL" || in_out "docker_installer=\$(curl -fsSL https://$INSTALL_URL"; then
        ok "скачивание и запуск разведены — отказ скачивания остаётся отказом скачивания"
    else
        bad "установщик качается и запускается одним конвейером" "$OUT" \
            "код возврата у 'curl | sh' берётся у sh: не скачавшись, установщик отдал бы sh пустоту, и мир записал бы «поставили»"
    fi

    # Подтверждение — ТЕМ ЖЕ путём, которым потом пойдёт подъём. «Установщик сказал ок» и
    # «докер отвечает» — разные вещи (`WORLD2` 4.2).
    if [ "$(printf '%s\n' "$JOURNAL" | grep -c -- '--context world-vps version')" -ge 2 ]; then
        ok "после установки докер подтверждён тем же путём, а не словом установщика"
    else
        bad "после установки никто не спросил, отвечает ли докер" "$JOURNAL" \
            "«поставили» и «отвечает» — разные вещи, и записывать надо вторую"
    fi

    na_chuzhoy_mashine_tolko_docker "поставлен ТОЛЬКО докер — ничего сверх него"

    if in_out "а ещё там появился ДОКЕР"; then ok "отчёт называет докер среди того, что на машине появилось"
    else bad "отчёт молчит о том, что на машине появился докер" "$OUT" \
        "перечень, из которого выпало главное изменение чужой машины, врёт (WORLD2 4.2)"; fi

    # Не root — тогда sudo, и право говорить с демоном даётся тут же: докер, с которым нашему
    # юзеру не дают разговаривать, машину в мир не вводит.
    run "${CHISTAYA[@]}" STUB_REMOTE_ROOT=no STUB_REMOTE_SUDO=yes -- add vps --addr world@10.8.0.5
    if [ "$RC" -eq 0 ] && has "sudo -n sh"; then ok "юзер не root — докер ставится через sudo"
    else bad "без root установка не прошла или пошла мимо sudo (код $RC)" "$JOURNAL"; fi
    if has "usermod -aG docker"; then ok "и право говорить с демоном даётся тем же шагом"
    else bad "докер поставлен, а говорить с ним нашему юзеру нечем" "$JOURNAL" \
        "докер, к демону которого нас не пускают, машину в мир не вводит — это половина установки"; fi
    # ТОТ ЖЕ сторож на ветке с правами: разойтись показанное и ушедшее могут именно здесь —
    # показать команду без `sudo`, а выполнить с ним, и человек согласится не на то.
    pokazano_to_zhe "и на ветке с sudo показанное человеку — ровно то, что ушло на машину"

    # Машина, где докер УЖЕ есть, не трогается: ни установки, ни вопросов о ней.
    run "${DEFAULT[@]}" -- add vps --addr world@10.8.0.5
    if has "$INSTALL_URL"; then
        bad "докер ставится на машину, где он уже есть" "$JOURNAL" \
            "мир не обновляет, не заменяет и не чинит стоящий докер: чужая версия не его дело, пока отвечает"
    else
        ok "докер на машине есть — мир не ставит его поверх и не трогает"
    fi

    part "ВТОРАЯ ВЕЩЬ ПОДНИМАЕТСЯ ТЕМ ЖЕ ПУТЁМ — без единой правки кода (WORLD2 3.7)"
    # Это главная проверка ступени. Она отвечает на один вопрос: описана вещь рецептом или
    # всё-таки зашита. Второй рецепт не совпадает с дверью ничем — ни именем проекта, ни
    # образом, ни именем контейнера, ни портом.
    # Публикацию подстава называет ЕЁ портом (`9099` из этого рецепта), а не портом двери:
    # у второй вещи всё своё, и порт — тоже. Стук обязан уйти туда (`WORLD2-133`).
    run "${DEFAULT[@]}" STUB_HOST_PORT=9099 -- add vps --addr world@10.8.0.5 --recipe "$OTHER"
    if [ "$RC" -eq 0 ]; then ok "чужая вещь встала тем же add — правок в коде не потребовалось"
    else bad "чужая вещь по своему рецепту не встала (код $RC)" "$OUT" \
        "потребовала правки — значит вещь была зашита, а не описана, и это дефект (WORLD2 3.7)"; fi

    if has "compose -f $OTHER"; then ok "подъём идёт по НАЗВАННОМУ рецепту"
    else bad "подъём пошёл не по названному рецепту" "$JOURNAL"; fi

    if has "save $OTHER_IMAGE"; then ok "везётся образ из рецепта: $OTHER_IMAGE"
    else bad "образ второй вещи не поехал" "имя образа обязано читаться из рецепта, а не из константы"; fi

    if has "$IMAGE"; then
        bad "в подъёме чужой вещи всплыл образ двери ($IMAGE)" \
            "механика подставляет своё умолчание вместо того, что назвал рецепт"
    else
        ok "образа двери в подъёме чужой вещи нет ни разу"
    fi

    # Здоровье спрашивается у контейнеров ЭТОГО проекта, а не у имени, известного заранее.
    if has "scales-cid1"; then ok "здоровье спрашивается у контейнера этой вещи (его назвал компоуз)"
    else bad "здоровье спрашивалось не у контейнеров рецепта" "$JOURNAL" \
        "имя контейнера — свойство рецепта; знать его механике неоткуда"; fi

    if in_out "scales"; then ok "имя вещи в отчёте взято из рецепта"
    else bad "в отчёте нет имени вещи из рецепта" "$OUT"; fi

    # ПОРТ У ЧУЖОЙ ВЕЩИ СВОЙ, и стук обязан уйти в него. Это вторая половина `WORLD2-133`:
    # первая ловит зашитую константу чужим числом, вторая — тем, что у ДВУХ вещей числа
    # разные. Зашей кто-нибудь порт снова — здесь он совпал бы с дверью, а не с рецептом.
    if has ":9099/"; then
        ok "в чужую вещь стучались её портом (9099 из рецепта)"
    else
        bad "стук в чужую вещь ушёл не в её порт" \
            "$(printf '%s\n' "$JOURNAL" | grep '^curl ' || true)" \
            "порт вещи объявлен её рецептом, и читается он у докера — знать его механике неоткуда"
    fi
    if has ":8080/"; then
        bad "в стуке по чужой вещи всплыл порт двери (8080)" \
            "$(printf '%s\n' "$JOURNAL" | grep '^curl ' || true)"
    else
        ok "порта двери в стуке по чужой вещи нет"
    fi
    if in_out "world-door" || in_out "дверь стоит"; then
        bad "отчёт про чужую вещь говорит про дверь" "$OUT"
    else
        ok "в отчёте про чужую вещь двери не поминают вовсе"
    fi

    part "СНЯТЬ И СПРОСИТЬ — ТОЖЕ ПО РЕЦЕПТУ, А НЕ ПО ЗАШИТОМУ ИМЕНИ"
    run STUB_CTX_EXISTS=1 STUB_CTX_ENDPOINT=ssh://world@10.8.0.5:22 STUB_REMOTE_DOCKER=1 \
        STUB_NET_EXISTS=1 -- drop vps --recipe "$OTHER" --with-image
    if [ "$RC" -eq 0 ] && has "compose -f $OTHER" && has " down"; then
        ok "снимается вещь названного рецепта"
    else
        bad "снятие пошло не по рецепту (код $RC)" "$JOURNAL"
    fi
    if has "image rm $OTHER_IMAGE"; then ok "--with-image снимает образ ИЗ РЕЦЕПТА"
    else bad "снят не тот образ" "$JOURNAL" "имя образа при снятии обязано быть тем же, что при подъёме"; fi

    run STUB_CTX_EXISTS=1 STUB_CTX_ENDPOINT=ssh://world@10.8.0.5:22 STUB_REMOTE_DOCKER=1 \
        STUB_REMOTE_IMAGE=1 STUB_NET_EXISTS=1 STUB_THINGS=scales -- status vps --recipe "$OTHER"
    if [ "$RC" -eq 0 ] && in_out "scales"; then ok "status говорит про вещь названного рецепта"
    else bad "status не назвал вещь рецепта (код $RC)" "$OUT"; fi
    if in_out "вещи там"; then ok "status называет и то, ЧТО ВООБЩЕ стоит на ресурсе — отдельным вопросом"
    else bad "status не говорит, какие вещи стоят на ресурсе" "$OUT" \
        "ресурс — машина, до которой дотянулись; что на ней стоит, выводить из нашего рецепта нельзя"; fi

    part "В МЕХАНИКЕ НЕ ОСТАЛОСЬ ЗАШИТОЙ ВЕЩИ"
    # Сторож на будущее: вернуть в путь подъёма имя контейнера двери или её образ — и проба
    # покраснеет здесь. Зелёная проба, которой больше нечего проверять, хуже отсутствующей
    # (`WORLD2` 4.2), поэтому старую сверку констант заменили на обратную: их быть НЕ ДОЛЖНО.
    hard="$(grep -n 'world-door\|ghcr\.io/omnifield/world' "$REMOTE" || true)"
    if [ -z "$hard" ]; then
        ok "в remote.sh нет ни имени контейнера двери, ни её образа"
    else
        bad "в remote.sh вернулась зашитая вещь" "$hard" \
            "имя контейнера и образ принадлежат рецепту: вторая вещь потребовала бы правки кода"
    fi

    part "ОБРАЗ НЕ ВЕЗЁТСЯ ДВАЖДЫ"
    run STUB_CTX_EXISTS=1 STUB_CTX_ENDPOINT=ssh://world@10.8.0.5:22 STUB_SSH_OK=1 \
        STUB_REMOTE_DOCKER=1 STUB_REMOTE_IMAGE=1 STUB_LOCAL_IMAGE=1 STUB_NET_EXISTS=1 \
        STUB_UP=ok STUB_HEALTH=healthy -- add vps
    if has "save $IMAGE"; then
        bad "образ везётся, хотя на ресурсе он уже есть" "поставка копии — самый долгий шаг; повторять его без нужды значит платить минутами за ничто"
    else
        ok "образ на ресурсе есть — не везём"
    fi
    if [ "$RC" -eq 0 ]; then ok "заведённому ресурсу адрес повторно не нужен — он в контексте"
    else bad "повторный add без адреса не прошёл (код $RC)" "$OUT"; fi

    run STUB_CTX_EXISTS=1 STUB_CTX_ENDPOINT=ssh://world@10.8.0.5:22 STUB_SSH_OK=1 \
        STUB_REMOTE_DOCKER=1 STUB_REMOTE_IMAGE=1 STUB_LOCAL_IMAGE=1 STUB_NET_EXISTS=1 \
        STUB_UP=ok STUB_HEALTH=healthy -- add vps --resend-image
    if has "save $IMAGE"; then ok "--resend-image везёт образ заново"
    else bad "--resend-image образ не повёз" "ключ есть, а свойства нет — это хуже отсутствия ключа"; fi

    # ДВА ПУТИ ДОСТАВКИ ОДНОЙ ВЕЩИ (`WORLD2` 1.5), И ВЫБИРАЕТ МЕЖДУ НИМИ НАЛИЧИЕ (`WORLD2-150`
    # B2, слова user). Реестр стал возможен только с полным именем образа (`WORLD2-121`): пока
    # имя было коротким, `docker pull` на той стороне пошёл бы в Docker Hub — не туда, куда мы
    # публикуем.
    part "ПУТЬ ДОСТАВКИ ОБРАЗА ВЫБИРАЕТ НАЛИЧИЕ, А НЕ ЧЕЛОВЕК (WORLD2-150 B2)"

    # Путь первый: образ у контроллера ЕСТЬ — везём копией.
    run "${DEFAULT[@]}" -- add vps --addr world@10.8.0.5
    if has "save $IMAGE" && ! has "pull $IMAGE"; then
        ok "образ есть у контроллера — поехал КОПИЕЙ, и реестр не тронут"
    else
        bad "образ есть здесь, а копией не поехал" "$JOURNAL" \
            "копия возит ровно то, что у тебя на руках; подмена её опубликованным видна только по digest'у"
    fi
    skazano_do "путь назван машине ДО копии: REMOTE-PATH: image-copy" \
        "REMOTE-PATH: image-copy" "STUB-DOCKER: пошла копия"
    skazano_do "и человеку — тоже до, а не вдогонку" \
        "везу его КОПИЕЙ" "STUB-DOCKER: пошла копия"

    # Путь второй: образа у контроллера НЕТ — тянет та машина. Это не отказ и не ключ: это
    # ровно то же «выбирает наличие», просто наличие другое.
    run "${DEFAULT[@]}" STUB_LOCAL_IMAGE=0 -- add vps --addr world@10.8.0.5
    if [ "$RC" -eq 0 ]; then ok "образа нет ни здесь, ни там — подъём всё равно прошёл целиком"
    else bad "без образа на этой машине подъём отказал (код $RC)" "$OUT" \
        "прежде здесь был отказ no-image; та машина умеет взять образ сама, и человеку об этом знать незачем"; fi
    if has "--context world-vps pull $IMAGE" && ! has "save $IMAGE"; then
        ok "образа у контроллера нет — образ тянет САМ РЕСУРС, копия не везётся"
    else
        bad "образа здесь нет, а путь выбран не тот" "$JOURNAL" \
            "везти копией то, чего у тебя нет, нечем: выбирает наличие, а не умолчание"
    fi
    skazano_do "путь назван машине ДО тяги: REMOTE-PATH: image-pull" \
        "REMOTE-PATH: image-pull" "STUB-DOCKER: пошла тяга"
    skazano_do "и человеку — тоже до, а не вдогонку" \
        "тянет его ИЗ РЕЕСТРА сам" "STUB-DOCKER: пошла тяга"

    # Ключ остался — им человек велит взять ОПУБЛИКОВАННОЕ, даже когда своё под рукой.
    run "${DEFAULT[@]}" -- add vps --addr world@10.8.0.5 --pull
    if [ "$RC" -eq 0 ] && has "--context world-vps pull $IMAGE" && ! has "save $IMAGE"; then
        ok "--pull велит взять из реестра даже при своём образе под рукой"
    else
        bad "--pull не сработал при своём образе на машине (код $RC)" "$JOURNAL" \
            "ключ есть, а свойства нет — это хуже отсутствия ключа"
    fi
    if in_out "велено взять образ ИЗ РЕЕСТРА"; then ok "и сказано, что путь назвал человек, а не наличие"
    else bad "не названо, почему пошли в реестр" "$OUT" \
        "«каким путём и почему» — разные ответы у ключа и у наличия, и путать их нельзя"; fi

    # С ключом свой демон не нужен вовсе: образ берут не у нас, и спрашивать нечего.
    run "${DEFAULT[@]}" STUB_LOCAL_IMAGE=0 STUB_LOCAL_DAEMON=1 -- add vps --addr world@10.8.0.5 --pull
    if [ "$RC" -eq 0 ]; then ok "с --pull своего демона не нужно — образ берут не у нас"
    else bad "с --pull подъём требует своего демона (код $RC)" "$OUT"; fi

    # Образ, который на ресурсе УЖЕ лежит, не везётся ни тем путём, ни другим.
    run "${DEFAULT[@]}" STUB_REMOTE_IMAGE=1 -- add vps --addr world@10.8.0.5
    if ! has "save $IMAGE" && ! has "pull $IMAGE" && in_out "REMOTE-PATH: image-present"; then
        ok "образ на ресурсе уже есть — не везём ни копией, ни из реестра, и это сказано"
    else
        bad "образ везётся, хотя на ресурсе он уже лежит" "$JOURNAL"
    fi

    # ИМЯ ВЕЩИ НАЗЫВАЕТСЯ МАШИНЕ — как путь и как отказ. Знает его тот, кто прочитал рецепт,
    # то есть мы: зовущему разбирать рецепт второй раз незачем, а без имени он не отличит
    # СВОЮ вещь от чужой на той же машине (`WORLD2-150`, живой прогон 2026-08-20: на одной
    # машине законно стоят раздачи разных юзеров, и список показывал их все).
    run "${DEFAULT[@]}" -- add vps --addr world@10.8.0.5
    if in_out "REMOTE-THING: $(sed -n 's/^name: *//p' "$RECIPE" | head -n1 | tr -d '"'"'"'"')"; then
        ok "имя вещи названо машине — тем же именем, каким её зовёт рецепт"
    else
        bad "имя вещи машине не названо" "зовущий не отличит свою вещь от чужой на той же машине" "$OUT"
    fi

    # Образ без HEALTHCHECK — это НЕ отказ и НЕ «здорова»: третье состояние, и оно называется
    # вслух. Вещь, у которой проверки нет вовсе, обязана вставать (`WORLD2` 4.2).
    run STUB_CTX_EXISTS=0 STUB_SSH_OK=1 STUB_REMOTE_DOCKER=1 STUB_REMOTE_IMAGE=1 \
        STUB_LOCAL_IMAGE=1 STUB_NET_EXISTS=1 STUB_UP=ok STUB_HEALTH=none \
        -- add vps --addr world@10.8.0.5 --recipe "$OTHER"
    if [ "$RC" -eq 0 ] && in_out "HEALTHCHECK"; then
        ok "вещь без HEALTHCHECK встаёт, и непроверенное названо непроверенным"
    else
        bad "вещь без HEALTHCHECK не встала либо промолчала об этом (код $RC)" "$OUT" \
            "чужая вещь не обязана иметь нашу проверку здоровья; выдать «встало» за «отвечает» тоже нельзя"
    fi

    part "СНЯТИЕ — на той стороне контейнер, на этой контекст"
    run STUB_CTX_EXISTS=1 STUB_CTX_ENDPOINT=ssh://world@10.8.0.5:22 STUB_REMOTE_DOCKER=1 \
        STUB_NET_EXISTS=1 -- drop vps
    if [ "$RC" -eq 0 ]; then ok "снятие прошло целиком"
    else bad "снятие не прошло (код $RC)" "$OUT"; fi
    if has "--context world-vps compose" && has " down"; then ok "контейнер снимается на том ресурсе"
    else bad "контейнер на ресурсе не снимается" "одна команда ставит — одна снимает, иначе за миром надо убирать руками"; fi
    if has "context rm world-vps"; then ok "контекст убирается с ЭТОЙ машины"
    else bad "контекст остаётся на машине человека" "list врал бы про ресурс, которого уже нет"; fi
    if has "--context world-vps network rm $NET"; then ok "общая сеть снимается, если свободна"
    else bad "сеть на ресурсе не снимается" "след остался бы там, где обещана чистота"; fi
    if printf '%s\n' "$JOURNAL" | grep -q 'down.*-v'; then
        bad "тома сносятся без спроса" "состояние поля — не мусор: стирать его молча нельзя"
    else
        ok "тома НЕ сносятся без --with-state — состояние поля переживает снятие"
    fi

    run STUB_CTX_EXISTS=1 STUB_CTX_ENDPOINT=ssh://world@10.8.0.5:22 STUB_REMOTE_DOCKER=1 \
        STUB_NET_EXISTS=1 -- drop vps --with-state --with-image
    if printf '%s\n' "$JOURNAL" | grep -q 'down.*-v'; then ok "--with-state сносит и тома"
    else bad "--with-state тома не снёс" "ключ есть, а свойства нет"; fi
    if has "--context world-vps image rm"; then ok "--with-image снимает образ с ресурса"
    else bad "--with-image образ не снял" "после него на машине не должно остаться и образа"; fi

    part "СПИСОК И СОСТОЯНИЕ"
    run STUB_CTX_EXISTS=1 -- list
    if in_out "vps" && [ "$RC" -eq 0 ]; then ok "list показывает заведённые ресурсы"
    else bad "list ресурсов не показал (код $RC)" "$OUT"; fi

    run STUB_CTX_EXISTS=1 STUB_CTX_ENDPOINT=ssh://world@10.8.0.5:22 STUB_REMOTE_DOCKER=1 \
        STUB_REMOTE_IMAGE=1 STUB_NET_EXISTS=1 -- status vps
    # Состояние спрашивается у ЧУЖОГО демона по каждой вещи отдельно: выведенное состояние
    # врёт ровно тогда, когда на него смотрят (`WORLD2` 4.2).
    if [ "$RC" -eq 0 ] && in_out "контейнер" && in_out "образ" && in_out "здоровье"; then
        ok "status говорит, что на ресурсе сейчас"
    else
        bad "status не назвал состояние ресурса (код $RC)" "$OUT"
    fi
    # Состояние вещи — её тома, и имена их читаются по метке проекта, а не по нашему списку.
    if in_out "world-field"; then ok "тома вещи названы по факту, а не по зашитому списку"
    else bad "тома вещи не названы" "$OUT" "своего списка томов у механики нет — он принадлежит рецепту"; fi
}

# ------------------------------------------------------------------ красный прогон
red() {
    part "КРАСНЫЙ ПРОГОН — нарочные поломки, у каждой СВОЙ код"

    want_refusal no-name "имя ресурса не названо" "${DEFAULT[@]}" -- add
    want_refusal bad-name "имя ресурса не годится" "${DEFAULT[@]}" -- add "ВПС"
    want_refusal no-address "ресурс не заведён и адрес не назван" "${DEFAULT[@]}" -- add vps
    want_refusal bad-address "адрес похож на IPv6" "${DEFAULT[@]}" -- add vps --addr "world@fe80::1:22"
    # ТРИ ОТКАЗА ПРО РЕЦЕПТ, а не один «плохой рецепт»: файла нет · файл не разбирается · в
    # разобранном не названо имя образа. Чинятся они разными действиями, и общий код отправлял
    # бы человека чинить не то (`WORLD2` 2.3).
    want_refusal no-recipe "рецепта, которого нет" "${DEFAULT[@]}" \
        -- add vps --addr world@10.8.0.5 --recipe "$STUB_DIR/такого-рецепта-нет.yaml"
    want_refusal bad-recipe "рецепт, который компоуз не разбирает" "${DEFAULT[@]}" \
        -- add vps --addr world@10.8.0.5 --recipe "$BROKEN"
    want_refusal recipe-no-image "рецепт без имени образа" "${DEFAULT[@]}" \
        -- add vps --addr world@10.8.0.5 --recipe "$NOIMAGE"
    # Отказ про рецепт обязан случиться ДО того, как мы тронули ресурс: непригодный рецепт не
    # вправе оставить за собой ни контекста, ни соединения.
    if has "context create" || has "ssh "; then
        bad "отказ по рецепту случился, когда ресурс уже трогали" "$JOURNAL" \
            "рецепт читается до первого шага наружу: чинить его — работа на этой машине"
    else
        ok "отказ по рецепту — до первого касания ресурса"
    fi

    want_refusal access-denied "ключ не принят" \
        STUB_CTX_EXISTS=0 STUB_SSH_OK=0 STUB_REMOTE_DOCKER=1 STUB_LOCAL_IMAGE=1 \
        -- add vps --addr world@10.8.0.5
    # ДОКЕР НА ТОЙ СТОРОНЕ — ТРИ РАЗНЫХ СОСТОЯНИЯ, И ТОЛЬКО ОДНО ИЗ НИХ МИР ЧИНИТ САМ.
    # Докера нет вовсе — ставит мир (это зелёный прогон). Докер СТОИТ, а демон молчит либо
    # прав говорить с ним не дают — мир его не трогает и отказывает, и коды разные: чинят их
    # разными действиями (`WORLD2` 2.3, «есть докер — не трогаем» из `3.5`).
    want_refusal no-remote-docker "докер на той стороне СТОИТ, а демон молчит" \
        STUB_CTX_EXISTS=0 STUB_SSH_OK=1 STUB_REMOTE_DOCKER=0 STUB_REMOTE_DOCKER_CLI=yes \
        STUB_REMOTE_DOCKER_WHY=down STUB_LOCAL_IMAGE=1 \
        -- add vps --addr world@10.8.0.5
    if has "$INSTALL_URL"; then
        bad "докер ставится на машину, где он уже стоит" "$JOURNAL" \
            "«есть докер — не трогаем» значит и не ставим поверх: сломанный чужой докер чинит его хозяин"
    else
        ok "стоящий докер мир не трогает — ни ставит поверх, ни чинит"
    fi
    want_refusal remote-docker-denied "докер СТОИТ, а прав говорить с его демоном нет" \
        STUB_CTX_EXISTS=0 STUB_SSH_OK=1 STUB_REMOTE_DOCKER=0 STUB_REMOTE_DOCKER_CLI=yes \
        STUB_REMOTE_DOCKER_WHY=denied STUB_LOCAL_IMAGE=1 \
        -- add vps --addr world@10.8.0.5

    # УСТАНОВКА НЕ ВЫШЛА — ОТКАЗ С ПРИЧИНОЙ, И ПРИЧИНЫ РАЗЛИЧИМЫ. «Не достала до сети» и
    # «нет прав» — разные состояния, и первое ЗАКОННОЕ: машина без выхода наружу не сломана.
    want_refusal docker-install-denied "докера нет, и ставить его нечем: ни root, ни sudo" \
        "${CHISTAYA[@]}" STUB_REMOTE_ROOT=no STUB_REMOTE_SUDO=no \
        -- add vps --addr world@10.8.0.5
    want_refusal docker-install-no-net "докера нет, и машина не достала до установщика" \
        "${CHISTAYA[@]}" STUB_REMOTE_NET=no \
        -- add vps --addr world@10.8.0.5
    # Отказ по сети обязан случиться ДО установки, а не после половины дела.
    if has "$INSTALL_URL | sh"; then
        bad "установка пошла, хотя машина до установщика не достала" "$JOURNAL" \
            "отказ не вправе оставлять за собой начатое (WORLD2 2.3, пункт 4)"
    else
        ok "не достала до сети — установка не начиналась вовсе"
    fi
    want_refusal docker-install-no-fetch "докера нет, и качать установщик нечем" \
        "${CHISTAYA[@]}" STUB_REMOTE_FETCH=none STUB_REMOTE_NET=no \
        -- add vps --addr world@10.8.0.5
    want_refusal docker-install-failed "установщик докера ответил ошибкой" \
        "${CHISTAYA[@]}" STUB_DOCKER_INSTALL=fail \
        -- add vps --addr world@10.8.0.5
    # «Установщик сказал ок» и «докер отвечает» — разные вещи, и вторую надо подтвердить.
    want_refusal docker-install-silent "докер поставлен, а по нашему пути не отвечает" \
        "${CHISTAYA[@]}" STUB_DOCKER_INSTALL=silent \
        -- add vps --addr world@10.8.0.5
    # Машина замолчала между проверкой доступа и вопросом про докер — это третье состояние,
    # и выдавать его за «докера нет» нельзя: мы бы полезли ставить докер вслепую.
    want_refusal access-denied "машина оборвала связь на вопросе про докер" \
        "${CHISTAYA[@]}" STUB_PROBE_DIES=1 \
        -- add vps --addr world@10.8.0.5
    # А это ЧЕТВЁРТОЕ: машина ответила, но не о докере. Так отвечает чужая оболочка юзера
    # (fish, csh) — не поломка связи и не отсутствие докера, и чинится оно третьим действием.
    want_refusal bad-remote-answer "машина ответила, но не о докере" \
        "${CHISTAYA[@]}" STUB_PROBE_ALIEN=1 \
        -- add vps --addr world@10.8.0.5
    if has "$INSTALL_URL"; then
        bad "докер ставится вслепую на машину, которая нам не ответила" "$JOURNAL" \
            "мы не знаем, есть там докер или нет: ставить поверх чужого — прямое нарушение границы"
    else
        ok "непонятный ответ — докер не ставится вслепую"
    fi
    # Реестр отказал — это СВОЯ ступень, а не «образ не доехал»: чинится она не повтором
    # передачи, а доступом той машины к реестру либо открытием пакета.
    want_refusal image-pull-failed "ресурсу не дают притянуть образ из реестра" \
        STUB_CTX_EXISTS=0 STUB_SSH_OK=1 STUB_REMOTE_DOCKER=1 STUB_REMOTE_IMAGE=0 \
        STUB_LOCAL_IMAGE=1 STUB_NET_EXISTS=1 STUB_PULL=1 \
        -- add vps --addr world@10.8.0.5 --pull
    # ТОТ ЖЕ отказ, когда путь выбрал не человек, а наличие: образа нет ни здесь, ни там, и
    # реестр не отдал. Выход тут другой — положить образ у себя, и он поедет копией.
    want_refusal image-pull-failed "образа нет нигде, и реестр не отдал" \
        STUB_CTX_EXISTS=0 STUB_SSH_OK=1 STUB_REMOTE_DOCKER=1 STUB_REMOTE_IMAGE=0 \
        STUB_LOCAL_IMAGE=0 STUB_NET_EXISTS=1 STUB_PULL=1 \
        -- add vps --addr world@10.8.0.5
    if in_out "поедет копией"; then ok "и выход назван тот, что чинит ЭТОТ случай: положи образ у себя"
    else bad "выход не назван либо назван чужой" "$OUT" \
        "«убери --pull» здесь бессмысленно: ключа никто не называл, путь выбрало наличие"; fi
    # Свой демон нужен, чтобы УЗНАТЬ, есть ли образ здесь. Не отвечает — это своя ступень, а
    # не «образа нет»: гадать между «нет образа» и «нечем посмотреть» нельзя (`WORLD2` 4.2).
    want_refusal no-daemon "свой демон молчит — узнать, есть ли образ здесь, нечем" \
        STUB_CTX_EXISTS=0 STUB_SSH_OK=1 STUB_REMOTE_DOCKER=1 STUB_REMOTE_IMAGE=0 \
        STUB_LOCAL_DAEMON=1 STUB_NET_EXISTS=1 \
        -- add vps --addr world@10.8.0.5
    want_refusal port-busy "порт на том ресурсе занят" \
        STUB_CTX_EXISTS=0 STUB_SSH_OK=1 STUB_REMOTE_DOCKER=1 STUB_REMOTE_IMAGE=1 \
        STUB_LOCAL_IMAGE=1 STUB_NET_EXISTS=1 STUB_UP=busy \
        -- add vps --addr world@10.8.0.5
    # ТО ЖЕ САМОЕ ДРУГИМИ СЛОВАМИ ДОКЕРА. Шаблонов в `remote.sh` три, потому что докер
    # говорит про занятый порт по-разному, а проверялся до сих пор ровно один — и рядом с
    # проверенным спокойно жил мёртвый, целиком лежавший внутри соседнего (`WORLD2-158`).
    # Второе слово стережём отдельно: иначе «шаблонов три» остаётся обещанием.
    want_refusal port-busy "порт занят, и докер сказал это ДРУГИМИ словами" \
        STUB_CTX_EXISTS=0 STUB_SSH_OK=1 STUB_REMOTE_DOCKER=1 STUB_REMOTE_IMAGE=1 \
        STUB_LOCAL_IMAGE=1 STUB_NET_EXISTS=1 STUB_UP=busy2 \
        -- add vps --addr world@10.8.0.5
    want_refusal no-such-resource "снимают ресурс, которого не заводили" \
        STUB_CTX_EXISTS=0 -- drop vps
    # «Запущено» и «отвечает» — разные вещи, и вторую мы обязаны дождаться либо отказать.
    # Ждём ВСЕ контейнеры рецепта, поэтому нездоровый обязан покраснеть, а не потеряться
    # среди здоровых соседей.
    want_refusal thing-silent "вещь запущена, а здоровой за отведённое время не стала" \
        WORLD_REMOTE_WAIT=0 STUB_CTX_EXISTS=0 STUB_SSH_OK=1 STUB_REMOTE_DOCKER=1 \
        STUB_REMOTE_IMAGE=1 STUB_LOCAL_IMAGE=1 STUB_NET_EXISTS=1 STUB_UP=ok \
        STUB_HEALTH=unhealthy -- add vps --addr world@10.8.0.5

    part "ОТКАЗ ОБЯЗАН НАЗЫВАТЬ ВЫХОД, А НЕ ТОЛЬКО ПРИЧИНУ (WORLD2 2.3)"
    run STUB_CTX_EXISTS=0 STUB_SSH_OK=0 STUB_REMOTE_DOCKER=1 STUB_LOCAL_IMAGE=1 \
        -- add vps --addr world@10.8.0.5
    if in_out "выход: "; then ok "у отказа есть выход, а не только диагноз"
    else bad "отказ без выхода" "«нельзя» без выхода — провал самого мира: юзеру нечего чинить"; fi

    part "ОТКАЗ НИЧЕГО НЕ ЛОМАЕТ (WORLD2 2.3, пункт 4)"
    # Ресурс не пустил — значит на нём не должно оказаться ни сети, ни контейнера, а на
    # машине человека — ни контекста. Попытка не состоялась целиком, а не наполовину.
    if has "context create" || has "network create" || has " up -d"; then
        bad "после отказа остались следы" "$JOURNAL" \
            "попытка обязана не состояться целиком, а не наполовину"
    else
        ok "после отказа не создано ни контекста, ни сети, ни контейнера"
    fi
}

# ------------------------------------------------------------------ живой прогон
# Единственная проверка, которая отвечает на вопрос задачи: встаёт ли дверь на чистом
# ресурсе. Подставной докер на него не отвечает и отвечать не может.
live() {
    part "ЖИВОЙ ПРОГОН на настоящем втором ресурсе"

    # Подставные инструменты убираем: здесь нужен настоящий докер и настоящий ssh.
    # Каталог заглушек закавычен ВНУТРИ подстановки: имя временного каталога приходит от
    # `mktemp`, и незакавыченным оно читалось бы как шаблон (`SC2295`).
    local real_path="${PATH#"$STUB_DIR:"}"

    if [ -z "$LIVE_ADDR" ]; then
        skip "дверь встаёт на чистом ресурсе" \
            "второго ресурса не названо — назови адрес и повтори:" \
            "  WORLD_PROBE_REMOTE_ADDR=world@10.8.0.5 ./deploy/probe-remote.sh --live" \
            "на том ресурсе нужен ssh с ключом; докера может не быть — мир поставит его сам, и" \
            "  машина БЕЗ докера — самый ценный живой прогон: подставной установщик не отвечает" \
            "  за настоящий дистрибутив, и подтвердить B1 может только он"
        return
    fi
    if ! PATH="$real_path" command -v docker >/dev/null 2>&1 \
       || ! PATH="$real_path" docker info >/dev/null 2>&1; then
        skip "дверь встаёт на чистом ресурсе" \
            "на ЭТОЙ машине нет живого докера — образ нечем взять и отправить" \
            "гоняй живой прогон там, где докер настоящий"
        return
    fi

    local rc out
    printf '  · поднимаю дверь на %s (имя ресурса %s)\n' "$LIVE_ADDR" "$LIVE_NAME" >&2
    out=$(PATH="$real_path" "$REMOTE" add "$LIVE_NAME" --addr "$LIVE_ADDR" 2>&1); rc=$?
    if [ "$rc" -eq 0 ]; then ok "дверь встала на чистом ресурсе одной командой"
    else bad "дверь на ресурсе не встала (код $rc)" "$out"; fi

    out=$(PATH="$real_path" "$REMOTE" status "$LIVE_NAME" 2>&1)
    case "$out" in
        *healthy*) ok "дверь на ресурсе здорова" ;;
        *)         bad "дверь на ресурсе не здорова" "$out" ;;
    esac

    printf '  · снимаю дверь вместе с состоянием и образом\n' >&2
    out=$(PATH="$real_path" "$REMOTE" drop "$LIVE_NAME" --with-state --with-image 2>&1); rc=$?
    if [ "$rc" -eq 0 ]; then ok "дверь снялась одной командой"
    else bad "дверь не снялась (код $rc)" "$out"; fi

    # Проверяем ЧУЖИМИ глазами — спрашиваем сам ресурс, а не свой контекст: контекста уже
    # нет, да и верить надо тому, что осталось на машине, а не тому, что мы о ней думаем.
    # Это ЧТЕНИЕ, а не работа на чужой машине: ничего туда не ставится и не кладётся.
    #
    # Адрес разбираем так же, как его разбирает сам инструмент: возьми мы `${ADDR%%:*}`,
    # ресурс с нестандартным ssh-портом проверялся бы на 22-м — и проверка «ничего не
    # осталось» краснела бы связью вместо того, чтобы отвечать на свой вопрос.
    local live_target="$LIVE_ADDR" live_port=22
    case "${live_target##*@}" in
        *:*) live_port="${live_target##*:}"; live_target="${live_target%:*}" ;;
    esac
    local left
    left=$(PATH="$real_path" ssh -o BatchMode=yes -p "$live_port" "$live_target" \
              "docker ps -a --filter name=$DOOR --format '{{.Names}}'; docker volume ls --format '{{.Name}}' | grep -E '^world-(field|stand)\$'" 2>&1)
    if [ -z "$(printf '%s' "$left" | tr -d '[:space:]')" ]; then
        ok "на ресурсе не осталось ни контейнера, ни томов"
    else
        bad "на ресурсе остались наши вещи" "$left" \
            "одна команда ставит — одна снимает; за миром не должно оставаться уборки"
    fi
}

# ------------------------------------------------------------------ прогон
case "$mode" in
    --green) green ;;
    --red)   red ;;
    --live)  live ;;
    --both)  green; red; live ;;
esac

part "ИТОГ"
printf '  проверок: %s, из них не сошлись: %s, НЕ ПРОГНАНО: %s\n' "$total" "$failed" "$skipped" >&2
printf '  докер и ssh в этом прогоне ПОДСТАВНЫЕ — контейнеров не запускалось ни одного\n' >&2

if [ "$failed" -gt 0 ]; then
    printf '\n\033[1;31m✘ краснота: %s из %s\033[0m\n' "$failed" "$total" >&2
    exit 1
fi
if [ "$skipped" -gt 0 ]; then
    printf '\n\033[1;33m• НЕПОЛНЫЙ ПРОГОН: %s не проверено\033[0m\n' "$skipped" >&2
    printf '  зелёного при неполном прогоне не бывает: «проверили, что смогли» и\n' >&2
    printf '  «проверили» — разные слова (WORLD2 4.2)\n' >&2
    exit 3
fi
printf '\n\033[1;32m✔ полный зелёный: %s из %s, включая живой прогон\033[0m\n' "$total" "$total" >&2
exit 0
