        ...
        case "$*" in
            *"${STUB_NODE_IMAGE:?}"*)
            ...
            "run -d"*)
                ...
                #
                # `STUB_RUN_FAIL` заставляет докер ОТКАЗАТЬ, и это не прихоть сценария:
                # именно так он ведёт себя на занятом имени, занятом порте и отсутствующем
                # образе. Проба обязана превратить его сырой текст в отказ с причиной и
                # выходом (`WORLD2-91`), а не умереть им.
...
# Шапка того же прогона: проба обязана СКАЗАТЬ, чем собран образ, который она берёт
# (`WORLD2-92`). Здесь штамп подставного образа совпадает с версией подставного гита —
# значит она обязана сказать «сходится», а не промолчать.
said "чем собран образ"
said "штамп"
said "сходится: образ сложен из того же, что лежит в дереве"

# ============== 10a. ОБРАЗ СТАРШЕ ДЕРЕВА И ДОКЕР ОТКАЗАЛ — два молчания разом
# `WORLD2-92`: проба брала готовый образ и молчала об этом. 12.08 это дважды дало ложный
# результат — сперва зелёный на прежнем образе, потом красный на образе до правки `core`, и
# полчаса ушло на выяснение, чья зона, при том что обе стороны были правы.
#
# `WORLD2-91`: заглушки поднимались голым `docker run -d`, и отказ докера убивал прогон
# сырым текстом — без причины на нашем языке и без следующего действия. То есть проба
# нарушала правило, которое сама стережёт в других (`WORLD2` 2.3).
#
# Оба случая в одном прогоне НАРОЧНО: подъём падает сразу, и сценарий стоит секунду, а не
# пятнадцать. Проверяем два разных молчания — про образ и про отказ.
part "10a. дерево ушло вперёд образа и докер отказал → сказано и то, и другое"
STUB_HAVE_IMAGE=1 STUB_HAVE_PROBE=1 STUB_STAMP=sha-0000000 STUB_RUN_FAIL=1 \
    run_case 1 "$HERE/probe.sh" --green
said "ДЕРЕВО УШЛО ВПЕРЁД ОБРАЗА"
said "sha-0000000"                       # чем собран образ — названо значением, а не «старый»
said "проверить написанное"              # и назван выход: пересобрать
said "не поднялся"                       # отказ подъёма — свой, а не сырой текст докера
said "докер сказал"                      # ответ докера приложен, он же первая причина
said "docker rm -f world-probe-loc"      # и назван выход, который можно исполнить

# ============== 10b. КРАСНЫЙ ПРОГОН НЕ ДЕЛИТ ИМЁН С ЗЕЛЁНЫМ
# Вторая половина `WORLD2-92`: улика зелёного погибала от уборки красного, а его же
# «занятое имя» превращалось в законное возвращение и давало два ложных провала. Теперь у
# красного прогона своя заглушка, и проверяется это по журналу вызовов: он не должен
# трогать имя зелёного ВООБЩЕ.
part "10b. заглушка красного прогона — своя, имени зелёного он не касается"
STUB_HAVE_IMAGE=1 STUB_HAVE_PROBE=1 STUB_RUN_FAIL=1 \
    run_case 1 "$HERE/probe.sh" --red
called "rm -f world-probe-loc-red"
if grep -qx -- 'rm -f world-probe-loc' "$STUB_LOG"; then
    bad "красный прогон тронул заглушку ЗЕЛЁНОГО — улику разбирать будет нечем"
else
    good "имени зелёной заглушки красный прогон не касался ни разу"
fi
...
# Пустая переменная — не «сними переопределение», а несказанное значение (`WORLD2-113`).
# Трактовок было ДВЕ и в одном файле: подстановка считала пустую незаданной, а проверка
# противоречия — заданной. Отказ из-за этого называл человеку ЧУЖОЕ значение: «в
# WORLD_REGISTRY — ghcr.io», хотя тот не писал туда ничего. Сценарий воспроизводит ту самую
# строку из находки и стережёт обе половины правки: что отказ теперь про пустое значение и
# что прежнего — лживого — больше нет.
part "29w. WORLD_REGISTRY задана ПУСТОЙ → отказ про несказанное значение, а не про «два реестра»"
WORLD_REGISTRY='' \
STUB_CONTROL_IMAGE=registry.local:5000/omnifield/world-control:dev \
STUB_HAVE_PROBE=1 STUB_BUILDER=ok \
    run_case 1 "$HERE/release.sh" --control
said "WORLD_REGISTRY задана, а значения в ней нет"
said "unset WORLD_REGISTRY"                  # выход назван, а не только причина
grep -qF 'два разных реестра' "$STUB_DIR/out" \
    && bad "отказ снова про «два реестра»: пустая переменная опять читается как названный реестр" \
    || good "про «два разных реестра» не сказано — причина названа своя"
grep -qF 'в WORLD_REGISTRY — ghcr.io' "$STUB_DIR/out" \
    && bad "умолчание снова подано как СОДЕРЖИМОЕ переменной — ровно находка WORLD2-113" \
    || good "умолчание не выдаётся за то, что написал человек"
not_called "push"
not_called "control/compose.yaml build"      # отказ ДО сборки: собирать нечего и незачем
