...
# Адрес, по которому машина видит хост-порт ДВЕРИ (умолчание `127.0.0.1`, довод целиком —
# в `up.sh`). Здесь он живёт только в подсказках-командах: снятие ходит к двери не само —
# ходит человек, и команда, которую ему дают, обязана исполняться на ЕГО машине.
...
Переменные среды:
  ...
  WORLD_HOST=10.8.1.1       адрес, по которому машина видит хост-порт двери —
                            подставляется в подсказки (умолчание 127.0.0.1)
...
if [ -n "$keep_in_field" ]; then
    ...
    warn "  curl -X DELETE http://$HOST:<хост-порт двери>/api/locations/$WORLD_NAME"
else
    ...
    if [ "$leave_status" = 0 ]; then
        ...
        warn "чем убрать её с хоста: curl -X DELETE http://$HOST:<хост-порт двери>/api/locations/$WORLD_NAME"
    fi
fi
...
$(printf '\033[1;32m✓ локация «%s» снята\033[0m' "$WORLD_NAME")
  ...
  СКЛАД МЕСТА ЦЕЛ: том с построенным снятие не трогает (down идёт без -v). Поднимешь
  локацию обратно — застройка окажется на месте; поле мира тоже цело и стоит как стояло

  кто сейчас в поле    curl -sS http://$HOST:<хост-порт двери>/api/locations
  поднять обратно      ./deploy/location-up.sh$CONFIG_KEY   (повторный вход не ошибка)
  стереть построенное  docker volume rm ${PROJECT}_place    (отдельным действием, нарочно)
GOTOVO
