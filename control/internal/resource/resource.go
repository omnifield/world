...
const (
	...
	// PathMark — приставка, которой сосед называет МАШИНЕ, каким путём пошёл: `REMOTE-PATH:
	// image-copy`. Значение принадлежит `deploy/remote.sh`, здесь оно повторено — это ШОВ,
	// и он стережётся пробой ровно так же, как `REMOTE-REFUSAL` и приставка контекста.
	//
	// Само слово после метки контроллер НЕ ТОЛКУЕТ и своего словаря путей не заводит:
	// перечень путей принадлежит соседу, и второй такой же разъехался бы с ним на первой
	// правке — тот же довод, что у кодов отказа (`internal/refusal`).
	PathMark = "REMOTE-PATH: "
	// ThingMark — приставка, которой сосед называет МАШИНЕ имя поднятой вещи: `REMOTE-THING:
	// world`. Имя принадлежит рецепту, и знает его тот, кто рецепт прочитал. Второй разбор
	// рецепта здесь не заводим: он разъехался бы с первым на первой правке компоуза — тот же
	// довод, что у кодов отказа и у путей.
	ThingMark = "REMOTE-THING: "
)
...
type Manager struct {
	...
	Port int
	// СВОЕЙ ВЕРСИИ СБОРКИ ЗДЕСЬ НЕТ И НЕ ЗАВОДИТСЯ (`WORLD2-146`). Была: ею контроллер
	// подставлял тег в имя образа рецепта. Снята вместе с подстановкой — версия вещи
	// контроллера не касается, а поле, которое некому читать, назавтра обрастает читателем.
	// Своя версия по-прежнему называется в журнале при подъёме контроллера (`cmd/control`).
	//
	...
	// OnPath — куда уходит метка пути, названная соседом, В МОМЕНТ ЕЁ ПОЯВЛЕНИЯ. Поле, а
	// не глобальный канал: подменяемость — то, чем проба проверяет, что метка уехала ПОКА
	// ШАГ ЕЩЁ ИДЁТ, а не после (`WORLD2-150` B2). Зовётся из чужой горутины — то, что за
	// этим полем стоит, обязано быть под замком (`internal/progress`).
	OnPath func(name, path string)
}
...
// │                                                                                      │
// │ Машина отвечает обо ВС�ём, что на ней поднято, и на одной машине законно стоят        │
// │ раздачи разных юзеров (`WORLD2` 3.4, «Копий нет — есть адреса»). Показывать их все —  │
// │ значит раскрывать чужие имена и порты каждому, кто дошёл до пульта, а пульт открыт    │
// │ по адресу (`B4`, решение того же дня).                                                │
// │                                                                                      │
// │ Занятость порта от этого не прячется: её называет ОТКАЗ при подъёме, и называет сам   │
// │ докер. Видеть соседа, чтобы узнать, что место занято, юзеру не нужно.                 │
// │                                                                                      │
// │ ПУСТОЙ СПИСОК ЗНАЧИТ «НЕ ЗНАЕМ», А НЕ «НИЧЕГО СВОЕГО НЕТ»: так выглядят скоупы,       │
// │ заведённые до того, как контроллер начал это помнить. Прятать у них всё значило бы    │
// │ стереть юзеру его вещи с экрана — беда хуже той, что чинится (`WORLD2` 4.2).          │
// └─────────────────────────────────────────────────────────────────────────────────────┘
func свои(стоит []Thing, наши []string) []Thing {
	if стоит == nil || len(наши) == 0 {
		return стоит
	}
	знаем := make(map[string]bool, len(наши))
	for _, имя := range наши {
		знаем[имя] = true
	}
	out := make([]Thing, 0, len(стоит))
	for _, t := range стоит {
		if знаем[t.Name] {
			out = append(out, t)
		}
...
// знает вещь, а не он.
// Возвращает ИМЯ поднятой вещи — то, которым её назвал рецепт. Оно уезжает в скоуп: на
// одной машине законно стоят вещи разных юзеров, и отличить своё можно, только помня, что
// сам поднимал (`WORLD2-150`, живой прогон 2026-08-20).
func (m *Manager) Raise(ctx context.Context, name, addr, recipePath string, env []string) (string, *refusal.Refusal) {
	// ЧЕМ ПОДНИМАЕМ — решает РЕЦЕПТ, а мы это НАЗЫВАЕМ ВСЛУХ (`WORLD2-146`). Своего имени
	// образа контроллер в подъём не подставляет: версия вещи его не касается. Строка пишется
	// при каждом подъёме — молчание тут и есть тот дефект, ради которого узел заведён
	// (`WORLD2-130`), и он никуда не делся.
	env = append(append([]string{}, env...), m.remoteEnv()...)
	m.say("control: вещь по рецепту %s на территории %s — %s", recipePath, name, m.raisedWith(ctx, recipePath, env))
	...
	res, err := m.Runner.Run(ctx, run.Command{
		...
		Args: args,
		Env:  env,
		// КАКИМ ПУТЁМ ПОШЛИ — читается ИЗ ЖИВОГО ПОТОКА, а не из собранного вывода после
		// выхода процесса. Разница здесь вся: копия образа по ssh и установка докера — шаги
		// на минуты, и метка, разобранная после, называет уже случившееся.
		OnLine: m.pathLine(name),
	})
	if err != nil || res.Code != 0 {
		return "", m.toolFailure(err, res, "поставить вещь по рецепту "+recipePath+" на "+addr+" не вышло")
	}
	return firstMatch(res.Out, ThingMark), nil
}

// pathLine — разбор одной строки stdout соседа на лету. Метку довозим ДОСЛОВНО: слово
// после приставки принадлежит ему, и переводить его в своё значило бы завести второй
// словарь путей, который разъедется с первым (`internal/refusal`, тот же довод про коды).
func (m *Manager) pathLine(name string) func(string) {
	return func(line string) {
		rest, found := strings.CutPrefix(strings.TrimSpace(strip(line)), PathMark)
		if !found {
			return
		}
		...
		// В журнал — тоже сейчас, а не потом: человек у машины контроллера читает его
		// журнал живьём, и «поехали копией» после копии ему так же бесполезно.
		m.say("control: территория %s — путь %s", name, path)
		if m.OnPath != nil {
			m.OnPath(name, path)
		}
	}
...
// │ раздачу: ту, что стоит на этой машине под именем по умолчанию, — а нашу оставил бы    │
// │ жить. «Снятие одной не трогает вторую» держится ровно на этом.                        │
// │                                                                                      │
// │ Тем же ходом закрывается и второе: рецепт объявляет пароль ОБЯЗАТЕЛЬНЫМ (`${…:?}`),   │
...
func (m *Manager) Lower(ctx context.Context, d Drop) (Dropped, *refusal.Refusal) {
	...
	// СНИМАЕМ ТЕМ ЖЕ, ЧЕМ СТАВИЛИ, и теперь это выходит само: имя образа оба раза называет
	// один и тот же рецепт, а не мы (`WORLD2-146`). Разъехаться нечему — второго имени в
	// зоне нет вовсе.
	//
	// Чего это НЕ обещает: `latest` подвижен. Имя то же, содержимое за ним могло смениться —
	// и `--with-image` унесёт то, что лежит под именем сейчас. Обещать больше мы не вправе.
	// Строка в журнале здесь по той же причине, что и при подъёме: чем действуем, говорим вслух.
	env = append(append([]string{}, env...), m.remoteEnv()...)
	m.say("control: снимаю вещь по рецепту %s с территории %s — %s", recipePath, name, m.raisedWith(ctx, recipePath, env))
