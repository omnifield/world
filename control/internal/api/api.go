...
//	GET    /api/fields             поля юзера: записи о комьюнити, которые ему известны
...
func (h *Handler) postScope(w http.ResponseWriter, r *http.Request) *refusal.Refusal {
	...
	if m := body.Machine; m != nil {
		...
		// │                                                                                │
		// │ ИСХОДОВ ЗДЕСЬ ТОЖЕ ЧЕТЫРЕ, и разведены все: `PresenceState` — личность в томе,   │
		// │ отказ ниже; `PresenceEmpty` — раздача встала пустой, ровно этого мы и ждали;    │
		// │ `PresenceNone` и отказ измерения — по адресу не отвечают, и запись состояния     │
		// │ следующим шагом упрётся в ту же беду и скажет о ней своим кодом. Молчаливого     │
		// │ успеха нет ни в одном из четырёх.                                                │
		// └───────────────────────────────────────────────────────────────────────────────┘
...
func (h *Handler) deleteScope(w http.ResponseWriter, r *http.Request) *refusal.Refusal {
	...
	// └─────────────────────────────────────────────────────────────────────────────────────┘
	// ┌─────────────────────────────────────────────────────────────────────────────────────┐
	// │ ЧЕТЫРЕ ИСХОДА ВОПРОСА, И РАЗВЕДЕНЫ ВСЕ ЧЕТЫРЕ. `Look` отвечает тремя значениями плюс │
	// │ отказом, и сторож, стоящий на ОДНОЙ ветке, пропускает остальные молча (три круга     │
	// │ ревью `WORLD2-152` — каждый раз одно и то же):                                        │
	// │                                                                                      │
	// │   отказ            измерить не вышло — говорим ОБ ЭТОМ, а не выдаём неизмеренное      │
	// │                    за снятое (`WORLD2` 4.2: непроверенное называется непроверенным);  │
	// │   `PresenceNone`   по адресу никто не отвечает — сняли то самое;                       │
	// │   `PresenceEmpty`  раздача ЖИВА и пуста (состояния в ней нет) — снятое ею не было;     │
	// │   `PresenceState`  раздача жива и отдаёт личность — снятое ею не было.                 │
	// │                                                                                      │
	// │ Последние два разведены НЕ перечислением, а умолчанием: `default` ловит и то значение │
	// │ `Presence`, которого сегодня нет. Появись оно — попадёт в безопасную сторону (отказ), │
	// │ а не в «снято»: язык о новом значении не предупредит, а обещание успеха дорого.       │
	// └─────────────────────────────────────────────────────────────────────────────────────┘
	неизмерено := ""
	живо, ref := sc.Look(r.Context())
	switch {
	case ref != nil:
		неизмерено = "снялось ли по адресу, проверить не вышло (" + ref.Code + ") — посмотри сам: " + addr.String()
		h.opt.Logf("control: раздача снята, а измерить это не вышло (%s): %s", ref.Code, addr)
	case живо == scope.PresenceNone:
		// По адресу больше никто не отвечает. Только это и значит «сняли».
	default:
		h.res.DropKey(m.Name)
		h.res.DropContext(r.Context(), m.Name)
		...
		чтоТам := "раздача ПО-ПРЕЖНЕМУ отвечает"
		if живо == scope.PresenceEmpty {
			// «Отвечает пустотой» — это ЖИВАЯ раздача, а не снятая: состояния в ней нет, а
			// сама она стоит. Свалить эти два случая в один значило бы отчитаться успехом
			// ровно там, где юзер стёр состояние руками.
			чтоТам = "раздача ПО-ПРЕЖНЕМУ отвечает (состояния в ней нет, но стоит она сама)"
		}
		return refusal.New(http.StatusConflict, "share-alive",
			fmt.Sprintf("снятие прошло, а по адресу %s %s: на машине снималось %s — то есть не то, что стоит по этому адресу",
				addr, чтоТам, чтоСнимали),
			"имя участка назови ТО ЖЕ, что при заведении: из него и порта скоупа собрано имя раздачи, и снимается она по нему",
