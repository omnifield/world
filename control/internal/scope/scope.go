...
//
// ┌─────────────────────────────────────────────────────────────────────────────────────┐
// │ ВЫХОД, ВЕДУЩИЙ ТУДА, ГДЕ ЧЕЛОВЕК УЖЕ СТОИТ, — НЕ ВЫХОД (`WORLD2` 2.3).               │
// │                                                                                      │
// │ Разбор адреса и разговор с раздачей общие у трёх ручек, и выходы у них были общие    │
// │ тоже. Написаны они были для ВХОДА, где «скоупа ещё нет — заведи его: POST /api/scope» │
// │ стоит на месте, и вместе с общей проверкой уехали в само заведение. Живой прогон      │
// │ 2026-08-20 дал юзеру петлю: «подними её: POST /api/scope» ровно тогда, когда он это   │
// │ и делал.                                                                              │
// │                                                                                      │
// │ Причина отказа остаётся ОДНА и живёт на месте: разъедься она по ручкам — получился бы │
// │ второй словарь того же самого. Наружу вынесен только тот выход, который НАЗЫВАЕТ      │
// │ РУЧКУ, и спрашивается он у того, кто спросил.                                         │
// │                                                                                      │
// │ Своя ручка спрашивающего лежит в той же строке таблицы, и это здесь главное: «не      │
// │ советуй ручку, на которой стоишь» проверяется тогда по таблице ЦЕЛИКОМ, а не по трём  │
// │ заученным случаям. Добавили спрашивающего — он проверился сам.                        │
// │                                                                                      │
// │ Не назвавшийся получает выходы БЕЗ ручек (`AskingUnnamed`, нулевое значение): новая   │
// │ ручка, забывшая назваться, промолчит, а не соврёт. Цена забывчивости — недостающая    │
// │ подсказка, а не петля.                                                                │
// └─────────────────────────────────────────────────────────────────────────────────────┘

// Asking — кто спрашивает: ручка, на которой стоит человек.
type Asking int

const (
	// AskingUnnamed — спрашивающий себя не назвал. Безопасная сторона: выходов с ручками нет.
	AskingUnnamed Asking = iota
	// AskingSignIn — вход в скоуп, `POST /api/session`.
	AskingSignIn
	// AskingCreate — заведение скоупа, `POST /api/scope`.
	AskingCreate
	// AskingDrop — снятие скоупа по адресу, `DELETE /api/scope`.
	AskingDrop
)

// trouble — О ЧЁМ отказ. Не код: код называет отказ целиком, а здесь — та беда, на которую
// у каждой ручки свой ответ.
type trouble int

const (
	// troubleNoAddress — адрес скоупа не назван (`no-address`).
	troubleNoAddress trouble = iota
	// troubleNoAnswer — по адресу никто не отвечает (`no-answer`).
	troubleNoAnswer
	// troubleNoState — раздача отвечает, а состояния в ней нет (`no-scope`).
	troubleNoState
)

// asker — ручка спрашивающего и выходы, которые ей годятся.
type asker struct {
	// handle — ручка, НА КОТОРОЙ СТОИТ человек. Ею и проверяется таблица: советовать её
	// самой себе нельзя.
	handle string
	ways   map[trouble][]string
}

var askers = map[Asking]asker{
	// Вход: скоупа по адресу может не быть вовсе, и завести его — законный следующий ход.
	AskingSignIn: {
		handle: "POST /api/session",
		ways: map[trouble][]string{
			troubleNoAddress: {"скоупа ещё нет — заведи его: POST /api/scope"},
			troubleNoAnswer:  {"подними её: POST /api/scope с этим адресом и машиной, где раздача должна стоять"},
			troubleNoState:   {"заведи его здесь же: POST /api/scope с этим адресом, паролем и именем"},
		},
	},
	// Заведение: «заведи скоуп» тут и есть то, что человек делает. Про адрес первый выход
	// верен и достаточен; про молчащую раздачу сказать есть что — её поднял ЭТОТ ЖЕ вызов.
	AskingCreate: {
		handle: "POST /api/scope",
		ways: map[trouble][]string{
			troubleNoAnswer: {"раздачу поднимает этот же вызов — проверь, что порт в адресе тот, на котором она слушает (у нашей вилки 8070)"},
		},
	},
	// Снятие: заводить нечего, человек убирает. Адрес у снятия тот же, каким заводили.
	AskingDrop: {
		handle: "DELETE /api/scope",
		ways: map[trouble][]string{
			troubleNoAddress: {"снимают по тому же адресу, по которому заводили"},
		},
	},
}

// ways — выходы спрашивающего для этой беды. Неизвестный спрашивающий — пусто, и это
// умолчание карты, а не перечисление: молчание безопаснее совета, ведущего в петлю.
func (a Asking) ways(t trouble) []string { return askers[a].ways[t] }
...
// `who` — кто спрашивает: от этого зависит ВЫХОД, но не причина (`WORLD2-144`).
func Parse(raw string, who Asking) (Address, *refusal.Refusal) {
	...
	if raw == "" {
		return Address{}, refusal.New(http.StatusBadRequest, "no-address",
			...
			With(who.ways(troubleNoAddress)...)
	}
...
type Scope struct {
	...
	password string
	client   *http.Client
	// who — КТО этот скоуп открыл. Держится здесь по той же причине, по какой пароль:
	// отказы рождаются глубоко внизу (`Read`, `reachFailure`), а выход зависит от ручки,
	// на которой стоит человек (`WORLD2-144`).
	who Asking
}
...
// `who` — та же ручка, которой разбирали адрес: скоуп открывают сразу за разбором, и
// назвать себя дважды по-разному нельзя.
func Open(addr Address, password string, timeout int, who Asking) *Scope {
	if timeout <= 0 {
		timeout = 10
	}
	return &Scope{
...
func (s *Scope) Read(ctx context.Context) (*state.State, *refusal.Refusal) {
	...
	case http.StatusNotFound:
		...
		return nil, refusal.New(http.StatusNotFound, "no-scope",
			...
			"назови адрес, где состояние уже лежит: две раздачи — два скоупа").
			With(s.who.ways(troubleNoState)...)
	default:
...
func (s *Scope) reachFailure(err error) *refusal.Refusal {
	...
	if errors.Is(err, syscall.ECONNREFUSED) {
		return refusal.New(http.StatusBadGateway, "no-answer",
			...
			With(s.who.ways(troubleNoAnswer)...)
	}
