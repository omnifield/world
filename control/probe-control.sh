...
# │  11. СКОУПОВ НА ОДНОЙ МАШИНЕ СКОЛЬКО УГОДНО: имя и порт раздачи контроллер берёт из    │
# │      того, что юзер уже назвал, две раздачи живут разом, у каждой своё состояние, и    │
# │      снятие одной не трогает вторую (`WORLD2-150` B3);                                 │
# │  12. КАКИМ ПУТЁМ ПОШЛА ДОСТАВКА, пульт узнаёт ПОКА ШАГ ИДЁТ, а не после (`B2`);        │
# │  13. пульт публикуется НАРУЖУ умолчанием, и цена сказана в ИТОГОВОМ блоке подъёма —    │
# │      там, где человек читает, а не выше по журналу, откуда оно уехало (`B4`);          │
# │  14. словарь кодов отказа стережётся КЛАССОМ, а не списком: что подсказки обещают —    │
# │      существует, что сосед может сказать — доезжает до пульта дословно, все коды;      │
# │  15. МИР ПОДНЯЛ РАЗДАЧУ И УШЁЛ (`C1`): свежий скоуп — личность и пустота, а нашей      │
# │      строки в `authorized_keys` той машины нет ни после заведения, ни после снятия;    │
# │  16. ЧТО МИР ЗАВЁЛ, ТО ОН И СНИМАЕТ: раздача снимается по адресу тем же рецептом и     │
# │      теми же значениями, повторный вызов отказывает внятно, а ТОМ переживает снятие —  │
# │      личность стирается только явным `?with-state=1`, и оба исхода сказаны вслух.      │
# │                                                                                      │
...
case "${1:-}" in
    ...
    # ПОДСКАЗКА — ЭТО ШАПКА ЦЕЛИКОМ, И ГРАНИЦА У НЕЁ ОДНА: первая строка, которая не
    # комментарий. Зашитый номер конца жил отдельно от шапки, которую печатает, и разъехался
    # с ней на первой же правке — этой самой, когда шапка выросла на шесть строк (тот же род,
    # что закрыла зона `share` в `WORLD2-150`). Номеров здесь больше нет ни одного.
...
# ┌─────────────────────────────────────────────────────────────────────────────────────┐
# │ СКОУПОВ НА ОДНОЙ МАШИНЕ СКОЛЬКО УГОДНО (`WORLD2` 3.4, `WORLD2-150` B3).              │
# │                                                                                      │
# │ Соседние порты — это соседние СКОУПЫ на той же машине. Проверять их надо живьём, а не │
# │ по журналу вызовов: журнал показал бы «имя названо» и там, где обоим названо ОДНО    │
# │ имя. Поэтому подставной подъём поднимает раздачу ТАМ и ПОД ТЕМ ИМЕНЕМ, которые назвал │
# │ контроллер, а состояние держит в файле по ИМЕНИ — как настоящий рецепт держит его в    │
# │ томе по `SHARE_NAME`. Зашей контроллер имя в умолчание, и вторая раздача правда сядет  │
# │ в том первой; зашей порт — правда не встанет по названному адресу.                     │
# └─────────────────────────────────────────────────────────────────────────────────────┘
SHARE_PORT_B=$((SHARE_PORT + 1))
SHARE_PORT_C=$((SHARE_PORT + 2))
SHARE_PORT_D=$((SHARE_PORT + 3))
SHARE_URL_B="http://127.0.0.1:$SHARE_PORT_B/"
SHARE_URL_C="http://127.0.0.1:$SHARE_PORT_C/"
SHARE_URL_D="http://127.0.0.1:$SHARE_PORT_D/"
# Состояния раздач, поднятых подставным подъёмом: по файлу на ИМЯ. Том рецепта устроен так
# же (`${SHARE_NAME}-state`), и «у каждой свой том» проверяется ровно здесь.
SHARE_STATE_DIR="$TMP/раздачи"
SHARE_PIDS="$TMP/раздачи.pid"
# Ворота метки пути: подставной подъём печатает метку и ЖДЁТ, пока проба их не откроет.
# Так проверяется не «сказал ли», а «сказал ДО того, как путь занял время» (B2).
PATH_GATE="$TMP/ворота-пути"
...
SRV_WORLD_IMAGE=""
SRV_SHARE_IMAGE=""
# Значения прогона для подставного подъёма: печатать ли метку пути, ждать ли ворот, вставать
# ли раздаче вообще. Пустые — обычный прогон.
SRV_PATH_SAY=""
SRV_PATH_GATE=""
SRV_NO_RISE=""
...
cleanup() {
    ...
    # Раздачи, поднятые подставным подъёмом: каждая своим именем и своим процессом. Уйди
    # проба, не сняв их, — следующий прогон упёрся бы в занятые порты «по наследству».
    if [ -s "$SHARE_PIDS" ]; then
        while read -r _imya _pid; do kill "$_pid" 2>/dev/null || true; done < "$SHARE_PIDS"
    fi
...
#!/usr/bin/env bash
printf 'remote.sh %s\n' "$*" >> "$CALLS"
# ЧЕМ его позвали — в журнал отдельной строкой: значения уезжают подстановкой рецепта, и
# увидеть их в аргументах нельзя. Не запиши мы это, порча «вернуть умолчание» прошла бы
# незамеченной.
printf 'env WORLD_IMAGE=%s SHARE_IMAGE=%s SHARE_NAME=%s SHARE_PORT=%s SHARE_PASSWORD=%s\n' \
    "${WORLD_IMAGE:-}" "${SHARE_IMAGE:-}" "${SHARE_NAME:-}" "${SHARE_PORT:-}" "${SHARE_PASSWORD:-}" >> "$CALLS"
...
# ИМЯ ВЕЩИ НАЗЫВАЕТСЯ И НА ПОДЪЁМЕ, И НА СНЯТИИ — как у настоящего соседа: имя читает
# `read_recipe`, а её зовут обе команды (`deploy/remote.sh`). Заглушка, молчащая на снятии,
# проверяла бы контроллер ДОБРЕЕ живого подъёма — и отказ, которому нечем назвать снимаемую
# вещь, выглядел бы нашей недоработкой, а не её.
printf 'REMOTE-THING: %s\n' "${SHARE_NAME:-world}"

# КАКИМ ПУТЁМ ПОШЛИ — сосед печатает метку в stdout ДО самого пути. Здесь она печатается
# так же и в том же месте: до долгого шага, а не после. Ворота ниже — это и есть «долгий
# шаг»: заглушка ждёт, пока проба не скажет «видела», и ждёт с пределом, чтобы прогон не
# висел вечно, если метка не доехала.
path_say() {
    printf 'REMOTE-PATH: %s\n' "$1"
    [ -n "${PATH_GATE:-}" ] || return 0
    # Ждём ЗАМЕТНО дольше, чем проба спрашивает ход. Сравняй эти два предела — и порча
    # «разобрать вывод после выхода процесса» проходила бы по счастливой случайности: подъём
    # сдавался бы ровно тогда, когда проба ещё смотрит, и метка успевала бы «показаться».
    n=0
    while [ ! -f "$PATH_GATE" ] && [ "$n" -lt 300 ]; do sleep 0.1; n=$((n + 1)); done
}

# ПОДЪЁМ ПО РЕЦЕПТУ РАЗДАЧИ — и раздача правда встаёт ТАМ и ПОД ТЕМ ИМЕНЕМ, что назвал
# контроллер. Иначе проверялось бы не то: мы смотрим, что он зовёт подъём и ЖДЁТ появления
# раздачи по названному адресу, а не что он сам что-то у себя записал. Порт и имя берутся
# из окружения — ровно оттуда, откуда их берёт настоящий рецепт.
case " $* " in
    *" add "*)
        case " $* " in
            *" --recipe ${SHARE_RECIPE:-нет-такого} "*)
                [ -n "${PATH_SAY:-}" ] && path_say "$PATH_SAY"
                # Рецепт раздачи объявляет пароль ОБЯЗАТЕЛЬНЫМ, а имя и порт читает
                # подстановкой. Не назвали — рецепт не разберётся, и настоящий подъём
                # откажет. Заглушка отказывает тем же кодом, а не молча делает по-своему.
                if [ -z "${SHARE_PASSWORD:-}" ] || [ -z "${SHARE_NAME:-}" ] || [ -z "${SHARE_PORT:-}" ]; then
                    printf 'REMOTE-REFUSAL: bad-recipe\n'
                    printf '\n✗ отказ: рецепту раздачи не назвали SHARE_NAME/SHARE_PORT/SHARE_PASSWORD\n' >&2
                    printf '  выход: назови их подъёму\n' >&2
                    exit 1
                fi
                [ -n "${NO_RISE:-}" ] && exit 0   # подъём «прошёл», а раздача не встала
                mkdir -p "$SHARE_STATE_DIR"
                "$SHARE_BIN" "127.0.0.1:$SHARE_PORT" "$SHARE_PASSWORD" "$SHARE_STATE_DIR/$SHARE_NAME.json" \
                    > "$SHARE_LOG.$SHARE_NAME" 2>&1 < /dev/null &
                printf '%s %s\n' "$SHARE_NAME" "$!" >> "$SHARE_PIDS"
                n=0
                while [ "$n" -lt 100 ]; do
    ...
    *" drop "*)
        # СНИМАЕМ ТО, ЧТО НАЗВАНО, а не «раздачу вообще»: имя приходит тем же значением,
        # каким поднимали. Позови контроллер снятие без него — здесь снялась бы раздача
        # ПО УМОЛЧАНИЮ, то есть соседняя, а названная осталась бы жить.
        #
        # ТОМ ПЕРЕЖИВАЕТ СНЯТИЕ и стирается ТОЛЬКО по `--with-state` — как у настоящего
        # рецепта (`compose down -v`). Стирай заглушка его всегда, порча «стирать том без
        # ключа» проходила бы молча: проверять было бы нечего.
        if [ -n "${SHARE_NAME:-}" ] && [ -s "$SHARE_PIDS" ]; then
            : > "$SHARE_PIDS.tmp"
            # Имена переменных ЛАТИНСКИЕ: bash не поддерживает не-ASCII в именах и молча
            # превращает `read -r имя` в «not a valid identifier» — читается пустота, и
            # снятие тихо не снимает ничего.
            while read -r imya pid; do
                if [ "$imya" = "$SHARE_NAME" ]; then
                    kill "$pid" 2>/dev/null || true
                    case " $* " in
                        *" --with-state "*) rm -f "$SHARE_STATE_DIR/$imya.json" ;;
                    esac
                else
                    printf '%s %s\n' "$imya" "$pid" >> "$SHARE_PIDS.tmp"
                fi
            done < "$SHARE_PIDS"
            mv "$SHARE_PIDS.tmp" "$SHARE_PIDS"
        fi ;;
esac
exit 0
FAKE
...
share_stop() {
    ...
    # Раздачи, поднятые подставным подъёмом: их теперь столько, сколько скоупов, и у каждой
    # своё имя. Второго списка одного и того же не бывает — он тут один.
    if [ -s "$SHARE_PIDS" ]; then
        while read -r _imya _pid; do kill "$_pid" 2>/dev/null || true; done < "$SHARE_PIDS"
        : > "$SHARE_PIDS"
    fi
...
start_server() {
    ...
    # ┌─────────────────────────────────────────────────────────────────────────────────┐
    # │ `SHARE_NAME`, `SHARE_PORT` И `SHARE_PASSWORD` ЗДЕСЬ НЕ ПЕРЕДАЮТСЯ НАМЕРЕННО.      │
    # │                                                                                  │
    # │ Их обязан назвать КОНТРОЛЛЕР — в этом вся проверка. Положи мы их в окружение      │
    # │ прогона, подставной подъём увидел бы их и на порче «перестать передавать»: имя и  │
    # │ порт нашлись бы сами, раздача встала бы, и проба прошла бы МОЛЧА. Ровно то, что   │
    # │ рядом с порчей проходит молча, и надо было убрать.                                │
    # └─────────────────────────────────────────────────────────────────────────────────┘
    ...
    SHARE_BIN="$TMP/share" SHARE_RECIPE="$SHARE_RECIPE" SHARE_LOG="$SHARE_LOG" \
    SHARE_STATE_DIR="$SHARE_STATE_DIR" SHARE_PIDS="$SHARE_PIDS" \
    PATH_GATE="${SRV_PATH_GATE:-}" PATH_SAY="${SRV_PATH_SAY:-}" NO_RISE="${SRV_NO_RISE:-}" \
...
# net_podstanovki_u_vyzova <кусок вызова> <имя проверки> — контроллер НЕ подставил имя образа
# в ИМЕННО ЭТОТ вызов подъёма. Смотреть надо у своего вызова, а не «где-нибудь в журнале»:
# снятие зовётся тем же путём, и порча «вернуть подстановку версии» зеленела бы на соседней
# строке про другое действие. Подставной подъём пишет свои значения строкой СРАЗУ за вызовом,
# и пустое значение после знака равенства — это и есть «не подставляли».
net_podstanovki_u_vyzova() {
    local vyzov="$1" imya="$2" stroki
    stroki="$(grep -F -A1 -- "$vyzov" "$CALLS" || true)"
    if [ -z "$stroki" ]; then
        bad "$imya" "подъёма «$vyzov» в журнале вызовов нет вовсе:" "$(tail -4 "$CALLS")"
        return
    fi
    if printf '%s\n' "$stroki" | grep -qE '(WORLD_IMAGE|SHARE_IMAGE)=[^ ]'; then
        bad "$imya" "контроллер подставил имя образа в подъём вещи — версия вещи его не касается:" "$stroki"
    else
        ok "$imya"
    fi
}

# pole <имя> — значение строкового поля ВЕРХНЕГО уровня в ответе.
#
# ┌─────────────────────────────────────────────────────────────────────────────────────┐
# │ ЗАЧЕМ ЭТО, ЕСЛИ ЕСТЬ `want_has`. Тот ищет текст ГДЕ УГОДНО в ответе — и зеленеет на   │
# │ чужой копии: слово «стёрт» есть и в перечне снятого, и в сказанном вслух, а свойство  │
# │ заявлено про второе. Сторож, привязанный к присутствию текста там, где заявлено       │
# │ свойство, проходит молча ровно на той порче, ради которой заведён (свой разбор класса │
# │ — `WORLD2-150`). Поэтому там, где проверяется СКАЗАННОЕ, поле вынимается по имени.    │
# │                                                                                      │
# │ Берётся ПОСЛЕДНЕЕ вхождение: ключи в ответе идут по алфавиту, и вложенное `note`      │
# │ (внутри `dropped`) стоит раньше верхнего.                                             │
# └─────────────────────────────────────────────────────────────────────────────────────┘
pole() { printf '%s' "$BODY" | sed -n "s/.*\"$1\":\"\([^\"]*\)\".*/\1/p"; }

# vyhody — выходы снятия (`dropped.ways`) одной строкой. В удачном ответе массив `ways`
# один, и это он: отказ сюда не приходит вовсе.
vyhody() { printf '%s' "$BODY" | sed -n 's/.*"ways":\[\([^]]*\)\].*/\1/p'; }
...
# завести [адрес] [пароль] [имя участка] [имя личности] — скоуп заводится ТАМ, где будет
# лежать: две пары (машина и скоуп) плюс личность.
zavesti() {
    local url="${1:-$SHARE_URL}" pass="${2:-$SHARE_PASS}" uchastok="${3:-vps}" imya="${4:-егор}"
    call POST /api/scope "{\"scope\":{\"addr\":\"$url\",\"password\":\"$pass\"},\"identity\":{\"name\":\"$imya\",\"brand\":\"\"},\"machine\":{\"name\":\"$uchastok\",\"addr\":\"world@10.8.0.5\",\"creds\":{\"kind\":\"key\",\"value\":\"-----ключ-----\"}}}"
    TOKEN="$(printf '%s' "$BODY" | sed -n 's/.*"token":"\([^"]*\)".*/\1/p')"
    # Файл поднятой раздачи находим ПО ЖУРНАЛУ, а не пишем копией: имя выводит контроллер, и
    # написанное здесь имя проверяло бы нашу догадку вместо его решения.
    # `|| true` не украшение: раздача могла и не встать (порча, отказ), и тогда имени в
    # журнале нет — а `set -e` оборвал бы весь прогон на пустом ответе.
    local fajl; fajl="$(fajl_razdachi || true)"
    [ -n "$fajl" ] && SHARE_FILE="$fajl"
    return 0
}

# снять [адрес] [пароль] [имя участка] [ключи ручки] — снять скоуп по адресу. Тело ТО ЖЕ,
# что у заведения, минус личность: снимают то, что лежит по адресу, а не того, кем себя
# назвали (`WORLD2-152`).
snyat() {
    local url="${1:-$SHARE_URL}" pass="${2:-$SHARE_PASS}" uchastok="${3:-vps}" klyuchi="${4:-}"
    call DELETE "/api/scope$klyuchi" "{\"scope\":{\"addr\":\"$url\",\"password\":\"$pass\"},\"machine\":{\"name\":\"$uchastok\",\"addr\":\"world@10.8.0.5\",\"creds\":{\"kind\":\"key\",\"value\":\"-----ключ-----\"}}}"
}

# fajl_razdachi [имя] — состояние раздачи, поднятой подставным подъёмом. Без имени берётся
# то, которым звали ПОСЛЕДНИЙ подъём.
fajl_razdachi() {
    local imya="${1:-}"
    if [ -z "$imya" ]; then
        imya="$(grep -o 'SHARE_NAME=[^ ]*' "$CALLS" 2>/dev/null | tail -n1 | cut -d= -f2)"
    fi
    [ -n "$imya" ] && printf '%s\n' "$SHARE_STATE_DIR/$imya.json"
}
...
# Имя образа РАЗДАЧИ — своё, и это не удвоение: у каждого образа своя версия и свой выпуск
# (`WORLD2-146`). Пока имя было одно, проверка «раздачу спросили отдельно» зеленела бы на
# ответе про дверь.
ENTRY_SHARE_IMAGE=ghcr.io/omnifield/world-share:latest
# ┌─────────────────────────────────────────────────────────────────────────────────────┐
# │ ОБЯЗАТЕЛЬНЫЕ ПЕРЕМЕННЫЕ РЕЦЕПТА РАЗДАЧИ — ПЕРЕЧЕНЬ БЕРЁТСЯ У СОСЕДА (`WORLD2-145`).   │
# │                                                                                      │
# │ Компоуз на необъявленной `${ИМЯ:?…}`-переменной отказывается разбирать файл ЦЕЛИКОМ:  │
# │ `config` не отдаёт ничего. Значит точка входа, спрашивающая имя образа, обязана       │
# │ называть КАЖДУЮ такую переменную — иначе её проверка мертва не на какой-то машине, а  │
# │ на всякой. Именно так она и была мертва с рождения.                                   │
# │                                                                                      │
# │ Перечень читается из ФАЙЛА СОСЕДА, а не пишется здесь копией: заведёт он вторую такую │
# │ переменную — проба покраснеет сама, а не на машине юзера. Соседа рядом нет (прогон из │
# │ распакованного образа) — перечень пуст, и проверка честно пропускается.                │
# └─────────────────────────────────────────────────────────────────────────────────────┘
ENTRY_OBYAZATELNYE="$( [ -f "$ROOT/share/compose.yaml" ] \
    && grep -o '\${[A-Z_][A-Z0-9_]*:?' "$ROOT/share/compose.yaml" | sed 's/^\${//;s/:?$//' | sort -u \
    || true)"
...
case "$1" in
    ...
    image) # `image inspect <имя>` — лежит ли образ двери на этой машине
           ...
           exit 1 ;;
    compose)
           # КАКОЙ рецепт спросили — видно по `-f`, и отвечать надо ПО НЕМУ. Один ответ на
           # оба рецепта скрыл бы главное: что раздачу точка входа не спрашивала вовсе либо
           # спрашивала не тем — ровно этим и была мертва проверка (`WORLD2-145`).
           retsept=
           shift
           while [ $# -gt 0 ]; do
               case "$1" in
           ...
           # ЧЕМ спросили — в журнал вызовов. Перечень имён приходит СНАРУЖИ (из рецепта
           # соседа), а не написан здесь: заглушка про обязательные переменные ничего не
           # знает и знать не должна — иначе проверялся бы один известный случай.
           znacheniya=
           for imya in ${STUB_LOG_VARS:-}; do
               eval "znachenie=\${$imya:-}"
               znacheniya="$znacheniya $imya=$znachenie"
           done
           printf 'compose config -f %s%s\n' "$retsept" "$znacheniya" >> "${ENTRY_CALLS:?}"
           case "$1" in
               config)
                   case "$retsept" in
                       *share*)
                           # Рецепт не читается ЦЕЛИКОМ — так ведёт себя компоуз на
                           # необъявленной `:?`-переменной и на незакавыченном двоеточии.
                           [ -n "${STUB_SHARE_BROKEN:-}" ] && {
                               printf 'error while interpolating services.share.environment.SHARE_PASSWORD: required variable SHARE_PASSWORD is missing\n' >&2
                               exit 1
                           }
                           printf '%s\n' "${STUB_SHARE_IMAGE-}" ;;
                       *)  [ -n "${STUB_DOOR_BROKEN:-}" ] && {
                               printf 'yaml: line 7: mapping values are not allowed in this context\n' >&2
                               exit 1
                           }
                           printf '%s\n' "${STUB_DOOR_IMAGE-}" ;;
                   esac
...
entry_env() {
    ENTRY_ENV=(
        ...
        "STUB_SHARE_IMAGE=$ENTRY_SHARE_IMAGE"
        "STUB_LOG_VARS=$ENTRY_OBYAZATELNYE"
    )
}
...
# entry_said_v_itoge <кусок> <имя> — сказано ли это В ИТОГОВОМ блоке («контроллер поднят»),
# а не где-то выше по журналу.
#
# ┌─────────────────────────────────────────────────────────────────────────────────────┐
# │ РАЗНИЦА С `entry_said` — НАЙДЕНА ПОРЧЕЙ, А НЕ ПРИДУМАНА.                             │
# │                                                                                      │
# │ Цена публикации стояла в выводе ДВАЖДЫ — предупреждением в начале и в итоговом блоке. │
# │ Сторож на присутствие проходил ЗЕЛЁНЫМ на снятии любой из копий: текст-то в выводе    │
# │ оставался. То есть он стерёг присутствие текста там, где заявлено свойство.           │
# │                                                                                      │
# │ Свойство здесь не «слово где-то напечатано», а «человек это прочитал»: подъём печатает│
# │ десятки строк, и сказанное в начале уезжает из терминала прежде, чем он дочитает.     │
# │ Поэтому проверяется МЕСТО, а копия осталась одна.                                     │
# └─────────────────────────────────────────────────────────────────────────────────────┘
entry_said_v_itoge() {
    local hvost
    hvost="$(sed -n '/контроллер поднят/,$p' "$ENTRY_OUT" 2>/dev/null || true)"
    if [ -z "$hvost" ]; then
        bad "$2" "итогового блока «контроллер поднят» в выводе нет вовсе:" "$(tail -n 12 "$ENTRY_OUT")"
        return
    fi
    case "$hvost" in
        *"$1"*) ok "$2" ;;
        *) bad "$2" "в ИТОГОВОМ блоке точки входа нет «$1» — выше по журналу не считается, оно уехало из терминала:" "$hvost" ;;
    esac
}
...
green() {
    ...
    # ┌─────────────────────────────────────────────────────────────────────────────────┐
    # │ КРЕДЫ МАШИНЫ В СВЕЖИЙ СКОУП НЕ УЕЗЖАЮТ (`WORLD2-152`).                           │
    # │                                                                                  │
    # │ Раньше здесь стояло обратное: «креды записались в скоуп — дальше берутся оттуда». │
    # │ Брать их оттуда некому: мир поднял раздачу и ушёл, а ключ, оставленный на всякий  │
    # │ случай, — это доступ, о котором юзер не просил. Свойство проверяется КЛАССОМ:      │
    # │ связка ключей свежего скоупа ПУСТА, а не «в ней нет вот этой строки».              │
    # └─────────────────────────────────────────────────────────────────────────────────┘
    if grep -qF -- '"ключи": []' "$SHARE_FILE"; then
        ok "связка ключей свежего скоупа ПУСТА — мир поднял раздачу и ушёл"
    else
        bad "ключ в свежем скоупе" "он там лежит, а ходить туда мир больше не будет:" "$(head -c 400 "$SHARE_FILE")"
    fi
    ...
    # ТЕРРИТОРИЙ У СВЕЖЕГО СКОУПА НЕТ ВОВСЕ: машина раздачи участком не становится, а своих
    # юзер ещё не заводил. Пустой список — законное состояние, и он ЕСТЬ (`WORLD2` 4.2).
    call GET /api/resources
    want_has '"resources":[]' "у свежего скоупа территорий нет, а список есть"
    case "$BODY" in
    ...
    # И ВРЕМЯНОК ЗАВЕДЕНИЯ ТОЖЕ НЕ ОСТАЛОСЬ: ключ ложился в связку и контекст заводился ради
    # подъёма раздачи, а раздача поднята. Останься они — контроллер держал бы дорогу к
    # машине, о которой в личности не написано ничего.
    if [ ! -f "$KEYS/world-vps" ] && ! grep -qx 'world-vps' "$CTXFILE"; then
        ok "времянки заведения сняты: ни ключа в связке, ни контекста докера"
    else
        bad "времянки после заведения" \
    ...
    # ВОТ ТЕПЕРЬ территория есть — заведена ОТДЕЛЬНЫМ решением юзера. Ресурс это МАШИНА, а не
    # «машина с дверью» (`WORLD2-131`): что на ней стоит — отдельный вопрос и отдельное поле.
    call GET /api/resources
    want_has '"name":"vps2"' "участок, заведённый юзером, в списке есть"
    ...
    # Контекста докера здесь НЕ ждём: на живой машине его заводит сам подъём (`cmd_add`), а
    # подставной — заглушка, и требовать от неё чужой работы значило бы проверять пробу.
    # Что контексты производны от скоупа, стережёт проверка ниже — на смене личности.
    ...
    # ┌─────────────────────────────────────────────────────────────────────────────────┐
    # │ ВЕРСИЯ ВЕЩИ КОНТРОЛЛЕРА НЕ КАСАЕТСЯ (`WORLD2-146`). Публикация раздельная, у       │
    # │ каждого образа своя версия — «раз я такой сборки, то и вещи такой же» неправда.    │
    # │                                                                                    │
    # │ Стерегутся ДВА разных требования, и путать их нельзя:                              │
    # │   · контроллер имя образа в подъём НЕ подставляет — это сторож на порчу «вернуть   │
    # │     подстановку версии», и без него правило живёт в голове, а не в машине;          │
    # │   · при каждом подъёме сказано, ЧЕМ поднято (`WORLD2-130`) — молчащий подъём был    │
    # │     дефектом и им остаётся, потому что `latest` подвижен.                            │
    # │                                                                                    │
    # │ Штамп сборки у контроллера при этом ЕСТЬ (`SRV_VERSION`) — и именно поэтому первая  │
    # │ проверка что-то значит: будь его нет, подставлять было бы попросту нечего.          │
    # └─────────────────────────────────────────────────────────────────────────────────┘
    part "ЗЕЛЁНЫЙ — вещи поднимаются тем, что назвал РЕЦЕПТ, и сказано чем"

    call POST /api/resources '{"name":"vps5","addr":"world@10.8.0.5","creds":{"kind":"key","value":"ключ"}}'
    want_status 201 "территория заводится"
    net_podstanovki_u_vyzova "remote.sh add vps5 " \
        "дверь поднимается ТЕМ, ЧТО НАЗВАЛ РЕЦЕПТ — своей версии контроллер не подставил"
    ...
    if grep -F -- "vps5" "$TMP/server.log" | grep -q 'ghcr.io/omnifield/world:latest'; then
    ...
    # И версия контроллера в имя образа не попала НИ ОДНИМ путём — ни подстановкой, ни тегом,
    # ни припиской. Ищем её у всех вызовов подъёма: сторож на порчу, а не на формулировку.
    if grep -q -- "$SRV_VERSION" "$CALLS"; then
        bad "версия уехала в вещь" "штамп контроллера просочился в подъём вещи:" "$(grep -n -- "$SRV_VERSION" "$CALLS" | tail -3)"
    else
        ok "штампа контроллера в подъёме вещи нет вовсе — ни подстановкой, ни тегом"
    fi
    # Раздача — та же дорога и без единой оговорки: её версия тоже не наша.
    net_podstanovki_u_vyzova "remote.sh add vps --addr world@10.8.0.5" \
        "раздача поднимается тем, что назвал ЕЁ рецепт — она публикуется своим выпуском"
    if grep -q 'world-share:latest' "$TMP/server.log"; then
        ok "и названо, чем поднята раздача — именем из её рецепта"
    else
        bad "раздача молчит" "в журнале не названо, чем поднята раздача:" "$(tail -3 "$TMP/server.log")"
    fi
    # СВОЯ ВЕЩЬ ЮЗЕРА — тот же путь, и теперь без исключения: раньше здесь стояла оговорка
    # «пином не пиним, её образ не наш», теперь оговорки нет вовсе.
    ...
    if grep -q 'весы:1.2' "$TMP/server.log"; then
        ok "своя вещь юзера идёт тем же путём, и сказано, чем она поднята"
    else
        bad "чужой рецепт" "в журнале не названо, чем поднята своя вещь юзера:" "$(tail -3 "$TMP/server.log")"
    fi

    # ВЕРСИИ НЕТ — законное состояние (собран не выпуском, а на месте), и оно называется
    # вслух. На подъём вещей оно НЕ влияет ничем: они и со штампом идут тем, что назвал
    # рецепт. Поэтому здесь стережётся ровно одно — что контроллер сказал это про СЕБЯ.
    SRV_VERSION=""
    ...
    net_podstanovki_u_vyzova "remote.sh add vps7 " "без штампа контроллер тем более ничего не подставляет"
    if grep -F -- "vps7" "$TMP/server.log" | grep -q 'latest'; then
        ok "чем поднято, сказано и без штампа — состояние версии на это не влияет"
    else
        bad "молчание про подъём" "у подъёма нет строки про то, чем поднято:" "$(tail -3 "$TMP/server.log")"
    fi
    if grep -q 'версии своей не знаю' "$TMP/server.log"; then
        ok "про свою версию сказано при подъёме контроллера, а не только при первом добавлении"
    else
        bad "стартовая строка" "контроллер не назвал своё состояние версии при подъёме:" "$(head -5 "$TMP/server.log")"
    fi

    # ИМЯ, НАЗВАННОЕ ХОЗЯИНОМ СНАРУЖИ, читает САМ РЕЦЕПТ — контроллер его не толкует и не
    # переписывает (`WORLD2` 0.1). Значит подъём обязан назвать то, что из этого вышло:
    # измеренное, а не наше намерение.
    ...
    if grep -F -- "vps8" "$TMP/server.log" | grep -q 'своя/дверь:мояверсия'; then
        ok "названо ТО, ЧТО И ПОЕХАЛО: имя хозяина прочитал рецепт, а подъём его назвал"
    else
    ...
    if [ -f "$ROOT/deploy/remote.sh" ]; then
        ...
        # ┌─────────────────────────────────────────────────────────────────────────────┐
        # │ МЕТКА ПУТИ — ШОВ, ЗАВЕДЁННЫЙ `WORLD2-150` B2. Сосед печатает `REMOTE-PATH:    │
        # │ <путь>` в stdout ДО самого пути; читаем её мы и довозим до пульта.            │
        # │                                                                              │
        # │ Приставка повторена у нас константой — значит стережётся, как `REMOTE-REFUSAL`│
        # │ и приставка контекста. Смени её сосед молча — контроллер перестал бы видеть   │
        # │ путь вовсе, и «говорит вслух» стало бы молчанием без единой красной строки.    │
        # └─────────────────────────────────────────────────────────────────────────────┘
        if grep -q "REMOTE-PATH: " "$ROOT/deploy/remote.sh"; then
            ok "метку пути сосед печатает той же приставкой, какую мы читаем (REMOTE-PATH: )"
        else
            bad "метка пути" "в deploy/remote.sh этой приставки больше нет — контроллер ослепнет на пути доставки"
        fi
        # Приставку читаем из НАШЕГО кода, а не пишем в пробе второй копией: копия
        # разъехалась бы с оригиналом молча — ровно та беда, ради которой шов и стерегут.
        if grep -q 'PathMark = "REMOTE-PATH: "' "$HERE/internal/resource/resource.go"; then
            ok "наша приставка метки пути — одно значение в одном месте"
        else
            bad "приставка метки пути" "в internal/resource её нет либо она другая — шов разъехался"
        fi
    else
    ...
    if [ -f "$ROOT/deploy/compose.yaml" ]; then
        ...
        # ПОДСТАНОВКИ ИМЕНИ ОБРАЗА ЗДЕСЬ БОЛЬШЕ НЕ СТЕРЕЖЁТСЯ, и это не забывчивость. Пока
        # контроллер пинил вещи своей сборкой, `WORLD_IMAGE` у соседа был ШВОМ: пропади он —
        # пин уехал бы в пустоту. Теперь имя образа целиком дело рецепта (`WORLD2-146`), и
        # сторож на чужую подстановку сторожил бы то, чем мы не пользуемся, — то есть ничего.
    else
    ...
    if [ -f "$ROOT/share/compose.yaml" ]; then
        ...
        # ОБЯЗАТЕЛЬНЫЕ ПЕРЕМЕННЫЕ РЕЦЕПТА (`${ИМЯ:?…}`) — тоже шов, и стережётся он НЕ ЗДЕСЬ,
        # а прогоном точки входа («образ раздачи: есть · нет · рецепт не читается»). Причина
        # названа нарочно: текстовая проверка «имя встречается в файле» зеленела на подсказке
        # в комментарии, то есть мерила не то (`WORLD2-137` — тот же род). Перечень берётся
        # из ЭТОГО файла соседа, а проверяется по ЖУРНАЛУ ВЫЗОВОВ — чем спросили на самом деле.
        ...
        # ┌─────────────────────────────────────────────────────────────────────────────┐
        # │ ДВА ЗНАЧЕНИЯ, КОТОРЫМИ РЕЦЕПТ РАЗВОДИТ ДВЕ РАЗДАЧИ (`WORLD2-150` B3).         │
        # │                                                                              │
        # │ `SHARE_NAME` держит имя проекта, контейнера и ТОМА; `SHARE_PORT` — публикацию.│
        # │ Имена принадлежат рецепту соседа: ими он с нами и разговаривает. Переименуй   │
        # │ он их молча — раздача встала бы с умолчаниями, то есть вторая села бы поверх   │
        # │ первой, а контроллер об этом не узнал бы.                                     │
        # │                                                                              │
        # │ Сторожим НЕ «слово встречается в файле», а то, что оно стоит в ТЕХ САМЫХ       │
        # │ строках: имя проекта, имя контейнера, имя тома и публикация. Проверка на       │
        # │ присутствие прошла бы молча, останься имя только в комментарии.                │
        # └─────────────────────────────────────────────────────────────────────────────┘
        for stroka in "^name:.*SHARE_NAME" "container_name:.*SHARE_NAME" "name:.*SHARE_NAME.*-state" \
                      "^ *- .*SHARE_BIND.*SHARE_PORT"; do
            if grep -qE -- "$stroka" "$ROOT/share/compose.yaml"; then
                ok "рецепт раздачи разводит две раздачи строкой «$stroka»"
            else
                bad "рецепт раздачи, строка «$stroka»" \
                    "значение из неё ушло — вторая раздача сядет на имя или порт первой:" \
                    "$(grep -nE 'name:|ports:|^ *- \"' "$ROOT/share/compose.yaml" | head -8)"
            fi
        done
    else
        skip "шов с share (пароль, порт, имя)" "share/compose.yaml рядом нет — сверять не с чем"
    fi
...
# ------------------------------------------------------------------ зелёный: своя подсказка
# ┌─────────────────────────────────────────────────────────────────────────────────────┐
# │ ПОДСКАЗКА ЭТОГО СКРИПТА — ЕГО ШАПКА ЦЕЛИКОМ, И КОНЦОВ У ШАПКИ ДВА.                   │
# │                                                                                      │
# │ Сторож заведён не из стройности: этим заходом шапка выросла на шесть строк, а         │
# │ подсказка печаталась зашитым диапазоном `2,34p` — и молча потеряла хвост. Тот же род  │
# │ уже разбирала зона `share` в этой же задаче; здесь он нашёлся у нас, своей же правкой.│
# │                                                                                      │
# │ Проверяем ПОРЧЕЙ КОПИИ, а не пересказом правила: пересказ зеленел бы вместе с ошибкой.│
# │ Метка дописывается в копию первой строкой шапки и последней — и обе обязаны появиться │
# │ в подсказке. Концы стерегутся ОТДЕЛЬНО: теряются они по разным причинам («снял с      │
# │ головы больше одной строки» — это не «зашил номер конца») и чинятся разными правками. │
# └─────────────────────────────────────────────────────────────────────────────────────┘
podskazka_green() {
    part "ЗЕЛЁНЫЙ — подсказка пробы это её шапка ЦЕЛИКОМ, и концов у шапки два"
    local kopiya="$TMP/podskazka-proby.sh"

    # ГОЛОВА: метка первой строкой шапки, сразу за `#!`.
    awk 'NR==1{print; print "# МЕТКА-ГОЛОВЫ-ШАПКИ"; next} {print}' "$HERE/probe-control.sh" > "$kopiya"
    chmod +x "$kopiya"
    if "$kopiya" --help 2>/dev/null | grep -qF "МЕТКА-ГОЛОВЫ-ШАПКИ"; then
        ok "подсказка печатает шапку С ПЕРВОЙ строки"
    else
        bad "--help потерял начало шапки" "первый абзац шапки до человека не доехал" \
            "граница подсказки одна — первая строка, которая не комментарий; номеров строк в ней быть не должно"
    fi

    # ХВОСТ: метка последней строкой шапки — перед первой не-комментарной.
    awk 'BEGIN{v=1} v && $0 !~ /^#/ { print "# МЕТКА-ХВОСТА-ШАПКИ"; v=0 } {print}' \
        "$HERE/probe-control.sh" > "$kopiya"
    chmod +x "$kopiya"
    if "$kopiya" --help 2>/dev/null | grep -qF "МЕТКА-ХВОСТА-ШАПКИ"; then
        ok "подсказка печатает шапку ДО ПОСЛЕДНЕЙ строки"
    else
        bad "--help потерял хвост шапки" "конец шапки до человека не доехал — вернулся зашитый номер строки" \
            "ровно это и случилось этим заходом: шапка выросла, а диапазон остался прежним"
    fi

    # ОБРАТНАЯ: подсказка не смеет уходить в код — иначе «печатает всё» решало бы задачу
    # ценой того, что человек читает наш `set -euo pipefail` вместо ответа на свой вопрос.
    if "$HERE/probe-control.sh" --help 2>/dev/null | grep -qF 'set -euo pipefail'; then
        bad "--help печатает код" "подсказка ушла за шапку — человек читает наш код вместо ответа"
    else
        ok "подсказка кончается на шапке и в код не уходит"
    fi
    rm -f "$kopiya"
}

# ------------------------------------------------------------------ зелёный: словарь кодов
# ┌─────────────────────────────────────────────────────────────────────────────────────┐
# │ КОДЫ ОТКАЗА — КОНТРАКТ С СОСЕДОМ, И ОН ИЗМЕНИЛСЯ (`WORLD2-150`).                     │
# │                                                                                      │
# │ Сторожить СПИСОК кодов пробой нельзя: перечень принадлежит соседу, и переписанный    │
# │ здесь он станет второй правдой, которая разъедется на первой же его правке. Поэтому  │
# │ обе проверки ниже — КЛАССОВЫЕ: перечень читается у того, кто им отказывает, а мы      │
# │ проверяем ОТНОШЕНИЕ к нему.                                                           │
# │                                                                                      │
# │   1. что наши подсказки ОБЕЩАЮТ, обязано существовать. Обещание снятого кода — не     │
# │      опечатка: человек ищет в журнале слово, которого там не будет никогда, и учится  │
# │      не верить подсказкам (ровно это и случилось с `no-image`);                        │
# │   2. что сосед МОЖЕТ сказать, обязано доехать до пульта дословно — КАЖДЫЙ код, а не    │
# │      два-три выбранных. Выбери мы список руками, новый код соседа приезжал бы          │
# │      непроверенным, а схлопнутый — молча.                                              │
# └─────────────────────────────────────────────────────────────────────────────────────┘

# kody_soseda — чем отказывает `deploy/remote.sh`. Читается ИЗ НЕГО.
kody_soseda() {
    [ -f "$ROOT/deploy/remote.sh" ] || return 0
    grep -ohE '(^|[[:space:]])refuse [a-z][a-z0-9-]*' "$ROOT/deploy/remote.sh" | awk '{print $NF}' | sort -u
}

# kody_zony — чем отказываем МЫ САМИ: скрипты зоны и её код.
kody_zony() {
    {
        grep -ohE '(^|[[:space:]])refuse [a-z][a-z0-9-]*' "$HERE/up.sh" "$HERE/control-entry.sh" 2>/dev/null \
            | awk '{print $NF}'
        grep -rhoE 'refusal\.(New|Newf|FromTool)\([^"]*"[a-z][a-z0-9-]*"' "$HERE/internal" "$HERE/cmd" 2>/dev/null \
            | grep -oE '"[a-z][a-z0-9-]*"$' | tr -d '"'
    } | sort -u
}

# obeschannye_kody — что наши скрипты обещают человеку словами «кодом X».
obeschannye_kody() {
    grep -ohE 'кодом [a-z][a-z0-9-]*' "$HERE/up.sh" "$HERE/control-entry.sh" 2>/dev/null \
        | awk '{print $NF}' | sort -u
}

slovar_green() {
    part "ЗЕЛЁНЫЙ — словарь кодов: обещанное существует, сказанное соседом доезжает дословно"

    local izvestnye obeschannye kod net=""
    izvestnye="$(kody_soseda; kody_zony)"
    obeschannye="$(obeschannye_kody)"
    if [ -z "$obeschannye" ]; then
        bad "обещанные коды" "в подсказках зоны не нашлось ни одного «кодом X» — сторож мерит пустоту"
    else
        for kod in $obeschannye; do
            printf '%s\n' "$izvestnye" | grep -qx -- "$kod" || net="$net $kod"
        done
        if [ -z "$net" ]; then
            ok "каждый код, обещанный подсказками зоны, существует у того, кто им отказывает"
        else
            bad "подсказка обещает несуществующий код:$net" \
                "человек будет искать в журнале слово, которого там не будет никогда" \
                "перечень существующих читается из deploy/remote.sh, control/up.sh, control/control-entry.sh и кода зоны"
        fi
    fi

    local soseda; soseda="$(kody_soseda)"
    if [ -z "$soseda" ]; then
        skip "коды соседа доезжают дословно" "deploy/remote.sh рядом нет — гонять нечего"
        return
    fi
    # ┌─────────────────────────────────────────────────────────────────────────────────┐
    # │ СНЯТЫЙ КОД (он собран ниже из кусков) НЕ СУЩЕСТВУЕТ — НИ В КОДЕ, НИ В ТЕКСТАХ.    │
    # │                                                                                  │
    # │ Запрет был СЛЕДСТВИЕМ ошибки: пока заведение само записывало машину раздачи       │
    # │ участком, снятие такого участка стёрло бы файл, в котором записано, что у юзера    │
    # │ есть машины. Причины нет — нет и запрета (`WORLD2-152`).                          │
    # │                                                                                  │
    # │ Смотрим КЛАССОМ, в двух разных местах: чем зона ОТКАЗЫВАЕТ (читается из её кода)  │
    # │ и что она ОБЕЩАЕТ (читается из её текстов). Кода нет, а текст обещает — человек    │
    # │ ищет в журнале слово, которого не будет никогда; ровно это и случилось с           │
    # │ `no-image`. Образец собран из кусков, иначе проба нашла бы саму себя.              │
    # └─────────────────────────────────────────────────────────────────────────────────┘
    local snyatyj_kod="drop-scope""-home" ostalos
    if printf '%s\n' "$(kody_zony)" | grep -qx -- "$snyatyj_kod"; then
        bad "$snyatyj_kod вернулся в код зоны" \
            "запрет был следствием ошибки, а не правилом: территория раздачи больше не заводится сама" \
            "снимать свой участок юзер вправе — что стоит на его машине, решает он (WORLD2 0.1)"
    else
        ok "снятого кода $snyatyj_kod в зоне нет — вместе с ошибкой, которая его породила"
    fi
    ostalos="$(grep -l -- "$snyatyj_kod" "$HERE"/*.md "$HERE"/*.sh 2>/dev/null || true)"
    if [ -z "$ostalos" ]; then
        ok "и в текстах зоны его не обещают"
    else
        bad "текст зоны обещает снятый код: $(printf '%s' "$ostalos" | tr '\n' ' ')" \
            "человек будет искать в журнале слово, которого там не будет никогда" \
            "control/README.md — ГРАМОТА зоны, её правит architect: правка названа в отчёте owner-control" \
            "сторож гаснет сам, как только код уйдёт из текста"
    fi

    # Снятый код обязан быть снят и у нас: подсказка, обещающая его, уже проверена выше, но
    # и в примерах кода ему не место. `no-image` — ровно тот случай.
    if printf '%s\n' "$soseda" | grep -qx no-image; then
        bad "no-image у соседа" 'он снят решением WORLD2-150 — а в deploy/remote.sh снова есть'
    else
        ok "снятого кода no-image у соседа нет — и подсказки зоны его больше не обещают"
    fi

    # ГОНИМ ВЕСЬ ПЕРЕЧЕНЬ, а не выбранные коды. Схлопни контроллер два кода в один — красным
    # станет ровно тот, который схлопнули; заведи сосед новый — он проверится сам.
    share_stop
    share_start "$(lichnost егор vps:world@10.8.0.5)" || { bad "раздача" "не поднялась"; return; }
    start_server || { bad "подъём контроллера" "процесс не поднялся"; return; }
    вход

    local vsego=0 razoshlos=""
    for kod in $soseda; do
        printf '%s' "$kod" > "$TMP/refuse"
        call POST /api/resources "{\"name\":\"kod$vsego\",\"addr\":\"world@10.8.0.7\",\"creds\":{\"kind\":\"key\",\"value\":\"-----ключ-----\"}}"
        vsego=$((vsego + 1))
        case "$BODY" in
            *"\"code\":\"$kod\""*) ;;
            *) razoshlos="$razoshlos $kod" ;;
        esac
        case "$BODY" in
            *'"from":"deploy/remote.sh"'*) ;;
            *) razoshlos="$razoshlos $kod(без from)" ;;
        esac
    done
    rm -f "$TMP/refuse"
    if [ -z "$razoshlos" ]; then
        ok "все $vsego кодов соседа доехали до пульта ДОСЛОВНО и с указанием, чей это отказ"
    else
        bad "коды соседа доезжают не дословно:$razoshlos" \
            "разъехавшийся словарь означает отказ, который пульт не умеет назвать" \
            "последний ответ:" "$BODY"
    fi
}
...
entry_green() {
    ...
    # └─────────────────────────────────────────────────────────────────────────────────┘
    part "ЗЕЛЁНЫЙ — точка входа называет, чем поднят контроллер, и НЕ обещает вещи той же сборки"
    if entry_live STUB_DOOR_HERE=1 WORLD_VERSION=sha-abc1234; then
        ...
        # ГЛАВНОЕ ЗДЕСЬ — ЧЕГО В ВЫВОДЕ БОЛЬШЕ НЕТ. Обещание «дверь и раздача встанут ТОЙ ЖЕ
        # сборки» стало ложью, когда публикация стала раздельной (`WORLD2-146`): вывод,
        # обещающий несуществующее, мы уже чинили один раз (`WORLD2-129`).
        entry_not_said "ТОЙ ЖЕ сборки" "обещания «вещи той же сборки» в выводе не осталось"
        entry_not_said "своей сборкой" "и второй его формы тоже нет"
        # Образ двери проверяется ТЕМ ЖЕ именем, каким его повезёт подъём. Теперь это имя
        # рецепта КАК ЕСТЬ: штамп контроллера в него не приписывается ни одним путём.
        entry_said "образ двери $ENTRY_DOOR_IMAGE на этой машине есть" "образ двери проверен именем ИЗ РЕЦЕПТА"
        entry_not_said "world:sha-abc1234" "версия контроллера в имя образа вещи не приписана"
    else
    ...
    if entry_live STUB_DOOR_HERE=1; then
        ...
        entry_said "версия ВЕЩИ контроллера не касается" "и сказано, что на вещи это не влияет вовсе"
    else
    ...
    # ┌─────────────────────────────────────────────────────────────────────────────────┐
    # │ ОБРАЗ РАЗДАЧИ — ТРИ СОСТОЯНИЯ, И ОНИ РАЗНЫЕ (`WORLD2-145`).                       │
    # │                                                                                  │
    # │ Проверка была МЕРТВА С РОЖДЕНИЯ: рецепт объявляет пароль обязательной переменной,  │
    # │ точка входа спрашивала имя образа без неё, компоуз отказывался читать файл целиком, │
    # │ а стдерр был заглушён — наружу выходила пустая строка. Обе ветки не печатались ни  │
    # │ на какой машине. Пришло это к нам дважды и оба раза выглядело оговоркой.            │
    # │                                                                                  │
    # │ Поэтому здесь проверяется не только «печатается», но и ЧЕМ спрошено: пароль в       │
    # │ вопросе обязан быть непустым, иначе печататься снова будет не то.                   │
    # └─────────────────────────────────────────────────────────────────────────────────┘
    part "ЗЕЛЁНЫЙ — образ раздачи: есть · нет · рецепт не читается"
    if entry_live STUB_DOOR_HERE=1; then
        entry_said "образ раздачи $ENTRY_SHARE_IMAGE на этой машине есть" "образ раздачи ЕСТЬ — и это напечатано"
        entry_called "compose config -f $ENTRY_STATE/share/compose.yaml" \
            "имя образа раздачи спрошено У ЕЁ рецепта, отдельным вопросом"
        # ОБЯЗАТЕЛЬНЫЕ ПЕРЕМЕННЫЕ РЕЦЕПТА НАЗВАНЫ, И НАЗВАНЫ НЕПУСТЫМИ (`WORLD2-145`).
        # Проверяется ЖУРНАЛ ВЫЗОВОВ, а не текст точки входа: текстовая проверка зеленела на
        # подсказке в комментарии — то есть мерила не то. Перечень имён — из файла соседа.
        if [ -z "$ENTRY_OBYAZATELNYE" ]; then
            skip "обязательные переменные рецепта раздачи" \
                "рецепта соседа рядом нет либо обязательных переменных в нём не объявлено" \
                "прогон из репозитория: перечень читается из share/compose.yaml"
        else
            nedano=""
            for peremennaya in $ENTRY_OBYAZATELNYE; do
                grep -q "share/compose.yaml.* $peremennaya=..*" "$ENTRY_CALLS" || nedano="$nedano $peremennaya"
            done
            if [ -z "$nedano" ]; then
                ok "и спрошено с непустыми обязательными переменными рецепта ($ENTRY_OBYAZATELNYE)"
            else
                bad "обязательные переменные в вопросе" \
                    "рецепт раздачи спрошен без:$nedano — компоуз не прочитает его целиком, и проверка образа снова мертва" \
                    "$(grep share/compose "$ENTRY_CALLS")"
            fi
        fi
    else
        bad "образ раздачи есть" "точка входа не довела подъём до конца:" "$(tail -n 12 "$ENTRY_OUT")"
    fi
    entry_stop

    # Образа раздачи на машине НЕТ — предупреждение с кодом отказа и командой.
    if entry_live; then
        want_no_entry_code "без образа раздачи контроллер всё равно поднимается"
        entry_said "образа раздачи $ENTRY_SHARE_IMAGE на этой машине нет" "образа НЕТ — и это напечатано под правильным именем"
        entry_said "docker pull $ENTRY_SHARE_IMAGE" "и чем это чинится, командой"
    else
        bad "образа раздачи нет" "точка входа не довела подъём до конца:" "$(tail -n 12 "$ENTRY_OUT")"
    fi
    entry_stop

    # РЕЦЕПТ НЕ ЧИТАЕТСЯ — и это ТРЕТИЙ ответ, отличный от обоих. Здесь же проверяется, что
    # причину компоуза мы довезли: без неё человеку нечего чинить.
    if entry_live STUB_DOOR_HERE=1 STUB_SHARE_BROKEN=1; then
        want_no_entry_code "нечитаемый рецепт раздачи не мешает подъёму контроллера"
        entry_said "рецепт раздачи $ENTRY_STATE/share/compose.yaml НЕ ЧИТАЕТСЯ" "сказано, что не читается ФАЙЛ"
        entry_said "required variable SHARE_PASSWORD is missing" "причина компоуза довезена как есть — есть что чинить"
        entry_not_said "образа раздачи $ENTRY_SHARE_IMAGE на этой машине нет" "и это НЕ выдано за «образа нет»"
    else
        bad "рецепт раздачи не читается" "точка входа не довела подъём до конца:" "$(tail -n 12 "$ENTRY_OUT")"
    fi
    entry_stop

    # Тот же класс у ДВЕРИ: рецепт двери не читается — ответ обязан отличаться от «образа
    # нет». Дефект был найден на раздаче, но он не её свойство, а свойство вопроса.
    if entry_live STUB_DOOR_HERE=1 STUB_DOOR_BROKEN=1; then
        entry_said "рецепт двери $ENTRY_STATE/deploy/compose.yaml НЕ ЧИТАЕТСЯ" "у двери различение то же — класс, а не случай"
        entry_said "mapping values are not allowed" "и причина компоуза довезена и здесь"
    else
        bad "рецепт двери не читается" "точка входа не довела подъём до конца:" "$(tail -n 12 "$ENTRY_OUT")"
    fi
    entry_stop

    part "ЗЕЛЁНЫЙ — чего изнутри не хватает: сказано, а не спрятано"
    # ┌─────────────────────────────────────────────────────────────────────────────────┐
    # │ ОБРАЗА ДВЕРИ НА МАШИНЕ НЕТ — И ЭТО БОЛЬШЕ НЕ НЕХВАТКА (`WORLD2-150` B2).          │
    # │                                                                                  │
    # │ Путь доставки выбирает НАЛИЧИЕ, а не человек: есть образ здесь — едет копией по   │
    # │ ssh, нет — та машина тянет его из реестра сама. Отказа `no-image` у соседа больше  │
    # │ нет вовсе, и подсказка, обещавшая его, обещала несуществующее.                     │
    # │                                                                                  │
    # │ Молчать при этом нельзя: пути разные ПО ВРЕМЕНИ, и человек вправе знать, какой     │
    # │ его ждёт. Поэтому проверяется не «сказано ли, что чего-то нет», а НАЗВАН ЛИ ПУТЬ.  │
    # └─────────────────────────────────────────────────────────────────────────────────┘
    if entry_live; then
        want_no_entry_code "без образа двери контроллер всё равно поднимается"
        entry_said "образа двери $ENTRY_DOOR_IMAGE на этой машине нет" "названо, чего нет — и под ПРАВИЛЬНЫМ именем"
        entry_said "image-pull" "названо, КАКИМ ПУТЁМ пойдёт доставка, когда образа здесь нет"
        entry_not_said "no-image" "отказа, которого у соседа больше нет, подсказка не обещает"
    else
        bad "подъём без образа двери" "точка входа не довела подъём до конца:" "$(tail -n 12 "$ENTRY_OUT")"
    fi
    entry_stop

    # Обратная сторона той же развилки: образ здесь ЕСТЬ — значит поедет копией по ssh, и
    # это тоже путь, а не молчание. Без этой половины проверялся бы один рукав из двух.
    if entry_live STUB_DOOR_HERE=1; then
        entry_said "image-copy" "названо, КАКИМ ПУТЁМ пойдёт доставка, когда образ здесь есть"
    else
        bad "подъём с образом двери" "точка входа не довела подъём до конца:" "$(tail -n 12 "$ENTRY_OUT")"
    fi
    entry_stop

    # Рецепт ПРОЧИТАН, а образа в нём не названо. Своей копии имени в зоне нет намеренно —
    # значит здесь честное «не проверил», а не выдуманное имя. И это НЕ то же самое, что
    # нечитаемый рецепт: там поломан файл, здесь — его содержимое, и чинят их разные правки.
    if entry_live STUB_DOOR_IMAGE= STUB_SHARE_IMAGE= ; then
        want_no_entry_code "рецепт без имени образа не останавливает подъём"
        entry_said "прочитан, а образа в нём не названо" "сказано, что образа в рецепте нет"
        entry_not_said "НЕ ЧИТАЕТСЯ" "и это не выдано за поломанный рецепт"
        entry_not_said "на этой машине нет" "не выдумано, будто образа нет на машине: мы не знаем, чего искать"
    else
        bad "рецепт без имени образа" "точка входа не довела подъём до конца:" "$(tail -n 12 "$ENTRY_OUT")"
    fi
    entry_stop
...
komanda_green() {
    ...
    local nado
    for nado in "--name world-control" "-v world-control-keys:/root/.ssh" \
    ...
    # ┌─────────────────────────────────────────────────────────────────────────────────┐
    # │ ПУБЛИКАЦИЯ В КОМАНДЕ ОБЯЗАНА СОВПАДАТЬ С УМОЛЧАНИЕМ ЗОНЫ (`WORLD2-150` B4).       │
    # │                                                                                  │
    # │ Сторожим не «наружу ли», а СОГЛАСИЕ двух путей: команда — путь юзера, файл         │
    # │ запуска — путь разработчика зоны, и разъехавшись они дают два разных пульта на     │
    # │ одном описании. Сменится решение — сменится умолчание, и сторож пойдёт за ним сам, │
    # │ а не станет второй правдой, которую придётся править отдельно.                     │
    # │                                                                                  │
    # │ Команда лежит в `control/README.md` — ГРАМОТЕ ЗОНЫ, которую пишет architect. Эта   │
    # │ проверка и есть то место, где расхождение обязано быть НАЗВАНО, а не замолчано.    │
    # └─────────────────────────────────────────────────────────────────────────────────┘
    local bind_umolchanie komanda_bind
    bind_umolchanie="$(umolchanie_binda "$HERE/compose.yaml")"
    komanda_bind="$(printf '%s\n' "$komanda" | grep -ohE '\-p [^ ]*:8090:8090' | head -n1 | sed 's/^-p //; s/:8090:8090$//')"
    if [ -z "$komanda_bind" ]; then
        bad "публикация в команде" "в команде из README нет публикации вида -p <где>:8090:8090:" "$komanda"
    elif [ "$komanda_bind" = "$bind_umolchanie" ]; then
        ok "публикация в команде та же, что умолчанием у зоны ($bind_umolchanie)"
    else
        bad "публикация в команде разъехалась с умолчанием зоны" \
            "команда в README публикует на $komanda_bind, а умолчание зоны — $bind_umolchanie" \
            "команда юзера и путь разработчика дают два разных пульта на одном описании" \
            "control/README.md — грамота зоны, её правит architect: правка названа в отчёте owner-control"
    fi
...
# ------------------------------------------------------------------ зелёный: мир уходит и убирает
# ┌─────────────────────────────────────────────────────────────────────────────────────┐
# │ `C1` ПРОВЕРЯЕТСЯ ЖИВЬЁМ И В ТРЁХ РАЗНЫХ МЕСТАХ, потому что это три разных свойства:  │
# │                                                                                      │
# │   в ФАЙЛЕ РАЗДАЧИ    свежий скоуп — личность и пустота: ни территорий, ни ключей;     │
# │   в ФАЙЛЕ МАШИНЫ     после заведения нашей строки в `authorized_keys` нет — мир ушёл; │
# │   по АДРЕСУ СКОУПА   снятая раздача больше не отвечает, а её том лежит на месте.      │
# │                                                                                      │
# │ Ни одно из трёх не проверяется ответом ручки: ответ собирается из памяти, а файл и    │
# │ занятый порт — нет. Ровно на этой подмене и держалась прежняя ошибка: контроллер      │
# │ отчитывался успехом, а строки копились на чужой машине (одиннадцать, живой прогон).   │
# └─────────────────────────────────────────────────────────────────────────────────────┘
skoup_snyat_green() {
    part "ЗЕЛЁНЫЙ — C1: мир поднял раздачу и ушёл; что завёл, то и снимает"
    share_stop
    rm -rf "$SHARE_STATE_DIR"; mkdir -p "$SHARE_STATE_DIR"
    : > "$SHARE_PIDS"
    start_server || { bad "подъём контроллера" "процесс не поднялся"; return; }

    # ── свежий скоуп пуст ────────────────────────────────────────────────────
    zavesti "$SHARE_URL_B" пароль-б vps борис
    want_status 201 "скоуп заведён"
    local fajl; fajl="$(fajl_razdachi || true)"
    if [ -z "$fajl" ] || [ ! -s "$fajl" ]; then
        bad "файл состояния" "раздача не встала или состояние в неё не легло: «$fajl»"
        return
    fi
    local razdel
    for razdel in '"ключи": []' '"территории": []' '"поля": []'; do
        if grep -qF -- "$razdel" "$fajl"; then
            ok "в свежем скоупе раздел $razdel ПУСТ — мир поднял раздачу и ушёл"
        else
            bad "свежий скоуп не пуст: $razdel" \
                "заведение записало в личность то, о чём юзер не просил — из этой записи и выросли «дом скоупа» и ключ на чужой машине" \
                "$(head -c 400 "$fajl")"
        fi
    done

    # ── на машине не осталось нашей строки ───────────────────────────────────
    # ЗАВЕДЕНИЕ ПАРОЛЕМ НА НАСТОЯЩЕЙ ПОДСТАВНОЙ МАШИНЕ: контроллер туда и правда заходит,
    # и правда кладёт строку. Смотрим В ЕЁ ФАЙЛ — ответ ручки сказал бы что угодно.
    if ! [ -x "$TMP/машина" ]; then
        skip "мир уходит с машины после подъёма раздачи" \
            "подставная машина не собралась — заходить некуда" \
            "она лежит в зоне: cd $HERE && go build ./internal/creds/sshtest/cmd"
    elif ! machine_start; then
        bad "подставная машина" "не поднялась на :$MACHINE_PORT"
    else
        call POST /api/scope "{\"scope\":{\"addr\":\"$SHARE_URL_C\",\"password\":\"пароль-ц\"},\"identity\":{\"name\":\"клава\",\"brand\":\"\"},\"machine\":{\"name\":\"vps\",\"addr\":\"world@127.0.0.1:$MACHINE_PORT\",\"creds\":{\"kind\":\"password\",\"value\":\"$MACHINE_PASS\"}}}"
        want_status 201 "скоуп заводится ПАРОЛЕМ на чистой машине"
        if [ -z "$(machine_keys | grep -c . || true)" ] || [ "$(machine_keys | grep -c . || true)" = "0" ]; then
            ok "после заведения в ~/.ssh/authorized_keys машины НАШЕЙ строки нет — мир ушёл"
        else
            bad "мир остался на машине" \
                "строка, положенная паролем, пережила подъём — это доступ, о котором юзер не просил" \
                "$(machine_keys)"
        fi
        local fajl_c; fajl_c="$(fajl_razdachi || true)"
        if [ -n "$fajl_c" ] && grep -qF -- '"ключи": []' "$fajl_c"; then
            ok "ключ, заведённый паролем, в скоуп не уехал — ходить туда мир больше не будет"
        else
            bad "ключ в свежем скоупе" "он там лежит, а мир на эту машину не ходит:" "$(head -c 400 "$fajl_c" 2>/dev/null)"
        fi
        # Цена названа И названа ЦЕЛИКОМ: что положили и что убрали. Половина этой правды
        # оставила бы юзера искать у себя строку, которой нет.
        want_has 'authorized_keys' "цена названа: сказано, что контроллер писал в чужую машину"
        want_has 'убрали' "и сказано, что строку убрали — юзеру не придётся искать её у себя"

        # ┌─────────────────────────────────────────────────────────────────────────────┐
        # │ СНЯТИЕ ПАРОЛЕМ — ТОЖЕ ЗАХОД НА ЧУЖУЮ МАШИНУ, и уходить с неё надо так же.    │
        # │                                                                              │
        # │ Проверять это на снятии ключом бесполезно: там мы ничего не клали, и «строк   │
        # │ нет» вышло бы само. Порча «снять раздачу, но не тронуть ключ» рядом с такой    │
        # │ проверкой проходит МОЛЧА — поэтому здесь заход паролем, и смотрим в файл       │
        # │ машины ПОСЛЕ снятия.                                                          │
        # └─────────────────────────────────────────────────────────────────────────────┘
        call DELETE /api/scope "{\"scope\":{\"addr\":\"$SHARE_URL_C\",\"password\":\"пароль-ц\"},\"machine\":{\"name\":\"vps\",\"addr\":\"world@127.0.0.1:$MACHINE_PORT\",\"creds\":{\"kind\":\"password\",\"value\":\"$MACHINE_PASS\"}}}"
        want_status 200 "скоуп, заведённый паролем, снимается тем же паролем"
        if [ "$(machine_keys | grep -c . || true)" = "0" ]; then
            ok "после снятия в ~/.ssh/authorized_keys машины не осталось наших строк"
        else
            bad "строка пережила снятие" \
                "мир снял раздачу и оставил на чужой машине доступ — убрать его через мир больше нечем" \
                "$(machine_keys)"
        fi

        # ┌─────────────────────────────────────────────────────────────────────────────┐
        # │ «В ЛЮБОМ ИСХОДЕ» — ЗНАЧИТ И В НЕУДАЧНОМ. Находка ревью `WORLD2-152`.         │
        # │                                                                              │
        # │ Строку с чужой машины убирают ДВА разных куска кода: удачный путь и           │
        # │ отложенная уборка. Проверка стояла только на первом — и порча «перевернуть    │
        # │ условие в отложенной уборке» проходила МОЛЧА: снятие отказывало, а доступ,     │
        # │ выданный ради него, оставался у юзера навсегда. Ровно тот класс, что мы ловим: │
        # │ сторож стоит там, где заявлено свойство, а не там, где оно ломается.           │
        # │                                                                              │
        # │ Ломаем НАСТОЯЩИМ отказом подъёма (не «мимо машины»): контроллер уже зашёл      │
        # │ паролем и уже положил строку, и только потом снятие не состоялось.             │
        # └─────────────────────────────────────────────────────────────────────────────┘
        # Та же клятва у ЗАВЕДЕНИЯ, и стережётся она здесь же: обещание одно на две ручки,
        # а ломаются они порознь. Подъём отказывает ПОСЛЕ того, как строка уже легла.
        printf 'no-remote-docker' > "$TMP/refuse"
        call POST /api/scope "{\"scope\":{\"addr\":\"$SHARE_URL_D\",\"password\":\"пароль-д\"},\"identity\":{\"name\":\"даша\",\"brand\":\"\"},\"machine\":{\"name\":\"vps\",\"addr\":\"world@127.0.0.1:$MACHINE_PORT\",\"creds\":{\"kind\":\"password\",\"value\":\"$MACHINE_PASS\"}}}"
        rm -f "$TMP/refuse"
        want_refusal no-remote-docker "заведение не состоялось — отказ подъёма доехал своим кодом"
        if [ "$(machine_keys | grep -c . || true)" = "0" ]; then
            ok "заведение ОТКАЗАЛО, а строка с машины всё равно ушла"
        else
            bad "строка пережила НЕУДАЧНОЕ заведение" \
                "скоуп не записан — значит приватной половины у юзера нет, и убрать эту строку ему нечем" \
                "$(machine_keys)"
        fi

        call POST /api/scope "{\"scope\":{\"addr\":\"$SHARE_URL_D\",\"password\":\"пароль-д\"},\"identity\":{\"name\":\"даша\",\"brand\":\"\"},\"machine\":{\"name\":\"vps\",\"addr\":\"world@127.0.0.1:$MACHINE_PORT\",\"creds\":{\"kind\":\"password\",\"value\":\"$MACHINE_PASS\"}}}"
        want_status 201 "второй скоуп на той же машине заведён — есть что снимать неудачно"
        printf 'down-failed' > "$TMP/refuse"
        call DELETE /api/scope "{\"scope\":{\"addr\":\"$SHARE_URL_D\",\"password\":\"пароль-д\"},\"machine\":{\"name\":\"vps\",\"addr\":\"world@127.0.0.1:$MACHINE_PORT\",\"creds\":{\"kind\":\"password\",\"value\":\"$MACHINE_PASS\"}}}"
        rm -f "$TMP/refuse"
        # Код взят НЕ любой: `no-such-resource` подъём и снятие толкуют особо — участок не
        # запирается навсегда из-за вещи, которой там уже нет. Нам нужен отказ, на котором
        # снятие ПРАВДА не состоялось, иначе ломали бы не тот путь.
        want_refusal down-failed "снятие не состоялось — отказ соседа доехал своим кодом"
        if [ "$(machine_keys | grep -c . || true)" = "0" ]; then
            ok "снятие ОТКАЗАЛО, а строка с машины всё равно ушла — отказ не оставляет за собой"
        else
            bad "строка пережила НЕУДАЧНОЕ снятие" \
                "снять раздачу не вышло, а доступ на чужой машине остался — и убрать его через мир больше нечем" \
                "(WORLD2 2.3 п. 5: отказ не вправе ничего оставлять за собой)" \
                "$(machine_keys)"
        fi
        machine_stop
    fi
    ...
    # ┌─────────────────────────────────────────────────────────────────────────────────┐
    # │ ЧУЖОЕ ИМЯ УЧАСТКА — ЭТО ОТКАЗ, А НЕ «СНЯТО». Находка живого прогона.              │
    # │                                                                                  │
    # │ Имя раздачи собрано из имени участка, а называет его ЮЗЕР — и назвать может       │
    # │ другое. Компоуз честно снимает проект, которого на машине нет, и выходит нулём;   │
    # │ мир отвечал «снято», а раздача жила дальше.                                       │
    # │                                                                                  │
    # │ Проверка стоит на АДРЕСЕ, а не на журнале вызовов: журнал сказал бы «снятие       │
    # │ позвано» и здесь. Соседний скоуп при этом обязан пережить чужое снятие — это      │
    # │ второе свойство, и оно проверялось раньше ОДНО, без первого.                       │
    # └─────────────────────────────────────────────────────────────────────────────────┘
    : > "$CALLS"
    snyat "$SHARE_URL_B" пароль-б chuzhoe
    want_refusal share-alive "снятие чужим именем участка ОТКАЗЫВАЕТ, а не отчитывается успехом"
    if (exec 3<>/dev/tcp/127.0.0.1/"$SHARE_PORT_B") 2>/dev/null; then
        ok "раздача по адресу жива — отказ сказал правду"
    else
        bad "раздача после чужого имени" "она упала, и проверяется тогда не тот случай"
    fi
    case "$BODY" in
        *chuzhoe-*) ok "названо, ЧТО снималось на машине — словом соседа, а не нашей догадкой" ;;
        *) bad "что снимали" "в отказе не названа вещь, которую снимали:" "$BODY" ;;
    esac

    # ── снятие по адресу ─────────────────────────────────────────────────────
    : > "$CALLS"
    snyat "$SHARE_URL_B" пароль-б vps
    want_status 200 "скоуп снимается по адресу — симметрично заведению"
    # Раздача снята ЖИВЬЁМ: по адресу больше никто не отвечает. Журнал вызовов сказал бы
    # «снятие позвано» и там, где оно позвано не про ту раздачу.
    if (exec 3<>/dev/tcp/127.0.0.1/"$SHARE_PORT_B") 2>/dev/null; then
        bad "раздача после снятия" "по адресу $SHARE_URL_B всё ещё кто-то отвечает"
    else
        ok "по адресу снятого скоупа больше никто не отвечает"
    fi
    if grep -q -- "remote.sh drop vps --recipe $SHARE_RECIPE" "$CALLS"; then
        ok "раздача снята ТЕМ ЖЕ рецептом, которым поднята — своего снятия зона не пишет"
    else
        bad "снятие рецептом" "в журнале вызовов нет снятия раздачи рецептом:" "$(cat "$CALLS")"
    fi
    # СНИМАЕМ ТУ ЖЕ РАЗДАЧУ, ЧТО ПОДНИМАЛИ: имя проекта, контейнера и тома рецепт собирает
    # из `SHARE_NAME`. Уйди оно — компоуз снял бы раздачу ПО УМОЛЧАНИЮ, то есть соседнюю.
    local snyato_imya
    snyato_imya="$(grep -A1 'remote.sh drop' "$CALLS" | grep -o 'SHARE_NAME=[^ ]*' | tail -n1 | cut -d= -f2)"
    if [ -n "$snyato_imya" ] && [ "$SHARE_STATE_DIR/$snyato_imya.json" = "$fajl" ]; then
        ok "снята ТА ЖЕ раздача, что поднимали ($snyato_imya) — а не раздача по умолчанию"
    else
        bad "снятие назвало не ту раздачу" "поднимали «$(basename "$fajl" .json)», снимаем «$snyato_imya»" "$(tail -4 "$CALLS")"
    fi
    # АДРЕС МАШИНЫ СНЯТИЕ БЕРЁТ ИЗ КОНТЕКСТА (`deploy/remote.sh`, cmd_drop: `ctx_exists ||
    # refuse no-such-resource`), а территории у скоупа нет — значит контекст обязан положить
    # контроллер, и положить ДО вызова.
    local ctx_stroka drop_stroka
    ctx_stroka="$(grep -n 'docker context \(create\|update\) world-vps' "$CALLS" | head -n1 | cut -d: -f1)"
    drop_stroka="$(grep -n 'remote.sh drop' "$CALLS" | head -n1 | cut -d: -f1)"
    if [ -n "$ctx_stroka" ] && [ -n "$drop_stroka" ] && [ "$ctx_stroka" -lt "$drop_stroka" ]; then
        ok "контекст докера заведён ДО снятия — сосед берёт адрес машины из него"
    else
        bad "контекст перед снятием" "его нет или он позже вызова: сосед откажет «такого ресурса не заводили»" "$(cat "$CALLS")"
    fi
    # ТОМ ОСТАЛСЯ, и это видно по файлу, а не по словам.
    if [ -s "$fajl" ]; then
        ok "том раздачи пережил снятие — личность лежит на месте"
    else
        bad "том стёрт без ключа" "личность стёрта молча — снятие вещи не означает потерю личности (`WORLD2` 1.9)"
    fi
    # ЧЕМ СТЕРЕТЬ — сказано ВЫХОДОМ, и выход зовёт в ЭТУ ручку, а не в соседнюю. Смотрим
    # в сами выходы, а не «где-нибудь в ответе»: та же команда, сказанная вторым голосом в
    # `note`, зеленила бы эту проверку при пустых выходах.
    case "$(vyhody)" in
        *'DELETE /api/scope?with-state=1'*) ok "сказано вслух, чем стереть оставшийся том, — и командой этой же ручки" ;;
        *) bad "чем стереть том" "в выходах снятия нет команды стирания:" "$(vyhody)" "$BODY" ;;
    esac
    # А ЧТО ИМЕННО ОСТАЛОСЬ — сказано СВОИМИ СЛОВАМИ: в томе раздачи лежит ЛИЧНОСТЬ, и
    # человеку важно это, а не «состояние вещи» из общего перечня.
    case "$(pole note)" in
        *личност*) ok "сказано, ЧТО осталось: в томе лежит личность, а не «состояние вещи»" ;;
        *) bad "что осталось" "сказанное вслух не называет личности:" "$(pole note)" ;;
    esac
    # И КОМАНДА СТИРАНИЯ СКАЗАНА ОДИН РАЗ. Две правды об одном разъезжаются молча, а сторож
    # на присутствие проходит зелёным, пока жива хоть одна копия (своя грабля `WORLD2-150`).
    case "$(pole note)" in
        *with-state=1*) bad "команда стирания сказана дважды" "она уже названа выходом — копия разъедется с ним молча:" "$(pole note)" ;;
        *) ok "команда стирания сказана ОДИН раз — выходом, а не заодно и словами" ;;
    esac
    # Времянки контроллера ушли: ключ и блок в config.
    if [ ! -f "$KEYS/world-vps" ]; then
        ok "ключ снятого скоупа не пережил его в связке контроллера"
    else
        bad "след после снятия" "ключ остался в связке: $(ls "$KEYS" | tr '\n' ' ')"
    fi

    # ── повторное снятие: внятный отказ, а не падение ────────────────────────
    snyat "$SHARE_URL_B" пароль-б vps
    want_refusal no-share "повторное снятие по тому же адресу отказывает внятно"

    # ── том стирается только явным ключом ────────────────────────────────────
    # Поднимаем скоуп заново НА ТОМ ЖЕ ТОМЕ. Раздача отдаёт прежнюю личность — и заведение
    # обязано отказать, а не стереть её: иначе обещание «том остался» было бы ложью через
    # один вызов, и стёрлось бы молча.
    zavesti "$SHARE_URL_B" пароль-б vps борис
    want_refusal scope-exists "заведение на уцелевшем томе не пишет поверх найденной личности"
    if [ -s "$fajl" ] && grep -q '"имя": *"борис"' "$fajl"; then
        ok "личность в томе цела — том и правда переживает снятие"
    else
        bad "личность после переподъёма" "в томе не она:" "$(head -c 200 "$fajl" 2>/dev/null)"
    fi
    snyat "$SHARE_URL_B" пароль-б vps "?with-state=1"
    want_status 200 "скоуп снимается вместе с томом по явному ключу"
    if [ ! -s "$fajl" ]; then
        ok "том стёрт — и стёрт только тогда, когда об этом попросили явно"
    else
        bad "with-state не сработал" "том на месте, а его просили стереть: $fajl"
    fi
    # Смотрим в СКАЗАННОЕ, а не в весь ответ: слово «стёрто» есть и в перечне снятого («тома
    # рецепта — состояние вещи стёрто»), и проверка на присутствие зеленела бы на нём —
    # то есть мерила бы не то, о чём заявлено свойство.
    case "$(pole note)" in
        *стёрт*личност*|*личност*стёрт*) ok "стирание ЛИЧНОСТИ названо вслух, а не молча сделано" ;;
        *) bad "стирание личности" "сказанное вслух не называет, что стёрта именно личность:" "$(pole note)" ;;
    esac
...
# ------------------------------------------------------------------ зелёный: два скоупа
# ┌─────────────────────────────────────────────────────────────────────────────────────┐
# │ `B3` ПРОВЕРЯЕТСЯ ЖИВЬЁМ, А НЕ ПО ЖУРНАЛУ ВЫЗОВОВ.                                    │
# │                                                                                      │
# │ Журнал сказал бы «имя названо» и там, где обоим скоупам названо ОДНО имя. Поэтому    │
# │ подставной подъём поднимает раздачу ТАМ и ПОД ТЕМ ИМЕНЕМ, что назвал контроллер, а    │
# │ состояние держит в файле по ИМЕНИ — как настоящий рецепт держит его в томе по         │
# │ `SHARE_NAME`. Тогда:                                                                  │
# │                                                                                      │
# │   зашили имя  → обе личности лягут в ОДИН файл, и вторая перезапишет первую;          │
# │   зашили порт → вторая раздача не встанет по названному адресу, и заведение упрётся   │
# │                 в отказ связи.                                                        │
# │                                                                                      │
# │ Ни то, ни другое проверкой на присутствие не ловится — а это ровно то, что рядом с    │
# │ порчей проходило бы молча.                                                            │
# └─────────────────────────────────────────────────────────────────────────────────────┘
mnogo_skoupov_green() {
    part "ЗЕЛЁНЫЙ — B3: скоупов на одной машине сколько угодно, и они не видят друг друга"
    share_stop
    rm -rf "$SHARE_STATE_DIR"; mkdir -p "$SHARE_STATE_DIR"
    : > "$SHARE_PIDS"
    start_server || { bad "подъём контроллера" "процесс не поднялся"; return; }

    # ОДНА машина, ОДНО имя участка, разные адреса скоупов. Имя участка живёт в скоупе, и у
    # двух разных личностей оно законно совпадает: разводить их обязан не юзер.
    zavesti "$SHARE_URL_B" пароль-б vps борис
    want_status 201 "первый скоуп на машине заведён"
    local fajl_b; fajl_b="$(fajl_razdachi || true)"

    zavesti "$SHARE_URL_C" пароль-ц vps клава
    want_status 201 "второй скоуп на ТОЙ ЖЕ машине заведён — машина не единица личности"
    local fajl_c; fajl_c="$(fajl_razdachi || true)"

    if [ -n "$fajl_b" ] && [ -n "$fajl_c" ] && [ "$fajl_b" != "$fajl_c" ]; then
        ok "у каждой раздачи СВОЁ имя — значит свой контейнер, свой проект и свой том"
    else
        bad "имена двух раздач" "они не разведены: «$fajl_b» и «$fajl_c»" "$(grep SHARE_NAME= "$CALLS" | tail -4)"
        return
    fi

    # ОБЕ ЖИВЫ ОДНОВРЕМЕННО, и каждая отвечает по своему адресу — тому, который назвал юзер.
    local zhivy=1
    (exec 3<>/dev/tcp/127.0.0.1/"$SHARE_PORT_B") 2>/dev/null || zhivy=
    (exec 3<>/dev/tcp/127.0.0.1/"$SHARE_PORT_C") 2>/dev/null || zhivy=
    [ -n "$zhivy" ] && ok "обе раздачи стоят ОДНОВРЕМЕННО, каждая по своему адресу" \
                    || bad "две раздачи разом" "одна из них не отвечает — вторая села поверх первой"

    # ЗАПИСЬ В ОДНУ НЕ ВИДНА В ДРУГОЙ. Смотрим в ФАЙЛЫ, а не в ответы: ответ можно собрать
    # из памяти, файл — нельзя.
    if grep -q '"имя": *"борис"' "$fajl_b" 2>/dev/null && grep -q '"имя": *"клава"' "$fajl_c" 2>/dev/null; then
        ok "у каждой раздачи своё состояние — запись в одну не видна в другой"
    else
        bad "состояния двух скоупов" "они смешались либо не легли:" \
            "$(head -c 120 "$fajl_b" 2>/dev/null)" "$(head -c 120 "$fajl_c" 2>/dev/null)"
    fi

    # ПОРТ — ТОТ, ЧТО НАЗВАЛ ЮЗЕР. Проверяем не «порт назван», а что названы РАЗНЫЕ и
    # именно те: зашитое умолчание дало бы одинаковые.
    local porty; porty="$(grep -o 'SHARE_PORT=[0-9]*' "$CALLS" | awk -F= '{print $2}' | sort -u | tr '\n' ' ')"
    case " $porty " in
        *" $SHARE_PORT_B "*) case " $porty " in
                                 *" $SHARE_PORT_C "*) ok "порт каждой раздачи взят из АДРЕСА её скоупа ($porty)" ;;
                                 *) bad "порт второй раздачи" "названо: $porty, а юзер сказал $SHARE_PORT_C" ;;
                             esac ;;
        *) bad "порт первой раздачи" "названо: $porty, а юзер сказал $SHARE_PORT_B" ;;
    esac

    # ┌─────────────────────────────────────────────────────────────────────────────────┐
    # │ СНЯТИЕ ОДНОЙ НЕ ТРОГАЕТ ВТОРУЮ. Единственный путь, которым контроллер снимает      │
    # │ раздачу, — уборка за собой, когда состояние лечь не смогло. Значит и проверять его │
    # │ надо им: поднимаем третий скоуп так, что раздача не встаёт, и смотрим, ЧТО он снял.│
    # │                                                                                  │
    # │ Позови он снятие без `SHARE_NAME` — компоуз взял бы имя по умолчанию, и снялась бы │
    # │ ЧУЖАЯ раздача. Здесь это видно прямо: соседние обязаны пережить уборку.            │
    # └─────────────────────────────────────────────────────────────────────────────────┘
    stop_server
    SRV_NO_RISE=1 start_server || { bad "подъём контроллера" "процесс не поднялся"; SRV_NO_RISE=""; return; }
    SRV_NO_RISE=""
    zavesti "$SHARE_URL_D" пароль-д vps даша
    case "$STATUS" in
        2*) bad "уборка за собой" "запись в невставшую раздачу почему-то прошла: $BODY" ;;
        *)  ok "состояние лечь не смогло — отказ, а не молчание" ;;
    esac
    local snyato podnyato
    podnyato="$(grep -o 'SHARE_NAME=[^ ]*' "$CALLS" | tail -n1 | cut -d= -f2)"
    snyato="$(grep -B1 'remote.sh drop' "$CALLS" >/dev/null 2>&1; grep -A1 'remote.sh drop' "$CALLS" | grep -o 'SHARE_NAME=[^ ]*' | tail -n1 | cut -d= -f2)"
    if [ -n "$snyato" ] && [ "$snyato" = "$podnyato" ]; then
        ok "убрано ТО ЖЕ, что поднимали ($snyato) — а не раздача по умолчанию"
    else
        bad "уборка назвала не ту раздачу" "поднимали «$podnyato», снимаем «$snyato»" "$(tail -6 "$CALLS")"
    fi
    zhivy=1
    (exec 3<>/dev/tcp/127.0.0.1/"$SHARE_PORT_B") 2>/dev/null || zhivy=
    (exec 3<>/dev/tcp/127.0.0.1/"$SHARE_PORT_C") 2>/dev/null || zhivy=
    [ -n "$zhivy" ] && ok "соседние раздачи уборку пережили — снятие одной не трогает вторую" \
                    || bad "уборка снесла соседа" "раздача другого скоупа на той же машине не отвечает"
...
# ------------------------------------------------------------------ зелёный: метка пути
# ┌─────────────────────────────────────────────────────────────────────────────────────┐
# │ `B2`: КАКИМ ПУТЁМ ПОШЛИ, ГОВОРИТ КОНТРОЛЛЕР — И ГОВОРИТ ПОКА ШАГ ИДЁТ.                │
# │                                                                                      │
# │ Проверка НЕ на присутствие метки в ответе: она попала бы туда и разобранная после     │
# │ выхода подъёма, то есть после того, как человек уже дождался. Здесь подставной подъём │
# │ печатает метку и ВСТАЁТ У ВОРОТ: пока проба не увидит путь в пульте и не откроет их,   │
# │ шаг не кончится. Разбирай контроллер вывод после `Wait` — ворота никто не откроет,     │
# │ подъём досидит до своего предела, и заведение упрётся в красноту.                      │
# └─────────────────────────────────────────────────────────────────────────────────────┘
put_green() {
    part "ЗЕЛЁНЫЙ — B2: путь доставки доезжает до пульта ДО того, как занял минуты"
    share_stop
    rm -rf "$SHARE_STATE_DIR"; mkdir -p "$SHARE_STATE_DIR"
    : > "$SHARE_PIDS"; rm -f "$PATH_GATE"

    SRV_PATH_SAY=image-copy SRV_PATH_GATE="$PATH_GATE" start_server \
        || { bad "подъём контроллера" "процесс не поднялся"; SRV_PATH_SAY=""; SRV_PATH_GATE=""; return; }
    SRV_PATH_SAY=""; SRV_PATH_GATE=""

    # Заведение уходит в фон: пока оно идёт, мы спрашиваем ход. Так же ведёт себя пульт —
    # он послал запрос и ждёт ответа, а показать человеку обязан уже сейчас.
    ( zavesti "$SHARE_URL_B" пароль-б vps борис > /dev/null 2>&1 ) &
    local zaved=$!

    # УВИДЕТЬ ПУТЬ МАЛО — надо увидеть его, ПОКА ДЕЙСТВИЕ ИДЁТ. Поэтому условие двойное:
    # путь назван И ход говорит «идёт». Метка, показавшаяся после конца шага, — это отчёт, а
    # не предупреждение, и засчитывать её значило бы засчитывать ровно ту порчу, которую
    # сторож заведён ловить.
    local n=0 vidno=""
    while [ "$n" -lt 100 ]; do
        raw_get /api/progress
        case "$BODY" in
            *'"path":"image-copy"'*) case "$BODY" in *'"busy":true'*) vidno=1; break ;; esac ;;
        esac
        kill -0 "$zaved" 2>/dev/null || break
        sleep 0.1; n=$((n + 1))
    done

    if [ -n "$vidno" ]; then
        ok "пульт увидел путь image-copy ПОКА ШАГ ЕЩЁ ШЁЛ — а не после него"
        case "$BODY" in
            *'"busy":true'*) ok "ход сказал, что действие ИДЁТ, а не кончилось" ;;
            *) bad "ход" "путь назван, а «идёт» не сказано: $BODY" ;;
        esac
        case "$BODY" in
            *'"name":"vps"'*) ok "ход назвал участок — тем словом, которое придумал юзер" ;;
            *) bad "ход без участка" "человеку не сказано, чего именно он ждёт: $BODY" ;;
        esac
        case "$BODY" in
            *10.8.0.5*|*пароль-б*) bad "ход выдал лишнее" "в ходе оказался адрес машины либо пароль: $BODY" ;;
            *) ok "ни адреса машины, ни пароля в ходе нет — читается он без входа" ;;
        esac
    else
        bad "путь ДО шага" "пульт не увидел метки, пока подъём ещё работал" \
            "значит она разбирается после выхода процесса — то есть после того, как человек дождался" \
            "$BODY"
    fi

    # Отпускаем подъём и дожидаемся конца заведения — иначе следующий раздел упрётся в
    # занятый порт и покраснеет не про то.
    : > "$PATH_GATE"
    wait "$zaved" 2>/dev/null || true
    rm -f "$PATH_GATE"

    # Кончилось — «идёт» гаснет, а шаги остаются: спрашивают ход не непрерывно, и шаг,
    # прошедший за секунду, иначе исчезал бы между двумя вопросами.
    raw_get /api/progress
    case "$BODY" in
        *'"busy":false'*) ok "действие кончилось — ход это назвал, а не завис в «идёт»" ;;
        *) bad "ход после конца" "он всё ещё «идёт»: $BODY" ;;
    esac
    case "$BODY" in
        *'"path":"image-copy"'*) ok "пройденный путь остался в ходе — его можно дочитать" ;;
        *) bad "шаги после конца" "они стёрлись вместе с действием: $BODY" ;;
    esac
...
# ------------------------------------------------------------------ зелёный: пульт наружу
# ┌─────────────────────────────────────────────────────────────────────────────────────┐
# │ `B4`: ПУЛЬТ ПУБЛИКУЕТСЯ НАРУЖУ УМОЛЧАНИЕМ, И ЦЕНА СКАЗАНА ВСЛУХ.                     │
# │                                                                                      │
# │ Стережём УМОЛЧАНИЕ, а не наличие переменной: `CONTROL_BIND` был на месте и когда      │
# │ публикация шла в петлю. Умолчание — это то, что получит человек, пришедший без        │
# │ переменных, и решает за него именно оно.                                              │
# └─────────────────────────────────────────────────────────────────────────────────────┘

# umolchanie_binda ФАЙЛ — значение по умолчанию у `CONTROL_BIND` в этом файле.
umolchanie_binda() {
    grep -ohE 'CONTROL_BIND:-[^}"]*' "$1" 2>/dev/null | head -n1 | sed 's/.*CONTROL_BIND:-//'
}

naruzhu_green() {
    part "ЗЕЛЁНЫЙ — B4: пульт публикуется наружу умолчанием, и цена названа вслух"

    local fajl bind
    for fajl in "$HERE/compose.yaml" "$HERE/up.sh"; do
        bind="$(umolchanie_binda "$fajl")"
        case "$bind" in
            0.0.0.0|::) ok "умолчание публикации в $(basename "$fajl") — наружу ($bind)" ;;
            "")         bad "умолчание публикации в $(basename "$fajl")" "CONTROL_BIND с умолчанием там не нашёлся" ;;
            *)          bad "умолчание публикации в $(basename "$fajl")" \
                            "оно вернулось в петлю ($bind) — до пульта с другой машины можно дойти только обходом" ;;
        esac
    done

    # Умолчание ОДНО на оба пути. Разъедься они — у разработчика зоны и у юзера получились
    # бы два разных пульта, и «умолчанием наружу» перестало бы что-то значить.
    if [ "$(umolchanie_binda "$HERE/compose.yaml")" = "$(umolchanie_binda "$HERE/up.sh")" ]; then
        ok "умолчание публикации одно и то же в файле запуска и в подъёме"
    else
        bad "два умолчания публикации" \
            "compose.yaml: $(umolchanie_binda "$HERE/compose.yaml") · up.sh: $(umolchanie_binda "$HERE/up.sh")"
    fi

    # ЦЕНА — В ВЫВОДЕ ТОЧКИ ВХОДА, а не в комментарии к файлу. Комментарий человек не
    # читает: он читает то, что напечатал ему подъём.
    if [ ! -S "$ENTRY_SOCK" ]; then
        skip "цена публикации в выводе точки входа" "подставной докер-сокет не завёлся"
        return
    fi
    if entry_live STUB_DOOR_HERE=1; then
        entry_said_v_itoge "докер-сокет" "цена названа: у контроллера сокет ЭТОЙ машины"
        # Слово «наружу» в итоговом блоке стоит ДВАЖДЫ и по разным поводам: один раз про
        # адрес, куда открывать пульт, второй — про цену. Ловим поэтому не его, а СОБСТВЕННОЕ
        # утверждение цены: иначе сторож зеленел бы на соседней строке про другое.
        entry_said_v_itoge "заходит любой, кто знает" "сказано, что в пульт заходит любой, кто знает адрес"
        entry_said_v_itoge "где не жалко" "правило «ставь там, где не жалко» произнесено, а не подразумевается"
        entry_said_v_itoge "docker port world-control" "названо, чем проверить, что опубликовано на самом деле"
        # ОДНА КОПИЯ, А НЕ ДВЕ. Пока их было две, снятие любой проходило зелёным — сторож
        # находил вторую. Копия того же обещания в одном выводе не страховка, а слепое пятно.
        local skolko
        skolko="$(grep -cF -- "где не жалко" "$ENTRY_OUT" || true)"
        [ "${skolko:-0}" = 1 ] \
            && ok "цена сказана РОВНО ОДИН раз — второй копии, прячущей потерю первой, нет" \
            || bad "копий цены $skolko" "одна и та же цена сказана дважды: снятие любой из них пройдёт молча"
    else
        bad "подъём с ценой" "точка входа не довела подъём до конца:" "$(tail -n 12 "$ENTRY_OUT")"
    fi
    entry_stop
}
...
up_green() {
    ...
    # ЦЕНА ПУБЛИКАЦИИ — И НА ПУТИ РАЗРАБОТЧИКА ТОЖЕ (`WORLD2-150` B4). Умолчание у обоих
    # путей одно, значит и цена у обоих одна: сказанная в одном месте и промолчанная в
    # другом, она означала бы, что решение касается только юзера.
    want_said "где не жалко" "подъём назвал цену публикации наружу"
    want_said "дошёл до пульта" "и назвал её тем же, чем она названа в точке входа: кто дошёл до пульта — дошёл до машины"
...
red() {
    ...
    # ┌─────────────────────────────────────────────────────────────────────────────────┐
    # │ ОТКАЗЫ СНЯТИЯ НАЗЫВАЮТСЯ СВОИМИ ИМЕНАМИ, а не общим «не получилось»: пароль не    │
    # │ подошёл · машину не назвали · раздачи по адресу нет · до машины не дойти. Чинит   │
    # │ каждый из них разное действие, и общий отказ отправил бы человека чинить наугад.  │
    # │                                                                                  │
    # │ Ни один из них не смеет ничего снять: раздача обязана пережить их все.            │
    # └─────────────────────────────────────────────────────────────────────────────────┘
    : > "$CALLS"
    call DELETE /api/scope "{\"scope\":{\"addr\":\"$SHARE_URL\",\"password\":\"\"},\"machine\":{\"name\":\"vps\",\"addr\":\"world@10.8.0.5\",\"creds\":{\"kind\":\"key\",\"value\":\"к\"}}}"
    want_refusal no-password "снятие без пароля не бывает: им доказывают, что скоуп твой"

    call DELETE /api/scope "{\"scope\":{\"addr\":\"$SHARE_URL\",\"password\":\"не-та\"},\"machine\":{\"name\":\"vps\",\"addr\":\"world@10.8.0.5\",\"creds\":{\"kind\":\"key\",\"value\":\"к\"}}}"
    want_refusal bad-password "чужим паролем скоуп не снимается"

    call DELETE /api/scope "{\"scope\":{\"addr\":\"$SHARE_URL\",\"password\":\"$SHARE_PASS\"}}"
    want_refusal no-machine "снять раздачу можно только НА машине, и машину называют"

    # Личность в теле снятия — не поле, а недоразумение: снимают то, что лежит по адресу.
    call DELETE /api/scope "{\"scope\":{\"addr\":\"$SHARE_URL\",\"password\":\"$SHARE_PASS\"},\"identity\":{\"name\":\"я\"},\"machine\":{\"name\":\"vps\",\"addr\":\"world@10.8.0.5\",\"creds\":{\"kind\":\"key\",\"value\":\"к\"}}}"
    want_refusal bad-body "лишнее поле в снятии краснеет, а не проглатывается молча"

    call DELETE /api/scope "{\"scope\":{\"addr\":\"http://127.0.0.1:1/\",\"password\":\"$SHARE_PASS\"},\"machine\":{\"name\":\"vps\",\"addr\":\"world@10.8.0.5\",\"creds\":{\"kind\":\"key\",\"value\":\"к\"}}}"
    want_refusal no-share "раздачи по адресу нет — снимать нечего, и это сказано прямо"

    # ДО МАШИНЫ НЕ ДОЙТИ — это ступень СВЯЗИ, а не «пароль не тот»: заходить нам некуда.
    call DELETE /api/scope "{\"scope\":{\"addr\":\"$SHARE_URL\",\"password\":\"$SHARE_PASS\"},\"machine\":{\"name\":\"vps\",\"addr\":\"world@127.0.0.1:1\",\"creds\":{\"kind\":\"password\",\"value\":\"х\"}}}"
    case "$BODY" in
        *'"code":"no-answer"'*|*'"code":"no-route"'*)
            ok "до машины не дойти — названо ступенью связи, а не общим «не получилось»" ;;
        *)  bad "недостижимая машина при снятии" "ждали ступень связи, получили:" "$BODY" ;;
    esac
    if grep -q 'remote.sh drop' "$CALLS"; then
        bad "отказ снял раздачу" "до отказа успели снять вещь на машине:" "$(cat "$CALLS")"
    else
        ok "ни один отказ снятия не тронул раздачу"
    fi
...
unproven() {
    ...
    # НАСТОЯЩИЙ КОМПОУЗ НА НАСТОЯЩЕМ РЕЦЕПТЕ РАЗДАЧИ — то самое допущение, на котором стоит
    # `WORLD2-145`. Три состояния прогнаны на заглушке, а что живой компоуз и правда
    # отказывается читать файл без пароля и правда читает его с паролем — не проверено:
    # докера здесь нет. Назвать это надо вслух: проверка, стоящая на непроверенном допущении,
    # уже была мертва один раз ровно так.
    skip "живой docker compose на настоящем share/compose.yaml" \
        "три состояния прогнаны на подставном компоузе; настоящий здесь не запускался" \
        "живьём: SHARE_PASSWORD=x docker compose -f share/compose.yaml config --images — обязан назвать образ" \
        "и он же БЕЗ пароля — обязан отказать, а не отдать пустоту" \
        "порча: снять кавычки у значения с двоеточием в share/compose.yaml — точка входа обязана сказать «рецепт не читается»"
}
