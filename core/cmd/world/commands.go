...
func cmdJoin(s streams, args []string) error {
	...
	// локация переобъявляется на каждом подъёме, и требовать снятия перед
	// каждым значило бы вернуть тот самый ручной шаг, который здесь и убран.
	событие := "вошла в поле"
	if !вошли.Created {
		событие = "подтвердила присутствие в поле (в поле с " + вошли.Location.Since + ")"
	}
	fmt.Fprintf(s.out, "локация %q %s\n", вошли.Location.Name, событие)
	таблица(s.out, [][2]string{
		{"дверь", *doorAddr},
