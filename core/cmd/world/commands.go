...
//
//	без подкоманды (и `world door`) — ДВЕРЬ: мир, реестр локаций, приём стенда;
	...
	case "door":
		...
		if len(rest) > 0 {
			fmt.Fprintf(s.err, "world door: лишнее слово %q: дверь настраивается переменными среды "+
				"(WORLD_ADDR · WORLD_DOOR_FILE · WORLD_STAND_DIR), флагов у неё нет — см. core/README.md\n", rest[0])
			return 1
		}
		runDoor()
		return 0
...
Режимы:
  world                     без подкоманды — ДВЕРЬ: реестр локаций, маршруты
                            к ним, приём стенда (запуск как был, ничего не менялось).
                            Лица здесь нет: пульт раздаёт контроллер
...
Переменные режима двери (WORLD_DOOR_FILE, WORLD_STAND_DIR)
