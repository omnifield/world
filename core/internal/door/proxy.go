...
// forget убирает построенный маршрут — вызывается при СНЯТИИ локации: иначе
// пул соединений к ушедшему соседу и запись о нём жили бы до перезапуска
// двери, а поле уже другое.
