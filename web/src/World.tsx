...
//   • ПРИСОЕДИНЕНИЯ К КОМЬЮНИТИ И ЗАЯВЛЕНИЯ РЕСУРСА — ручки у контроллера есть (`WORLD2-122`),
//     и то и другое здесь ВИДНО, но не действием: `WORLD2-163` просил догнать форму соседа, а
//     не заводить два новых действия. Показать и смолчать было бы худшим исходом, поэтому
//     сказано и здесь, и в грамоте зоны;
 ...
 *
 * ЗАГОЛОВОК «РЕСУРС» У ОТВЕТА МАШИНЫ СМЕНИЛСЯ НА «МАШИНА» (`WORLD2-163`), и это не вкусовщина:
 * у участка появилось поле `resource` — ЗАЯВЛЕННЫЙ им ресурс. Две строки под одним словом
 * читались бы как одно и то же, а это разные вопросы: «дотянулись ли мы до машины» (измерено)
 * и «что участок сказал о себе сам» (заявлено).
 */
  ...
  return (
    <li class="pult__item" data-resource={props.res.name}>
      ...
      <dl class="pult__facts">
        ...
        <dt>комьюнити</dt>
        <dd>
          <Комьюнити fields={props.res.fields} />
        </dd>
        <dt>заявленный ресурс</dt>
        <dd>
          <Заявлено resource={props.res.resource} />
        </dd>
        <dt>вещи</dt>
        <dd>
...
/**
 * Комьюнити, в которых состоит участок (`WORLD2` 1.3, `2.5` п. 3).
 *
 * ПОКАЗЫВАЕМ, А НЕ МОЛЧИМ (`WORLD2-163`): поле приезжает в ответе, и «оно есть, но его не
 * видно» — худший из исходов. Действий здесь нет: присоединяет и отвязывает контроллер своими
 * ручками, а пульт показывает то, что тот отдал, — и это сказано в грамоте зоны, а не только
 * молчанием разметки.
 *
 * Пусто — ОБЫЧНОЕ состояние, а не «не спросили»: третьего исхода у принадлежности нет. И
 * говорится оно словами, а не пустым местом. Рядом сказано главное, чего человек мог бы
 * испугаться: адрес локации от комьюнити не двигается вовсе (`2.1` п. 5) — меняется только
 * то, кто вещь видит.
 */
function Комьюнити(props: { fields: string[] }) {
  return (
    <Show
      when={props.fields.length > 0}
      fallback={
        <span data-fields="none">
          ни в одном — и это законное состояние: адрес участка от комьюнити не зависит,
          меняется только то, кто его видит
        </span>
      }
    >
      <ul class="pult__things" data-fields="list">
        <For each={props.fields}>
          {(имя) => (
            <li data-in-field={имя}>
              <span class="pult__name">{имя}</span>
            </li>
          )}
        </For>
      </ul>
...
/**
 * Что участок ЗАЯВИЛ о себе сам — его словами (`WORLD2` 2.5 пп. 2, 6, 7).
 *
 * ЗАЯВЛЕНО, А НЕ ИЗМЕРЕНО, и отсюда запрет на экране: рядом не рисуется ничего, что выглядело
 * бы проверкой «хватит ли» — ни полоски, ни доли, ни «мало/достаточно». Мир ресурс не меряет и
 * ресурсом не отказывает: нехватка его — данность, а не отказ (`WORLD2` 0.2). Досчитанная нами
 * оценка была бы вторым источником правды рядом со словами юзера, и врала бы она ровно тогда,
 * когда на неё посмотрят.
 *
 * Строку показываем как есть — не сокращаем и не разбираем на цифры: цифр в ней может не быть
 * вовсе, а ресурс в цифрах даёт отдельный инструмент осмотра, и он отложен (`2.5` п. 10).
 */
function Заявлено(props: { resource: string }) {
  return (
    <Show
      when={props.resource}
      fallback={
        <span data-claim="none">
          не заявлял — о себе участок говорит сам, и за него мир не досчитывает
        </span>
      }
    >
      {(сказано) => <span data-claim="said">{сказано()}</span>}
    </Show>
  );
}
