...
describe("источники ресурса", () => {
  ...
  it("ресурса в цифрах на экране нет: только то, что сказал контроллер", async () => {
    // `WORLD2` 2.5: память и ядра даёт отдельный инструмент осмотра, и он отложен сознательно.
    // Появится здесь выдуманная цифра — проба покраснеет. Заголовок «машина» стоит там, где
    // раньше стоял «ресурс» (`WORLD2-163`): у участка появился ЗАЯВЛЕННЫЙ ресурс, и два разных
    // вопроса под одним словом читались бы как один.
    const { корень } = await показать({ resources: ok([УЧАСТОК, МОЛЧУН]) });
    ...
    expect(факты).toEqual(["адрес", "машина", "комьюнити", "заявленный ресурс", "вещи"]);
  });

  it("комьюнити участка ВИДНЫ — поле есть в ответе, и оно на экране", async () => {
    // Форма соседа поехала (`WORLD2-122`), и молчаливое «поле приезжает, но его не видно» —
    // худший из трёх исходов: человек не узнал бы, кто его участок видит, и решил бы, что
    // членства нет вовсе.
    const { корень } = await показать({ resources: ok([УЧАСТОК]) });
    const строка = корень.querySelector<HTMLElement>(`[data-resource='${УЧАСТОК.name}']`)!;

    expect(УЧАСТОК.fields.length).toBeGreaterThan(0);
    for (const имя of УЧАСТОК.fields) {
      expect(строка.querySelector(`[data-in-field='${имя}']`)).not.toBeNull();
    }
    expect(строка.querySelector("[data-fields]")?.getAttribute("data-fields")).toBe("list");
  });

  it("ни в одном комьюнити — сказано словами, а не пустым местом", async () => {
    // Пустой список у комьюнити — ОБЫЧНОЕ состояние, а не «не спросили» (`WORLD2` 2.5 п. 4):
    // третьего исхода тут нет. Пустая ячейка заставила бы гадать, отвалилось ли что-то.
    const { корень } = await показать({ resources: ok([МОЛЧУН]) });
    const пусто = корень.querySelector<HTMLElement>(
      `[data-resource='${МОЛЧУН.name}'] [data-fields]`,
    )!;

    expect(МОЛЧУН.fields).toEqual([]);
    expect(пусто.getAttribute("data-fields")).toBe("none");
    expect(пусто.textContent).toContain("ни в одном");
    // Адрес локации на комьюнити не смотрит вовсе (`WORLD2` 2.1 п. 5) — и это сказано рядом,
    // потому что «ушёл из комьюнити» человек прочитал бы как «переехал».
    expect(пусто.textContent).toContain("адрес участка от комьюнити не зависит");
  });

  it("заявленный ресурс показан словами участка — и пустой сказан словами", async () => {
    const { корень } = await показать({ resources: ok([УЧАСТОК, МОЛЧУН]) });
    const заявил = корень.querySelector<HTMLElement>(
      `[data-resource='${УЧАСТОК.name}'] [data-claim]`,
    )!;
    const молчит = корень.querySelector<HTMLElement>(
      `[data-resource='${МОЛЧУН.name}'] [data-claim]`,
    )!;

    expect(заявил.getAttribute("data-claim")).toBe("said");
    expect(заявил.textContent).toBe(УЧАСТОК.resource);
    expect(молчит.getAttribute("data-claim")).toBe("none");
    expect(молчит.textContent).toContain("не заявлял");
  });

  it("рядом с заявленным ресурсом нет проверки «хватит ли»", async () => {
    // ЗАЯВЛЕНО, А НЕ ИЗМЕРЕНО (`WORLD2` 2.5 пп. 2, 6, 7). Мир ресурс не меряет и им не
    // отказывает: нехватка ресурса — данность, а не отказ (`0.2`). Полоска, доля или слово
    // «достаточно» рядом с чужими словами были бы нашей оценкой, выданной за знание мира.
    const { корень } = await показать({ resources: ok([УЧАСТОК]) });
    const строка = корень.querySelector<HTMLElement>(`[data-resource='${УЧАСТОК.name}']`)!;

    expect(строка.querySelector("meter, progress, [role='progressbar']")).toBeNull();
    for (const вердикт of ["хватит", "хватает", "достаточно", "недостаточно", "мало"]) {
      expect(строка.textContent).not.toContain(вердикт);
    }
  });
