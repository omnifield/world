...
#
# Последним разделом проба стережёт не шлюз, а ШЕЛЛ, которым шлюз написан: код линтера,
# погашенный в зоне, не должен возвращаться молча, пока он выключен в работе CI на всё
# дерево (`WORLD2-157`). Линтера на машине нет — это тоже код 3, а не зелёное.
#
set -euo pipefail
export LC_ALL=C
...
# Линтер зоны гоняется в любом режиме: он не про поведение шлюза, а про шелл, которым шлюз
# написан, — и своя половина порчи у него внутри.
run_lint
